...
namespace Epsitec.Common.DocumentEditor.Dialogs
{
    ...
    {
        public Update(DocumentEditor editor)
            : base(editor) { }
        ...
        public override void Show()
        {
            ...
            this.version.Text =
                "<font size=\"120%\">"
                + string.Format(Res.Strings.Dialog.Update.Version, About.GetVersion())
                + "</font>";

            if (this.editor.InstallType == InstallType.Full)
            {
                string key = SerialAlgorithm.ReadCrDocSerial();
                string date = SerialAlgorithm
                    .GetExpirationDate(key)
                    .Subtract(new System.TimeSpan(1, 0, 0, 0))
                    .ToShortDateString();
                this.limit.Text =
                    "<font size=\"120%\">"
                    + string.Format(Res.Strings.Dialog.Update.Limit, date)
                    + "</font>";
            }
            else if (this.editor.InstallType == InstallType.Expired)
            {
                this.limit.Text =
                    "<font size=\"120%\">" + Res.Strings.Dialog.Update.Over + "</font>";
            }
            else
            {
                this.limit.Text = " ";
            ...
            b.Append(
                string.Format(
                    "{0}<tab/><a href=\"{1}\">{2}</a><br/>",
                    Res.Strings.Dialog.Update.Web1,
                    Res.Strings.Dialog.Update.Web2,
                    Res.Strings.Dialog.Update.Web3
                )
            );
            b.Append(chip);
            b.Append(
                string.Format(
                    "{0}<tab/><a href=\"{1}\">{2}</a><br/>",
                    Res.Strings.Dialog.Update.Mail1,
                    Res.Strings.Dialog.Update.Mail2,
                    Res.Strings.Dialog.Update.Mail3
                )
            );
            b.Append(chip);
            b.Append(
                string.Format(
                    "{0}<tab/>{1}<br/>",
                    Res.Strings.Dialog.Update.Address1,
                    Res.Strings.Dialog.Update.Address2
                )
            );
            b.Append(chip);
            b.Append(
                string.Format(
                    "{0}<tab/>{1}<br/>",
                    Res.Strings.Dialog.Update.Phone1,
                    Res.Strings.Dialog.Update.Phone2
                )
            );
            //-			b.Append(chip);
