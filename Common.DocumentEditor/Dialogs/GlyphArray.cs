...
namespace Epsitec.Common.DocumentEditor.Widgets
{
    ...
    public class GlyphArray
        : Epsitec.Common.Widgets.Widget,
            Epsitec.Common.Widgets.Helpers.IToolTipHost
    {
        public GlyphArray()
        {
            this.AutoFocus = true;
        ...
        public GlyphArray(Widget embedder)
            : this()
        {
        ...
        public int IndexToUnicode(int index)
        {
            ...
            if (index < 0 || index >= this.unicodes.Length)
                return 0;
                ...
                for (int i = first; i <= last; i++)
                {
                    if (!isSymbol)
                    {
                        if (i >= 0x0400 && i <= 0x04FF)
                            continue; // cyrillique ?
                        if (i >= 0x0500 && i <= 0x05FF)
                            continue; // araménien et hébreu ?
                        ...
                        if (i >= 0x0E00 && i <= 0x0EFF)
                            continue; // thaï ?
                        if (i >= 0x1000 && i <= 0x10FF)
                            continue; // géorgien ?
                        if (i >= 0x1100 && i <= 0x11FF)
                            continue; // jamos ?
                        if (i >= 0x3000 && i <= 0x30FF)
                            continue; // katakana ?
                        if (i >= 0x3100 && i <= 0x31FF)
                            continue; // bopomofo ?
                        if (i >= 0xE000 && i <= 0xF0FF)
                            continue; // réservé ?
                    }
        ...
        protected override void UpdateClientGeometry()
        {
            ...
            {
                this.scroller.SetManualBounds(
                    new Rectangle(
                        this.Client.Size.Width - this.scroller.ActualWidth,
                        0,
                        this.scroller.ActualWidth,
        ...
        protected virtual bool ProcessKeyEvent(Message message)
        {
            ...
            switch (message.KeyCode)
            {
                case KeyCode.ArrowLeft:
                    return this.MoveSelectedCell(-1);
                case KeyCode.ArrowRight:
                    return this.MoveSelectedCell(1);
                case KeyCode.ArrowUp:
                    return this.MoveSelectedCell(-2);
