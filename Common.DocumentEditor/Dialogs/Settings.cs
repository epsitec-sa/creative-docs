...
namespace Epsitec.Common.DocumentEditor.Dialogs
{
    ...
    {
        public Settings(DocumentEditor editor)
            : base(editor) { }
        ...
        public override void Show()
        {
            ...
            if (this.window == null)
            {
                ...
                this.window.Icon = Support.ImageProvider.Instance.GetImageFromManifestResource(
                    "Epsitec.Common.DocumentEditor.Images.Settings.icon",
                ...
                Common.Document.DocumentDialogs.CreateTitle(
                    bookGeneral,
                    Res.Strings.Dialog.Settings.Startup
                );

                combo = this.CreateCombo(
                    bookGeneral,
                    "FirstAction",
                    Res.Strings.Dialog.Settings.StartupAction
                );
                ...
                combo.SelectedItemIndex = GlobalSettings.FirstActionRank(
                    this.globalSettings.FirstAction
                );

                check = this.CreateCheck(
                    bookGeneral,
                    "SplashScreen",
                    Res.Strings.Dialog.Settings.SplashScreen
                );
                check.ActiveState = this.globalSettings.SplashScreen
                    ? ActiveState.Yes
                    : ActiveState.No;

                Common.Document.DocumentDialogs.CreateTitle(
                    bookGeneral,
                    Res.Strings.Dialog.Settings.AutoUpdate
                );

                check = this.CreateCheck(
                    bookGeneral,
                    "AutoChecker",
                    Res.Strings.Dialog.Settings.AutoChecker
                );
                check.ActiveState = this.globalSettings.AutoChecker
                    ? ActiveState.Yes
                    : ActiveState.No;

                Common.Document.DocumentDialogs.CreateTitle(
                    bookGeneral,
                    Res.Strings.Dialog.Settings.PanelProperties
                );

                check = this.CreateCheck(
                    bookGeneral,
                    "LabelProperties",
                    Res.Strings.Dialog.Settings.LabelProperties
                );
                check.ActiveState = this.globalSettings.LabelProperties
                    ? ActiveState.Yes
                    : ActiveState.No;
                ...
                Common.Document.DocumentDialogs.CreateTitle(
                    bookPeriph,
                    Res.Strings.Dialog.Settings.Mouse
                );

                combo = this.CreateCombo(
                    bookPeriph,
                    "MouseWheelAction",
                    Res.Strings.Dialog.Settings.MouseWheel
                );
                for (int i = 0; i < GlobalSettings.MouseWheelActionCount; i++)
                {
                    Common.Document.Settings.MouseWheelAction action =
                        GlobalSettings.MouseWheelActionType(i);
                    combo.Items.Add(GlobalSettings.MouseWheelActionString(action));
                }
                combo.SelectedItemIndex = GlobalSettings.MouseWheelActionRank(
                    this.globalSettings.MouseWheelAction
                );

                field = this.CreateField(
                    bookPeriph,
                    "DefaultZoom",
                    Res.Strings.Dialog.Settings.MouseZoom
                );
                field.MinValue = 1.1M;
                field.MaxValue = 4.0M;
                field.Step = 0.1M;
                ...
                check = this.CreateCheck(
                    bookPeriph,
                    "FineCursor",
                    Res.Strings.Dialog.Settings.FineCursor
                );
                check.ActiveState = this.globalSettings.FineCursor
                    ? ActiveState.Yes
                    : ActiveState.No;

                Common.Document.DocumentDialogs.CreateTitle(
                    bookPeriph,
                    Res.Strings.Dialog.Settings.Screen
                );

                combo = this.CreateCombo(
                    bookPeriph,
                    "Adorner",
                    Res.Strings.Dialog.Settings.Adorner
                );
                ...
                {
#if !DEBUG
                    if (name == "LookAquaMetal")
                        continue;
                    if (name == "LookAquaDyna")
                        continue;
                    if (name == "LookXP")
                        continue;
#endif
                ...
                field = this.CreateField(
                    bookPeriph,
                    "ScreenDpi",
                    Res.Strings.Dialog.Settings.ScreenDpi
                );
                ...
                Common.Document.DocumentDialogs.CreateTitle(
                    bookQuick,
                    Res.Strings.Dialog.Settings.TabPage.QuickHelp
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonQuickFirst,
                    Res.Strings.Dialog.Settings.QuickFirst
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonQuickDown,
                    Res.Strings.Dialog.Settings.QuickDown
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonQuickLast,
                    Res.Strings.Dialog.Settings.QuickLast
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonQuickDefault,
                    Res.Strings.Dialog.Settings.QuickDefault
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonQuickClear,
                    Res.Strings.Dialog.Settings.QuickClear
                );
                ...
                this.quickList.StyleV =
                    CellArrayStyles.ScrollNorm
                    | CellArrayStyles.Separator
                    | CellArrayStyles.SelectLine;
                ...
                Common.Document.DocumentDialogs.CreateTitle(
                    bookExport,
                    Res.Strings.Dialog.Settings.QuickExport
                );

                combo = this.CreateCombo(
                    bookExport,
                    "QuickExportFormat",
                    Res.Strings.Dialog.Settings.QuickExportType
                );
                ...
                combo.SelectedItemIndex = GlobalSettings.QuickExportFormatRank(
                    this.globalSettings.QuickExportFormat
                );

                field = this.CreateField(
                    bookExport,
                    "QuickExportDpi",
                    Res.Strings.Dialog.Settings.QuickExportDpi
                );
        ...
        private void HandleComboSettingsChanged(object sender)
        {
            ...
            {
                this.globalSettings.FirstAction = GlobalSettings.FirstActionType(
                    combo.SelectedItemIndex
                );
            }

            if (combo.Name == "MouseWheelAction")
            {
                this.globalSettings.MouseWheelAction = GlobalSettings.MouseWheelActionType(
                    combo.SelectedItemIndex
                );
            }
            ...
            {
                this.globalSettings.QuickExportFormat = GlobalSettings.QuickExportFormatType(
                    combo.SelectedItemIndex
                );
            }
        }
