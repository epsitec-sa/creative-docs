...
namespace Epsitec.Common.DocumentEditor.Dialogs
{
    ...
    {
        public Glyphs(DocumentEditor editor)
            : base(editor)
        {
            this.fontFace = "Arial";
        ...
        public override void Show()
        {
            ...
            if (this.window == null)
            {
                ...
                this.book.ActivePageChanged += new EventHandler<CancelEventArgs>(
                    this.HandleBookActivePageChanged
                );
                ...
                this.fieldFontFace.ComboOpening += new EventHandler<CancelEventArgs>(
                ...
                ToolTip.Default.SetToolTip(
                    this.buttonFilter,
                    DocumentEditor.GetRes("Action.TextFontFilter")
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.currentFont,
                    Res.Strings.Dialog.Glyphs.Tooltip.CurrentFont
                );
                ...
                ToolTip.Default.SetToolTip(
                    this.slider,
                    Res.Strings.Dialog.Glyphs.Tooltip.ArraySize
                );
        ...
        protected void UpdateAlternates()
        {
            ...
            if (document != null)
            {
                Common.Document.Objects.TextBox2 edit =
                    document.Modifier.RetEditObject() as Common.Document.Objects.TextBox2;
                if (edit != null)
                {
                    if (edit.EditGetSelectedGlyph(out code, out glyph, out font))
                    {
                        ...
                        foreach (string feature in supported)
                        {
                            ...
                            {
                                if (
                                    table.LookupType == Common.OpenType.LookupType.Alternate
                                    || table.LookupType == Common.OpenType.LookupType.Single
                                )
                                {
                                    list.Add(feature);
                                    break;
            ...
            else
            {
                ...
                if (codes != null)
                {
                    foreach (int code in codes)
                    {
                        ...
                        string complet = string.Format(
                            "  <font face=\"Arial\" size=\"120%\">{0}</font><tab/>{1}",
                            c.ToString(),
                            text
                        );
        ...
        protected int[] UnicodeList(int family)
        {
            ...
            if (family == 1)
                return UnicodeSpace;
            if (family == 2)
                return UnicodeBusiness;
            if (family == 3)
                return UnicodeMath;
            if (family == 4)
                return UnicodeGreekLower;
            if (family == 5)
                return UnicodeGreekUpper;
            if (family == 6)
                return UnicodeForm;
            return null;
        }
        ...
        {
            0x00A0,
            0x202F, // NoBreakSpace, NarrowNoBreakSpace
            0x2001,
            0x2000, // cadratin
            0x2004,
            0x2005,
            0x2006, // 1/3, 1/4, 1/6 cadratin
        ...
        protected int[] UnicodeForm =
        {
            ...
            0x2021, // obèle
            0x00BA,
            0x00AA, // indicateurs ordinal masculin/féminin
            0x2642,
            0x2640, // mâle/femelle
        };
        ...
        protected void EditInsert()
        {
            ...
            if (this.book.ActivePage.Name == "List")
            {
                ...
                if (family == 7) // textes de substitutions ?
                {
                    ...
                    {
                        case 0:
                            text = Res.Strings.Dialog.Glyphs.Substitute.Text.Latin;
                            break;
                        case 1:
                            text = Res.Strings.Dialog.Glyphs.Substitute.Text.French;
                            break;
                        case 2:
                            text = Res.Strings.Dialog.Glyphs.Substitute.Text.English;
                            break;
                    }

                    text = TextLayout.ConvertToSimpleText(text); // conversion des &quot; en "
                    this.editor.CurrentDocument.Modifier.EditInsertText(text, "", "");
                    this.editor.CurrentDocument.Modifier.EditInsertText(
                        Unicode.Code.ParagraphSeparator
                    );
                }
                else
                {
            ...
            if (this.book.ActivePage.Name == "Alternates")
            {
                ...
                this.editor.CurrentDocument.Modifier.EditInsertGlyph(
                    code,
                    glyph,
                    fontFace,
                    fontStyle
        ...
        protected void UpdateFontStyle()
        {
            ...
            Common.OpenType.FontIdentity[] list = TextContext.GetAvailableFontIdentities(
                this.fontFace
            );
            ...
            if (this.Array.SelectedItemIndex != -1)
            {
                if (this.book.ActivePage.Name == "Array")
                {
                    ...
                    text = string.Format(
                        "{0}: {1}",
                        code.ToString("X4"),
                        Misc.GetUnicodeName(code, this.fontFace, this.fontStyle)
                    );
                }
                ...
                if (this.book.ActivePage.Name == "Alternates")
                {
                    ...
                    text = string.Format(
                        ...
                        code.ToString("X4"),
                        glyph.ToString("X4"),
                        Misc.GetUnicodeName(code, fontFace, fontStyle)
                    );
                }
            }
