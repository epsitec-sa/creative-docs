...
namespace Epsitec.Common.FormEngine
{
    ...
    public sealed class Engine
    {
        public Engine(IFormResourceProvider resourceProvider)
        {
            ...
            var safeResourceResolver = new SafeResourceResolver(
        ...
        public UI.Panel CreateForm(FormDescription formDescription)
        {
            ...
            List<FieldDescription> baseFields,
                finalFields;
            Druid entityId;
            this.arrange.Build(
                formDescription,
        ...
        )
        {
            ...
            if (root is UI.Panel)
            {
                for (int i = index; i < fields.Count; i++)
                {
                    ...
                    if (
                        nextField != null
                        && nextField.Type == FieldDescription.FieldType.Glue
                        && !nextField.DeltaHidden
                    )
                    {
                        isGlueAfter = true;
                    }
                    ...
                    if (
                        field.Type == FieldDescription.FieldType.BoxBegin
                        || // début de boîte ?
                    ...
                    {
                        throw new System.InvalidOperationException(
                            "Type incorrect (la liste de FieldDescription devrait être aplatie)."
                        );
                    }
                }
                    ...
                    if (lastLabelId != labelsId[i])
                    {
                        ...
                        else
                        {
                            ...
                            grid.ColumnDefinitions.Add(
                                new Widgets.Layouts.ColumnDefinition(
                                    new Widgets.Layouts.GridLength(
                                        relWidth,
                                        Widgets.Layouts.GridUnitType.Proportional
                                    ),
                                    minWidth,
                            ...
                            System.Console.WriteLine(
                                string.Format("Column {0}: relWidth={1}%", i, relWidth)
                            );
                        }
                    }
                }
                System.Console.WriteLine(
                    string.Format("GridLayoutEngine with {0} columns", grid.ColumnDefinitions.Count)
                );
            ...
            for (int i = index; i < fields.Count; i++)
            {
                ...
                if (
                    nextField != null
                    && nextField.Type == FieldDescription.FieldType.BoxEnd
                    && !nextField.DeltaHidden
                )
                {
                    isLastOfBox = true;
        ...
        private void PreprocessBoxBegin(
        ...
        private static void LabelIdUse(List<int> labelsId, int labelId, int column, int count)
        {
        ...
        private static int GetColumnIndex(List<int> labelsId, int column)
        {
        ...
        private Widget CreateBox(
            Widget root,
            Widgets.Layouts.GridLayoutEngine grid,
            FieldDescription field,
            System.Guid guid,
            List<int> labelsId,
            ref int column,
        ...
        )
        {
            ...
            double mLeft =
                ((field.BoxFrameEdges & FrameEdges.Left) == 0) ? 0 : (field.BoxFrameWidth - 1) / 2;
            double mRight =
                ((field.BoxFrameEdges & FrameEdges.Right) == 0) ? 0 : (field.BoxFrameWidth - 1) / 2;
            double mTop =
                ((field.BoxFrameEdges & FrameEdges.Top) == 0) ? 0 : (field.BoxFrameWidth - 1) / 2;
            double mBottom =
                ((field.BoxFrameEdges & FrameEdges.Bottom) == 0)
                    ? 0
                    : (field.BoxFrameWidth - 1) / 2;
            ...
            grid.RowDefinitions[row].BottomBorder = FieldDescription.GetRealSeparator(
                field.SeparatorBottom,
                isLastOfBox
            );
        ...
        )
        {
            ...
            switch (editionMode)
            {
                ...
                default:
                    throw new System.InvalidOperationException(
                        string.Format("Invalid edition mode {0}", editionMode)
                    );
            }
            ...
            else
            {
                ...
                if (columnsRequired == 1) // tout sur une seule colonne ?
                {
                    ...
                    placeholder.ControllerParameters = UI.Controllers
                        .AbstractController
                        .NoLabelsParameter; // cache le label
                }
        ...
        private UI.Placeholder CreatePlaceholder(
            Widget root,
            Druid entityId,
            FieldDescription field
        )
        {
            ...
            switch (fieldDef.Relation)
            {
                case FieldRelation.None:
                    placeholder = new UI.Placeholder() { Embedder = root };
                    placeholder.SetBinding(
                        UI.Placeholder.ValueProperty,
                        new Binding(BindingMode.TwoWay, field.GetPath(DataSource.DataName))
                    );
                    break;
                ...
                case FieldRelation.Collection:
                    ...
                    placeholder.SetBinding(
                        UI.CollectionPlaceholder.CollectionProperty,
                        new Binding(BindingMode.OneWay, field.GetPath(DataSource.DataName))
                    );
                    break;
            }
        ...
        private void CreateSeparator(
            ...
            FieldDescription field,
            System.Guid guid,
            FieldDescription nextField,
            List<int> labelsId,
            ref int column,
            ref int row,
            bool isGlueAfter,
            ref List<Druid> lastTitle
        )
        {
            ...
            if (type == FieldDescription.FieldType.Line || type == FieldDescription.FieldType.Title)
            {
                if (grid != null)
                {
                    grid.RowDefinitions.Add(new Widgets.Layouts.RowDefinition());
                    grid.RowDefinitions[row].TopBorder =
                        (type == FieldDescription.FieldType.Title) ? 0 : m + 1;
        ...
        private void ApplyFontTextStyle(
            TextStyle textStyle,
            FieldDescription.FontFaceType face,
            FieldDescription.FontStyleType style
        )
        {
            if (
                face != FieldDescription.FontFaceType.Default
        ...
        private static int CountFields(List<FieldDescription> fields, int index)
        {
            ...
            {
                if (
                    fields[i].Type == FieldDescription.FieldType.Field
        ...
        private static FieldDescription SearchNextElement(List<FieldDescription> fields, int index)
        {
            //	Cherche le prochain élément.
            if (
                fields[index].Type == FieldDescription.FieldType.BoxBegin
            ...
            )
            {
                ...
                {
                    if (
                        fields[i].Type == FieldDescription.FieldType.BoxBegin
        ...
        private static FieldDescription SearchNextField(List<FieldDescription> fields, int index)
        {
