...
namespace Epsitec.Common.BigList
{
    public class ItemListSelection : System.IDisposable
    {
        ...
        {
            get { return this.selectedItemCount; }
        }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (
                    this.ChangeFlagState(
                        i,
                        x => x.Select(ItemSelection.Deselect),
                        ItemStateDetails.IgnoreNull
                    )
                )
                {
                    e.Add(i, false);
        ...
        private bool ChangeFlagState(
            int index,
            System.Action<ItemState> action,
            ItemStateDetails flags = ItemStateDetails.None
        )
        {
