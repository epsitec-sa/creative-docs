    ...
    public abstract class AbstractItemViewFactory : IItemViewFactory
    {
                ...
                switch (panel.GetPanelLayout())
                {
                    ...
                    default:
                        throw new System.NotSupportedException(
                            string.Format("Layout {0} not supported", panel.GetPanelLayout())
                        );
                }
            }
        ...
        protected ItemViewWidget CreateElements(ItemView itemView)
        {
            ...
            switch (shape)
            {
                ...
                default:
                    throw new System.NotSupportedException(
                        string.Format("ItemViewShape {0} not supported", panel.ItemViewShape)
                    );
            }

            if (shape == ItemViewShape.Tile)
            {
                viewContainer.Padding = new Drawing.Margins(
                    2,
                    2,
                    AbstractItemViewFactory.TileMargin,
                    AbstractItemViewFactory.TileMargin
                );
            }
            ...
            for (int i = 0; i < header.ColumnCount; i++)
            {
                ...
                Widgets.Widget element = this.CreateElement(
                    name,
                    panel,
                    itemView,
                    ItemViewShape.ToolTip
                );
        ...
        /// </returns>
        protected abstract Widgets.Widget CreateElement(
            string name,
            ItemPanel panel,
            ItemView view,
            ItemViewShape shape
