...
namespace Epsitec.Common.UI.Controllers
{
    ...
    public abstract class AbstractController : Types.DependencyObject, IController, IGridPermeable
    {
        ...
        {
            Support.EventHandler handler = (Support.EventHandler)
                this.GetUserEventHandler(EventNames.ActualValueChanged);
        ...
        public virtual IEnumerable<PermeableCell> GetChildren(
            int column,
            int row,
            int columnSpan,
            int rowSpan
        )
        {
            ...
            if (columnSpan < count)
            {
                throw new System.ArgumentException(
                    string.Format(
                        "Not enough columns for content; got {0} but at least {1} needed",
                        columnSpan,
                        count
                    )
                );
            ...
            foreach (WidgetRecord record in this.widgets)
            {
                if (record.Widget.Visibility)
                {
                    if (index == count - 1)
                    {
                        yield return new Widgets.Layouts.PermeableCell(
                            record.Widget,
                            column + index,
                            row + 0,
                            columnSpan - index,
        ...
        {
            add { this.AddUserEventHandler(EventNames.ActualValueChanged, value); }
            remove { this.RemoveUserEventHandler(EventNames.ActualValueChanged, value); }
        }
        ...
        public static readonly DependencyProperty PlaceholderProperty = DependencyProperty.Register(
            ...
            new DependencyPropertyMetadata(
                AbstractController.GetPlaceholderValue,
                AbstractController.SetPlaceholderValue
            )
        );
