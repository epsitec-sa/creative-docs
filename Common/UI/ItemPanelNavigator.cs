    ...
    public class ItemPanelNavigator
    {
        ...
        private ItemView FindNeighbour(Widgets.Direction direction)
        {
            ...
            while (panel != null)
            {
                ...
                if ((group == null) || (group.ItemView == null))
                {
                    break;
                }
        ...
        private ItemView FindNeighbour(
            Widgets.Direction direction,
            ItemPanel panel,
            List<ItemPanel> exclusionList
        )
        {
            BestRecord best = new BestRecord();

            System.Predicate<ItemView> viewPredicate =
                this.GetMatchItemViewFocusBoundsPredicateAndRecordBest(direction, best);
            System.Predicate<ItemView> groupPredicate = this.GetMatchItemViewBoundsPredicate(
                direction,
                exclusionList
            );
        ...
        {
            if ((view.IsGroup) && (view.IsExpanded))
            {
                return false;
            }
            else
        ...
        private System.Predicate<ItemView> GetMatchItemViewFocusBoundsPredicateAndRecordBest(
            Widgets.Direction direction,
            BestRecord best
        )
        {
                ...
                case Epsitec.Common.Widgets.Direction.Up:
                    predicate = delegate(ItemView view)
                    {
                        if (this.FilterGroups(view))
                        {
                            ...
                            double overlap = ItemPanelNavigator.GetOverlap(
                                bounds.Left,
                                bounds.Right,
                                this.currentX1,
                                this.currentX2
        ...
        private System.Predicate<ItemView> GetMatchItemViewBoundsPredicate(
            Widgets.Direction direction,
            IList<ItemPanel> exclusionList
        )
        {
            System.Predicate<ItemView> predicate;
            ...
            switch (direction)
            {
                case Epsitec.Common.Widgets.Direction.Up:
                    predicate = delegate(ItemView view)
                    {
                        if ((!view.IsExpanded) || (exclusionList.Contains(view.Owner)))
                        {
                            return false;
                        }
        ...
        private void UpdateCurrentPosition()
        {
            ...
            {
                Drawing.Rectangle bounds = this.MapToRoot(
                    this.currentPanel,
                    this.currentView.FocusBounds
                );
