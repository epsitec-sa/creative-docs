...
namespace Epsitec.Common.Support
{
    ...
    public class ResourceBundleCollection : ICollection<ResourceBundle>, IList<ResourceBundle>
    {
            ...
            set
            {
                if (this[index] != value)
                {
                    throw new System.InvalidOperationException(
                        "ResourceBundle may not be replaced."
                    );
                }
            }
        }
        ...
        {
            get { return this[level, this.manager.ActiveCulture]; }
        }
        public ResourceBundle this[ResourceLevel level, CultureInfo culture]
        {
            get
            {
                ...
                {
                    if (
                        Resources.EqualCultures(
                            bundle.ResourceLevel,
                            bundle.Culture,
                            level,
                            culture
                        )
                ...
                for (int i = 1; i < ids.Length; i++)
                {
                    if (Resources.StripSuffix(Resources.ExtractName(ids[i])) != name)
                    {
                        throw new System.ArgumentException(
                            string.Format(
                                "Invalid name '{0}' in argument ids[{1}]",
                                Resources.StripSuffix(Resources.ExtractName(ids[i])),
                                i
                            )
                        );
                    }
