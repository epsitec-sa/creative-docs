...
namespace Epsitec.Common.Support
{
    ...
    public static class Resources
    {
        static Resources()
        {
            ...
            Resources.cultures = names
                .Select(code => CultureInfo.GetCultureInfoByIetfLanguageTag(code))
                .ToList();
        ...
        public static bool SplitFieldIdWithoutIdResolution(
            string id,
            out string bundle,
            out string field
        )
        ...
        private static CultureInfo InternalFindSpecificCultureInfo(string twoLetterCode)
        {
            ...
            for (int i = 0; i < cultures.Length; i++)
            {
                ...
                if (
                    (item.TwoLetterISOLanguageName == twoLetterCode)
                    && (name.Length == 5)
                    && (name[2] == '-')
                )
                {
            ...
            if (found.Count > 0)
            {
                foreach (CultureInfo item in found)
                {
                    ...
                    if ((name[3] == name[0]) && (name[4] == name[1]))
                    {
                        return item;
                    }
                }
        ...
        public static bool EqualCultures(
            ResourceLevel levelA,
            CultureInfo cultureA,
            ResourceLevel levelB,
            CultureInfo cultureB
        )
        {
            if (levelA != levelB)
            {
        ...
        public static bool EqualCultures(CultureInfo cultureA, CultureInfo cultureB)
        {
            ...
            if ((cultureA == null) || (cultureB == null))
            {
                return false;
            }
        ...
        internal static string CreateBundleKey(
            string prefix,
        ...
        internal static string CreateCaptionKey(
            Druid druid,
            ResourceLevel level,
            CultureInfo culture
        )
        ...
        private static void Rebinder(object resourceManager, Types.ResourceBinding binding)
        {
            ...
            if (that.SetResourceBinding(binding) == false)
            {
                throw new ResourceException(
                    string.Format("Cannot bind to ressource '{0}'", binding.ResourceId)
                );
            }
        }
        ...
        private static readonly ResourceProviderFactory factory;
