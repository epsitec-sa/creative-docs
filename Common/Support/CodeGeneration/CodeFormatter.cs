...
namespace Epsitec.Common.Support.CodeGeneration
{
    ...
    public class CodeFormatter : System.IDisposable
    {
        ...
        {
            get { return this.indentationChars; }
            set
            {
                if (this.indentationChars != value)
                {
                    if (this.indentationLevel > 0)
                    {
                        throw new System.InvalidOperationException(
                            "Cannot change IndentationChars while outputting code"
                        );
                    }
        ...
        {
            get { return (this.isInClassCount > 0) && (!this.isInInterface); }
        }
        ...
        public void WriteBeginInterface(
            CodeAttributes attributes,
            string code,
            IEnumerable<string> specifiers
        )
        ...
        public void WriteField(CodeAttributes attributes, params string[] fragments)
        {
            ...
            {
                throw new System.InvalidOperationException(
                    "Trying to generate field for an interface"
                );
            }
        ...
        public void WriteCodeLine(string line)
        {
            ...
            if ((this.isAbstract) && (trimmed.Length > 0))
            {
                if (
                    (trimmed.StartsWith("#"))
                    || (trimmed.StartsWith("//"))
        ...
        private int FindCodeStartPos()
        {
            ...
            while (pos < this.output.Length)
            {
                switch (this.output[pos])
                {
                    ...
                    case '/':
                        if ((pos < this.output.Length - 1) && (this.output[pos + 1] == '/'))
                        {
                            inComment = true;
                            pos += 2;
                            continue;
        ...
        private void UpdateIsAbstract(CodeAttributes attributes)
        {
            ...
            {
                this.isAbstract =
                    (attributes.Accessibility == CodeAccessibility.Abstract)
                    || (attributes.IsPartialDefinition);
            }
            else
            {
                this.isAbstract = false;
        ...
        private void WriteBeginBlockOrSemicolumnIfAbstract(CodeAttributes attributes, string code)
        {
            ...
            )
            {
                ...
                else
                {
                    throw new System.InvalidOperationException(
                        string.Format(
                            "Partial method defined with {0} visibility",
                ...
                if (
                    (attributes.Accessibility == CodeAccessibility.Final)
                ...
                else
                {
                    throw new System.InvalidOperationException(
                        string.Format(
                            "Partial method defined with {0} accessibility",
        ...
        {
            ElementState previousState =
                this.elementStates.Count == 0 ? ElementState.None : this.elementStates.Peek();
            ...
            switch (state)
            {
                ...
                case ElementState.Property:
                    if (
                        (previousState == ElementState.Class)
                    ...
                    {
                        throw new System.InvalidOperationException(
                            string.Format("{0} not defined in a class or an interface", state)
                        );
                    }
                    break;
                ...
                case ElementState.PropertySetter:
                    ...
                    {
                        throw new System.InvalidOperationException(
                            string.Format("{0} not defined in a property", state)
                        );
                    }
                    break;
                ...
                case ElementState.Namespace:
                    ...
                    {
                        throw new System.InvalidOperationException(
                            string.Format("Namespace cannot be defined in {0}", previousState)
                        );
                    }
                    break;
                ...
                case ElementState.Interface:
                    ...
                    else
                    {
                        throw new System.InvalidOperationException(
                            string.Format(
                                "Trying to define an interface in {0}, not a class or a namespace",
                                previousState
                            )
                        );
                    }
                ...
                case ElementState.Class:
                    ...
                    else
                    {
                        throw new System.InvalidOperationException(
                            string.Format(
                                "Trying to define a class in {0}, not a class or a namespace",
                                previousState
                            )
                        );
                    }
                ...
                case ElementState.InstanceVariable:
                    ...
                    else
                    {
                        throw new System.InvalidOperationException(
                            string.Format(
                                "Trying to define an instance variable in {0}, not a class",
                                previousState
                            )
                        );
                    }
        ...
        private void PopElementState(ElementState state)
        {
            ...
            {
                throw new System.InvalidOperationException(
                    string.Format("Ending element {0}, but none was started", state)
                );
            }
            ...
            throw new System.InvalidOperationException(
                string.Format("Ending element {0}, but expected {1}", state, expectedState)
            );
        }
