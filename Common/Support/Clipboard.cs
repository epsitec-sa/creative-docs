                        ...
                        foreach (Capture styleCapture in styleCaptures)
                        {
                            ...
                            else
                            {
                                ...
                                )
                                {
                                    ...
                                    try
                                    {
                                        Types.InvariantConverter.Convert(
                                            matchOpt.Groups["opt"].Captures[0].Value,
                                            out n
                                        );
                ...
                else if ((c == ';') && (lastElem >= 0))
                {
                    ...
                    else
                    {
                        throw new System.FormatException(
                            string.Format("Illegal entity {0} found.", elem)
                        );
                    }
                }
            }
...
*/
        ...
        static Clipboard()
        {
            ...
            Clipboard.regexStyle = new Regex(
                @"\A<span((\s*style\s*=\s*(?<a>['""])(?<style>.*?)\k<a>)|(\s*\w*)|(\s*\w*\s*=\s*[^\s>]*?\s*))*\s*\>\Z",
                RegexOptions.Compiled | RegexOptions.Multiline
            );
            Clipboard.regexMsoSpacerun = new Regex(
                @"\Amso\-spacerun\:\s*(?<opt>\w*)\s*\Z",
                RegexOptions.Compiled | RegexOptions.Multiline
            );
            Clipboard.regexMsoTabcount = new Regex(
                @"\Amso\-tab\-count\:\s*(?<opt>\w*)\s*\Z",
                RegexOptions.Compiled | RegexOptions.Multiline
            );
