...
namespace Epsitec.Common.Support.ResourceAccessors
{
    ...
    public class StringResourceAccessor : AbstractResourceAccessor
    {
        ...
        )
        {
            ...
            if ((refField.IsEmpty) && (patchField.IsEmpty))
            {
        ...
        {
            ResourceBundle bundle1 = this.ResourceManager.GetBundle(
                this.GetBundleName(),
            ...
            if (this.ResourceManager.BasedOnPatchModule)
            {
                bundle2 = this
                    .ResourceManager.GetManagerForReferenceModule()
                    .GetBundle(this.GetBundleName(), ResourceLevel.Default);
            }
        ...
        protected override void PersistItem(CultureMap item)
        {
            ...
            foreach (string twoLetterISOLanguageName in item.GetDefinedCultures())
            {
                ...
                if (usePatchModule)
                {
                    ...
                    if (
                        (
                            StringResourceAccessor.ComputeDelta(
                                ref data,
                                refModuleManager.GetBundle(this.GetBundleName(), level, culture),
                                item.Id
                            )
        ...
        )
        {
            ...
            else
            {
                ...
                if (item.IsCultureDefined(twoLetterISOLanguageName))
                {
                    ...
                    {
                        oldData.SetValue(
                            Res.Fields.ResourceString.Text,
                            newData.GetValue(Res.Fields.ResourceString.Text)
                        );
                    }
                    if (field.About != null)
                    {
                        oldData.SetValue(
                            ...
                            newData.GetValue(Res.Fields.ResourceBase.Comment)
                        );
                    }
                    if (field.ModificationId > 0)
                    {
                        oldData.SetValue(
                            ...
                            newData.GetValue(Res.Fields.ResourceBase.ModificationId)
                        );
                    }
        ...
        /// otherwise, <c>false</c>.
        /// </returns>
        private static bool ComputeDelta(
            ref StructuredData data,
            ResourceBundle refBundle,
            Druid druid
        )
        {
        ...
        private static void FillDataFromField(ResourceBundle.Field field, Types.StructuredData data)
        {
            data.SetValue(
                Res.Fields.ResourceString.Text,
                ResourceBundle.Field.IsNullString(field.AsString) ? null : field.AsString
            );
            data.SetValue(
                Res.Fields.ResourceBase.Comment,
        ...
        internal static int GetModificationId(StructuredData data)
        {
            ...
            {
                return StringResourceAccessor.GetModificationId(
                    data.GetValue(Res.Fields.ResourceBase.ModificationId)
                );
            }
        }
        ...
        {
            if ((UndefinedValue.IsUndefinedValue(modId)) || (modId == null))
            {
                return 0;
            }
            else
