    ...
    public class CodeProject
    {
                ...
                if (this.templateItems.TryGetValue(item, out oldValue))
                {
                    switch (item)
                    {
                        ...
                        case TemplateItem.ReferenceInsertionPoint:
                            this.templateItems[item] = string.Concat(
                                oldValue,
                                CodeProject.LineSeparator,
                                newValue
                            );
                            break;
                        ...
                        default:
                            throw new System.NotSupportedException(
                                string.Format("Cannot add duplicate item {0}", item)
                            );
                    }
                }
                else
        ...
        private static string GetProjectFileTemplate()
        {
            ...
            using (
                System.IO.Stream resourceStream = assembly.GetManifestResourceStream(
                    "Epsitec.Common.Support.Resources.ProjectFileTemplate.xml"
                )
            )
            {
                using (
                    System.IO.StreamReader resourceReader = new System.IO.StreamReader(
                        resourceStream,
        ...
        private static string Replace(string source, TemplateItem item, string value)
        {
            ...
            else
            {
                ...
                foreach (
                    string line in value.Split(
                        new string[] { CodeProject.LineSeparator },
