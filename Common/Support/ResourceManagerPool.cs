...
namespace Epsitec.Common.Support
{
    ...
    public sealed class ResourceManagerPool : DependencyObject
    {
                ...
                if (this.defaultPrefix != value)
                {
                    if (this.defaultPrefix != null)
                    {
                        throw new System.InvalidOperationException(
                            "The default prefix may not be changed"
                        );
                    }
        ...
        {
            get { return this.modules.Values; }
        }
        ...
        {
            get { return this.resourceProbingPaths; }
        }
        ...
        public IEnumerable<ResourceBundle> FindAllLoadedBundles(
            System.Predicate<ResourceBundle> filter
        )
        {
        ...
        public void SetupDefaultRootPaths()
        {
            ...
            this.AddModuleRootPath(
                SymbolicNames.Custom,
                System.IO.Path.Combine(userPath, ResourceManagerPool.CustomModulesDirectory),
                true
            );
            this.AddModuleRootPath(
                SymbolicNames.Library,
                System.IO.Path.Combine(userPath, ResourceManagerPool.LibraryModulesDirectory),
                true
            );
            this.AddModuleRootPath(
                SymbolicNames.Patches,
                System.IO.Path.Combine(userPath, ResourceManagerPool.PatchModulesDirectory),
                true
            );
            this.AddModuleRootPath(
                SymbolicNames.Live,
                System.IO.Path.Combine(userPath, ResourceManagerPool.LiveModulesDirectory),
                true
            );
        }
        ...
        /// otherwise.
        /// </returns>
        public bool AddModuleRootPath(
            string symbolicName,
            string path,
            bool tolerateMissingDirectories
        )
        {
        ...
        public string GetRootRelativePath(string path)
        {
            ...
            {
                if ((item.Value.Length > root.Length) && (path.StartsWith(item.Value)))
                {
                    root = item.Value;
                    prefix = item.Key;
        ...
        public string GetRootRelativePath(ResourceModuleInfo module, string path)
        {
            ...
            )
            {
                return this.GetRootRelativePath(
                    string.Concat(module.FullId.Path, System.IO.Path.DirectorySeparatorChar, path)
                );
            }
            else
            {
        ...
        public string GetRootAbsolutePath(string path)
        {
            ...
            foreach (KeyValuePair<string, string> item in this.moduleRoots)
            {
                if ((item.Key.Length > root.Length) && (path.StartsWith(item.Key)))
                {
                    root = item.Key;
                    prefix = item.Value;
                    result = path.Substring(root.Length);
            ...
            if (result.StartsWith("%"))
            {
                throw new System.ArgumentException(
                    string.Format("Path '{0}' cannot be resolved", path)
                );
            }
        ...
        public string GetRootAbsolutePath(ResourceModuleInfo module, string path)
        {
            ...
            {
                return this.GetRootAbsolutePath(
                    string.Concat(module.FullId.Path, System.IO.Path.DirectorySeparatorChar, path)
                );
            }
            else
        ...
        {
            foreach (
                string path in ResourceModule.FindModulePaths(this.GetRootAbsolutePath(rootPath))
            )
            {
        ...
        public void ScanForAllModules()
        {
            ...
            {
                if ((string.IsNullOrEmpty(root.Key)) || (string.IsNullOrEmpty(root.Value)))
                {
                    continue;
                }
        ...
        /// A (possibly empty) list of <see cref="ResourceModuleInfo"/> instances.
        /// </returns>
        public IList<ResourceModuleInfo> FindModuleInfos(
            System.Predicate<ResourceModuleInfo> predicate
        )
        {
        ...
        {
            get { return ResourceManagerPool.defaultPool; }
            set { ResourceManagerPool.defaultPool = value; }
        }
        ...
        readonly Dictionary<string, ResourceBundle> bundles =
            new Dictionary<string, ResourceBundle>();
        readonly List<Weak<ResourceManager>> managers = new List<Weak<ResourceManager>>();
        int mergedBundlesCount;
        readonly Dictionary<string, ResourceModuleInfo> modules =
            new Dictionary<string, ResourceModuleInfo>();
