...
namespace Epsitec.Common.Support.EntityEngine
{
    ...
    public class EntityContext : IEntityPersistenceManager
    {
        ...
        public EntityContext(
            SafeResourceResolver resourceResolver,
            EntityLoopHandlingMode loopHandlingMode = EntityLoopHandlingMode.Throw,
            string name = null
        )
        {
            this.name = name;
        ...
        {
            get { return this.dataGeneration; }
        }
        ...
        {
            get { return !this.suspendConstraintChecking.IsZero; }
        }
        ...
        {
            get { return this.persistenceManagers; }
        }
        ...
        internal TResult EvaluateFunc<TResult>(
                ...
                return manager.Execute(
                    ...
                    () =>
                        string.Format(
                            "Entity {0} ({1}), field {2}",
                            entityType.FullName,
                            entity.GetEntityStructuredTypeId(),
                            id
                        )
                );
            }
        ...
        internal IValueStore CreateValueStore(AbstractEntity entity)
        {
            ...
            if (type == null)
            {
                ...
                if ((genericEntity == null) || (typeId.IsValid))
                {
                    throw new System.NotSupportedException(
                        string.Format("Type {0} is not supported", typeId)
                    );
                }
            }
        ...
        {
            AbstractEntity entity =
                EntityClassFactory.CreateEmptyEntity(entityId)
                ?? this.CreateGenericEntity(entityId);
        ...
        private void CreateRelatedEntities(AbstractEntity entity, Stack<Druid> parents)
        {
            ...
            foreach (string id in type.GetFieldIds())
            {
                ...
                if ((field.Relation == FieldRelation.Reference) && (field.IsNullable == false))
                {
        ...
        private AbstractEntity HandleGraphLoop(Stack<Druid> parents, Druid entityId)
        {
            ...
            else
            {
                System.Diagnostics.Debug.Assert(
                    this.loopHandlingMode == EntityLoopHandlingMode.Skip
                );
        ...
        internal PropertySetter FindPropertySetter(AbstractEntity entity, string id)
        {
            ...
            this.propertySetters[key] = setter = DynamicCodeFactory.CreatePropertySetter(
                this.FindPropertyInfo(entity, id)
            );
        ...
        internal PropertyGetter FindPropertyGetter(AbstractEntity entity, string id)
        {
            ...
            this.propertyGetters[key] = getter = DynamicCodeFactory.CreatePropertyGetter(
                this.FindPropertyInfo(entity, id)
            );
        ...
        private System.Reflection.PropertyInfo FindPropertyInfo(AbstractEntity entity, string id)
        {
            ...
            System.Reflection.PropertyInfo[] propertyInfos = entityType.GetProperties(
            ...
            {
                foreach (
                    EntityFieldAttribute attribute in propertyInfo.GetCustomAttributes(
                        typeof(EntityFieldAttribute),
                        true
                    )
                )
                {
        ...
        internal void NotifyEntityChanged(
            AbstractEntity entity,
            string id,
            object oldValue,
            object newValue
        ...
        private bool IsNullable(StructuredTypeField field)
        {
            ...
            if ((nullableType != null) && (nullableType.IsNullable))
            {
                return true;
            }
            else
