...
namespace Epsitec.Common.Support.EntityEngine
{
    ...
    public static class EntityInfo
    {
        ...
        public static StructuredTypeField GetStructuredTypeField(
            Expression propertyLambdaExpression
        )
        {
            return EntityInfo.GetStructuredTypeField(
                ExpressionAnalyzer.GetLambdaPropertyInfo(propertyLambdaExpression)
            );
        }
        ...
        {
            if ((entityId.IsEmpty) || (string.IsNullOrEmpty(fieldId)))
            {
                return null;
            }
        ...
        {
            return EntityInfo.GetFieldCaptionId(
                ExpressionAnalyzer.GetLambdaPropertyInfo(propertyLambdaExpression)
            );
        }
        ...
        public static Druid GetFieldCaptionId(PropertyInfo propertyInfo)
        {
            ...
            var fieldAttribute = propertyInfo
                ...
                .OfType<EntityFieldAttribute>()
                .FirstOrDefault();

            if ((fieldAttribute == null) && (propertyInfo.Name.EndsWith("ForEdition")))
            {
                return EntityInfo.GetFieldCaptionId(
                    propertyInfo.DeclaringType.GetProperty(
                        propertyInfo.Name.StripSuffix("ForEdition")
                    )
                );
            }

            if ((fieldAttribute == null) || (fieldAttribute.FieldId == null))
            {
                return Druid.Empty;
            }
        ...
        {
            return EntityInfo.GetFieldCaption(
                ExpressionAnalyzer.GetLambdaPropertyInfo(propertyLambdaExpression)
            );
        }
        ...
        /// </returns>
        public static AbstractEntity WalkEntityGraph(
            AbstractEntity rootEntity,
            IEnumerable<PropertyInfo> fieldPath,
            NullNodeAction nullNodeAction
        )
        {
            var node = rootEntity;
            ...
            foreach (var fieldPropertyInfo in fieldPath)
            {
                ...
                {
                    var child =
                        fieldPropertyInfo.GetValue(node, EntityInfo.emptyIndexArray)
                        as AbstractEntity;
                        ...
                        switch (nullNodeAction)
                        {
                            ...
                            default:
                                throw new System.NotImplementedException(
                                    string.Format(
                                        "{0} not implemented",
                                        nullNodeAction.GetQualifiedName()
                                    )
                                );
                        }

                        child =
                            System.Activator.CreateInstance(fieldPropertyInfo.PropertyType)
                            as AbstractEntity;
