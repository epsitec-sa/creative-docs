    ...
    public static class AssemblyLoader
    {
        static AssemblyLoader()
        {
            System.AppDomain.CurrentDomain.AssemblyLoad +=
                AssemblyLoader.HandleCurrentDomainAssemblyLoad;
        ...
        {
            get { return AssemblyLoader.recursionCount.IsNotZero; }
        }
        ...
        /// The assembly if it could be found; otherwise, <c>null</c>.
        /// </returns>
        public static Assembly LoadFromPath(
            string name,
            string loadPath,
            bool loadDependencies = false
        )
        {
            ...
            using (
                AssemblyLoader
                    .recursionCount.Enter()
                    .AndFinally(AssemblyLoader.DispatchPendingEvents)
            )
            {
                Assembly assembly = null;

                if (
                    (string.IsNullOrEmpty(loadPath) == false)
                        ...
                        if (System.IO.File.Exists(fullName))
                        {
                            ...
                            catch (System.Exception ex)
                            {
                                System.Diagnostics.Debug.WriteLine(
                                    "Could not load assembly from file "
                                        + fullName
                                        + " : "
                                        + ex.Message
                                );
        ...
        private static void LoadDependencies(Assembly assembly, string loadPath = null)
        {
            ...
            foreach (var name in assembly.GetReferencedAssemblies())
            {
                ...
                if ((Comparer.Equal(token, token1)) || (Comparer.Equal(token, token2)))
                {
                    if (
                        AssemblyLoader.LoadFromPath(name.Name, loadPath, loadDependencies: true)
                        == null
                    )
                    {
                        System.Diagnostics.Debug.WriteLine(
                            string.Format(
                                "Failed to load assembly {0} referenced by {1}",
                                name.FullName,
                                assembly.FullName
                            )
                        );
        ...
        /// </returns>
        public static IList<Assembly> LoadMatching(
            string pattern,
            System.IO.SearchOption searchOption,
            AssemblyLoadMode loadMode = AssemblyLoadMode.OnlySigned,
            string subfolder = null
        )
        {
            string loadPath = AssemblyLoader.GetAssemblyLoadPath();
            ...
            if (System.IO.Directory.Exists(loadPath))
            {
                foreach (string ext in new string[] { ".dll", ".exe" })
                {
                    ...
                    foreach (
                        string path in System.IO.Directory.GetFiles(
                            loadPath,
                            filePattern,
                            searchOption
                        )
        ...
        public static IList<Assembly> FindMatching(
            AssemblyLoadMode loadMode = AssemblyLoadMode.OnlySigned
        )
        {
            lock (AssemblyLoader.exclusion)
            {
                return AssemblyLoader
                    .assemblies.Values.Where(x => AssemblyLoader.Match(x, loadMode))
                    .ToArray();
            }
        }
        ...
        private static string GetAssemblyLoadPath()
        {
            ...
            if (loadName.StartsWith(filePrefix))
            {
                ...
                if (loadPath != loadName)
                {
                    if (AssemblyLoader.loadPathDifferences++ == 0)
                    {
                        System.Diagnostics.Debug.WriteLine(
                            string.Format(
                                "Assembly load path different from executable path\n"
                                    +
                                    /* */"  DLL load path: '{0}'\n"
                                    +
                                    /* */"  EXE load path: '{1}'",
            ...
            else
            {
                throw new System.Exception(
                    string.Format(
                        "Support assembly not loaded from a file: CodeBase = '{0}'",
                        loadName
                    )
                );
            }
            ...
            switch (loadMode)
            {
                ...
                default:
                    throw new System.NotSupportedException(
                        string.Format("{0} not supported", loadMode.GetQualifiedName())
                    );
            }
        }

        private static void HandleCurrentDomainAssemblyLoad(
            object sender,
            ...
            lock (AssemblyLoader.exclusion)
            {
                if (AssemblyLoader.assemblies.ContainsKey(name))
                {
                    throw new System.InvalidOperationException(
                        "Duplicate assembly loaded: " + name
                    );
                }
        ...
        private static readonly SafeCounter recursionCount = new SafeCounter();
        private static readonly Queue<System.Action> pendingEvents = new Queue<System.Action>();

        private static readonly Assembly CurrentAssembly = typeof(AssemblyLoader).Assembly;
        private static readonly byte[] EpsitecPublicKey = AssemblyLoader
