    ...
    public class FileProvider : AbstractResourceProvider
    {
            ...
            : base(manager)
        {
            foreach (var path in FileProvider.GetProbingPaths(manager).Distinct())
            {
                if (this.SelectPath(path))
                {
                    ...
                    this.SelectLocale(CultureInfo.CurrentUICulture);
                    return;
                }
            }
        ...
        public override bool SelectModule(ref ResourceModuleId module)
        {
            ...
            if (moduleName != null)
            {
                ...
                if ((!string.IsNullOrEmpty(modulePath)) && (System.IO.Directory.Exists(modulePath)))
                {
                    ResourceModuleId info = FileProvider.GetModuleId(modulePath);

                    moduleId = info.Id;

                    if ((moduleId >= 0) && ((module.Id < 0) || (module.Id == moduleId)))
                    {
                        module = info;

                        this.pathPrefix = string.Concat(
                            modulePath,
            ...
            foreach (string file in this.GetModuleProbingDirectories())
            {
                ...
                if (this.ValidateId(moduleName))
                {
                    ...
                    if (info.Id >= 0)
                    {
                        if (
                            modules.FindIndex(
                                delegate(ResourceModuleId item)
                                {
        ...
        )
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                {
                    System.Text.RegularExpressions.Regex typeRegex =
                        Support.RegexFactory.FromSimpleJoker(typeFilter);
                ...
                switch (mode)
                {
                    ...
                    default:
                        throw new System.ArgumentException(
                            string.Format("Mode {0} not supported.", mode),
                            "mode"
                        );
                }
                ...
                switch (level)
                {
                    ...
                    default:
                        throw new ResourceException(
                            string.Format(
                                "Invalid resource level {0} for resource '{1}'.",
                                level,
                                id
                            )
                        );
        ...
        private bool SelectPath(string path)
        {
            ...
            path = System.IO.Path.Combine(
                IO.PathTools.RemoveUntilDir("App.CreativeDocs", path)
                    ?? IO.PathTools.RemoveUntilDir("Common.Tests", path),
                "Resources"
            );
