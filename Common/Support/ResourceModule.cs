    ...
    public static class ResourceModule
    {
        ...
        public static ResourceModuleInfo LoadManifest(string modulePath)
        {
            if (System.IO.Directory.Exists(modulePath))
            {
                ...
                string moduleInfoPath = System.IO.Path.Combine(
                    modulePath,
                    ResourceModule.ManifestFileName
                );
                ...
                if (textReader != null)
                {
                    using (textReader)
                    {
                        ...
                        if (root.Name == ResourceModule.XmlModuleInfo)
                        {
                            ...
                            string layerAttribute = root.GetAttribute(
                                ResourceModule.XmlAttributeLayer
                            );
                            string nameAttribute = root.GetAttribute(
                                ResourceModule.XmlAttributeName
                            );
                            string namespaceAttributeDefault = root.GetAttribute(
                                ResourceModule.XmlAttributeNamespaceDefault
                            );
                            string namespaceAttributeEntities = root.GetAttribute(
                                ResourceModule.XmlAttributeNamespaceEntities
                            );
                            string namespaceAttributeForms = root.GetAttribute(
                                ResourceModule.XmlAttributeNamespaceForms
                            );
                            string namespaceAttributeRes = root.GetAttribute(
                                ResourceModule.XmlAttributeNamespaceRes
                            );
                            string textModeAttribute = root.GetAttribute(
                                ResourceModule.XmlAttributeTextMode
                            );
                            string assembliesAttribute = root.GetAttribute(
                                ResourceModule.XmlAttributeAssemblies
                            );
                            ...
                            if (string.IsNullOrEmpty(idAttribute))
                            {
                                throw new System.FormatException(
                                    string.Format(
                                        "{0} specifies no {1} attribute in module {2}",
                                        ResourceModule.XmlModuleInfo,
                                        ResourceModule.XmlAttributeId,
                                        modulePath
                                    )
                                );
                            }
                            ...
                            textMode = InvariantConverter.ToEnum<ResourceTextMode>(
                                textModeAttribute,
                                ResourceTextMode.String
                            );

                            ResourceModuleInfo info = new ResourceModuleInfo();

                            System.Xml.XmlNodeList nodes = root.GetElementsByTagName(
                                ResourceModule.XmlReferenceModulePath
                            );
                            System.Xml.XmlElement node =
                                nodes.Count == 1 ? (nodes[0] as System.Xml.XmlElement) : null;
                            ...
                            else if (nodes.Count > 1)
                            {
                                throw new System.FormatException(
                                    string.Format(
                                        "{0} specifies more than 1 {1} element in module {2}",
                                        ResourceModule.XmlModuleInfo,
                                        ResourceModule.XmlReferenceModulePath,
                                        modulePath
                                    )
                            ...
                            if (node != null)
                            {
                                ...
                                {
                                    int developerId = InvariantConverter.ToInt(
                                        versionNode.GetAttribute(ResourceModule.XmlAttributeId)
                                    );
                                    int buildNumber = InvariantConverter.ToInt(
                                        versionNode.GetAttribute(ResourceModule.XmlAttributeBuild)
                                    );
                                    System.DateTime buildDate = InvariantConverter.ToDateTime(
                                        versionNode.GetAttribute(ResourceModule.XmlAttributeDate)
                                    );
                            ...
                            info.SourceNamespaceDefault = string.IsNullOrEmpty(
                                namespaceAttributeDefault
                            )
                                ? null
                                : namespaceAttributeDefault;
                            info.SourceNamespaceEntities = string.IsNullOrEmpty(
                                namespaceAttributeEntities
                            )
                                ? null
                                : namespaceAttributeEntities;
                            info.SourceNamespaceForms = string.IsNullOrEmpty(
                                namespaceAttributeForms
                            )
                                ? null
                                : namespaceAttributeForms;
                            info.SourceNamespaceRes = string.IsNullOrEmpty(namespaceAttributeRes)
                                ? null
                                : namespaceAttributeRes;
                            info.TextMode = textMode;
                            info.Assemblies = string.IsNullOrEmpty(assembliesAttribute)
                                ? null
                                : assembliesAttribute;
                            info.Freeze();
        ...
        private static void SetAttribute(System.Xml.XmlElement root, string attribute, string value)
        {
        ...
        public static System.Xml.XmlDocument CreateXmlManifest(
        ...
        )
        {
            ...
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeId,
                InvariantConverter.ToString(info.FullId.Id)
            );
            ResourceModule.SetAttribute(root, ResourceModule.XmlAttributeName, info.FullId.Name);
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeLayer,
                ResourceModuleId.ConvertLayerToPrefix(info.FullId.Layer)
            );
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeTextMode,
                System.Enum.GetName(typeof(ResourceTextMode), info.TextMode)
            );
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeAssemblies,
            ...
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeNamespaceDefault,
                info.SourceNamespaceDefault
            );
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeNamespaceEntities,
                info.SourceNamespaceEntities == info.SourceNamespaceDefault
                    ? null
                    : info.SourceNamespaceEntities
            );
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeNamespaceForms,
                info.SourceNamespaceForms == info.SourceNamespaceDefault
                    ? null
                    : info.SourceNamespaceForms
            );
            ResourceModule.SetAttribute(
                root,
                ResourceModule.XmlAttributeNamespaceRes,
                info.SourceNamespaceRes == info.SourceNamespaceDefault
                    ? null
                    : info.SourceNamespaceRes
            );
            ...
            if (info.HasVersions)
            {
                ...
                {
                    System.Xml.XmlElement versionNode = xml.CreateElement(
                        ResourceModule.XmlVersion
                    );

                    versionNode.SetAttribute(
                        ResourceModule.XmlAttributeId,
                        InvariantConverter.ToString(version.DeveloperId)
                    );
                    versionNode.SetAttribute(
                        ResourceModule.XmlAttributeBuild,
                        InvariantConverter.ToString(version.BuildNumber)
                    );
                    versionNode.SetAttribute(
                        ResourceModule.XmlAttributeDate,
                        InvariantConverter.ToString(version.BuildDate)
                    );
            ...
            {
                System.Xml.XmlElement node = xml.CreateElement(
                    ResourceModule.XmlReferenceModulePath
                );
        ...
        {
            if ((info == null) || (info.HasVersions == false))
            {
                return "";
            }
        ...
        internal const string XmlAttributeNamespaceDefault = "namespace";
        internal const string XmlAttributeNamespaceEntities = "namespaceEntities";
        internal const string XmlAttributeNamespaceForms = "namespaceForms";
        internal const string XmlAttributeNamespaceRes = "namespaceRes";
