    ...
    internal sealed class FileInfo
    {
        private FileInfo()
        {
            ...
            System.Diagnostics.Debug.Assert(
                PidlHandle.VirtualDesktopHandle.Pidl != System.IntPtr.Zero
            );
        }
        ...
        private static void InitializeWellKnownFolderItemsLocked()
        {
            if (FileInfo.wellKnownFolders.Count == 0)
            {
                ...
                FileInfo.InitializeWellKnownFolderItems(
                    FolderQueryMode.LargeIcons.Open(),
                    "Large-Open"
                );
                FileInfo.InitializeWellKnownFolderItems(
                    FolderQueryMode.SmallIcons.Open(),
                    "Small-Open"
                );
        ...
        private static bool FindWellKnownFolderItem(
            FolderItemHandle handle,
            string fullPath,
        ...
        private static FolderQueryMode InitializeWellKnownFolderItems(
            FolderQueryMode mode,
            string prefix
        )
        {
        ...
        public static FolderItem CreateFolderItem(FolderId file, FolderQueryMode mode)
        {
            ...
            uint result = ShellApi.SHGetFolderLocation(
                System.IntPtr.Zero,
                (short)csidl,
                System.IntPtr.Zero,
                0,
            ...
            if ((result != 0) && (result != 1))
            {
                ...
                {
                    return FileInfo.CreateFolderItem(
        ...
        public static FolderItem CreateFolderItem(string path, FolderQueryMode mode)
        {
            ...
            FileInfo.instance.root.ParseDisplayName(
                System.IntPtr.Zero,
                System.IntPtr.Zero,
                path,
                ref eaten,
        ...
        public static FolderItem GetParentFolderItem(FolderItem path, FolderQueryMode mode)
        {
            ...
            if ((handle != PidlHandle.VirtualDesktopHandle) && (handle != null))
            {
        ...
        private static IEnumerable<FolderItem> GetRecentFolderItemsHack(
            FolderQueryMode mode,
            System.Predicate<FileFilterInfo> filter
        )
        {
        ...
        private static IEnumerable<FolderItem> GetFolderItems(
            PidlHandle handle,
            FolderQueryMode mode,
            bool disposeHandleWhenFinished,
            System.Predicate<FileFilterInfo> filter
        )
        {
            if (handle == null)
            ...
            try
            {
                FileInfo.instance.root.BindToObject(
                    pidlPath,
                    System.IntPtr.Zero,
                    ref ShellGuids.IID_IShellFolder,
                    out ptrPath
                );
            }
            catch
            ...
            ShellApi.SHCONT flags =
                ShellApi.SHCONT.SHCONTF_FOLDERS
                | ShellApi.SHCONT.SHCONTF_NONFOLDERS
                | ShellApi.SHCONT.SHCONTF_INCLUDEHIDDEN;
            ...
            if (ptrList != System.IntPtr.Zero)
            {
                ...
                while (list.Next(1, out pidlElement, out count) == 0)
                {
                    ...
                    if (pidlTemp != System.IntPtr.Zero)
                    {
                        ...
                        if (filter == null)
                        {
                            yield return FileInfo.CreateFolderItemAndInheritPidl(
                                mode,
                                pidlTemp,
                                fullPath,
                                fileAttributes
        ...
        private static FolderItem CreateFolderItemAndInheritPidl(
            FolderQueryMode mode,
            System.IntPtr pidl
        )
        {
            return FileInfo.CreateFolderItemAndInheritPidl(
                mode,
                pidl,
                FileInfo.GetFullPathFromPidl(pidl)
            );
        }
        ...
        )
        {
            ...
            {
                if (
                    FileInfo.FindWellKnownFolderItem(new PidlHandle(pidl), fullPath, mode, out item)
                )
                {
                    return item;
                }
            ...
            FileInfo.GetIconAndDescription(
                pidl,
                fullPath,
                mode,
                fileAttr,
        ...
        private static bool GetIconAndDescription(
            System.IntPtr pidl,
            string fullPath,
            FolderQueryMode mode,
            uint attr,
            out System.Drawing.Icon icon,
            out string displayName,
            out string typeName,
            out FolderItemAttributes attributes
        )
        {
            if (pidl == System.IntPtr.Zero)
            {
            ...
            if ((!string.IsNullOrEmpty(fullPath)) && (fullPath.Length == 3) && (fullPath[1] == ':'))
            {
            ...
            if ((info.dwAttributes & (uint)ShellApi.SFGAO.SFGAO_LINK) != 0)
            {
                ...
                else
                {
                    System.Diagnostics.Debug.WriteLine(
                        "Unsupported SGFAO_LINK on file: " + fullPath
                    );
                }
            }
        ...
        private static void GetShellFileInfo(
            System.IntPtr pidl,
            string fullPath,
            FolderQueryMode mode,
            bool requestAttributes,
            uint fileAttr,
            ref ShellApi.SHFILEINFO info
        )
        {
        ...
        private static ShellApi.CSIDL GetCsidl(FolderId id)
        {
            ...
            throw new System.ArgumentException(
                string.Format("{0} cannot be mapped to a CSIDL", id)
            );
        }
        ...
        private static FolderItem recentFolderItem = FileInfo.CreateFolderItem(
            FolderId.Recent,
            FolderQueryMode.NoIcons
        );
        private static Dictionary<FolderKey, FolderItem> wellKnownFolders =
            new Dictionary<FolderKey, FolderItem>();
