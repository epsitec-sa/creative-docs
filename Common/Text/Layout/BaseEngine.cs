...
namespace Epsitec.Common.Text.Layout
{
    ...
    public abstract class BaseEngine : System.IDisposable
    {
        ...
        public virtual Layout.Status Render(
        ...
        public virtual Layout.Status FillProfile(
            Layout.Context context,
            int length,
        ...
        public int GetNextFragmentLength(
            ulong[] text,
            int start,
            int length,
            int fragmentLength,
            out double breakPenalty
        )
        {
            ...
            for (int i = fragmentLength; i < length; i++)
            {
                ...
                Debug.Assert.IsTrue(
                    (info == Unicode.BreakInfo.No)
                        || (info == Unicode.BreakInfo.NoAlpha)
                        || (i + 1 == length)
                );
            }
        ...
        public static bool GenerateGlyphs(
            Layout.Context context,
            OpenType.Font font,
            ulong[] text,
            int offset,
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                else if (analyzer.IsControl(code))
                {
                    if (context.ShowControlCharacters)
                    {
                        ...
                        temp[i] |=
                            BaseEngine.MapToVisibleControlCharacter(code) & Unicode.Bits.CodeMask;
                    }
                    else
                    {
                        temp[i] &= ~Unicode.Bits.FullCodeMask;
                    ...
                    (code == (int)Unicode.Code.SoftHyphen)
                    &&
                    /**/(i + 1 < length)
                )
                {
                    ...
                    (code == (int)Unicode.Code.MongolianTodoHyphen)
                    &&
                    /**/(i != 0)
                )
                {
        ...
        public static bool GenerateGlyphsAndStretchClassAttributes(
            Text.TextContext context,
            OpenType.Font font,
            ulong[] text,
            int offset,
            int length,
        ...
        public static double GetSpaceWidth(OpenType.Font font, int code)
        {
            ...
            return Unicode.BreakAnalyzer.GetSpaceWidth(
                code,
                spaceWidth,
                enWidth,
                emWidth,
