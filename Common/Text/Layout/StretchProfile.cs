...
namespace Epsitec.Common.Text.Layout
{
    ...
    public class StretchProfile
    {
        ...
        {
            get { return this.countNoStretch; }
        }

        public short CountCharacter
        {
            get { return this.countCharacter; }
        }

        public short CountSpace
        {
            get { return this.countSpace; }
        }

        public short CountEndSpace
        {
            get { return (short)(this.countEndSpace + this.countEndCSpace); }
        }

        public short CountKashida
        {
            get { return this.countKashida; }
        }

        public double WidthNoStretch
        {
            get { return this.widthNoStretch; }
        }

        public double WidthCharacter
        {
            get { return this.widthCharacter; }
        }

        public double WidthSpace
        {
            get { return this.widthSpace; }
        }

        public double WidthEndSpace
        {
            get { return this.widthEndSpace + this.widthEndCSpace; }
        }

        public double WidthKashida
        {
            get { return this.widthKashida; }
        }
        ...
        {
            if (
                (stretch != Unicode.StretchClass.Space)
        ...
        )
        {
            ...
            {
                attributes[i] = (byte)
                    Unicode.BreakAnalyzer.GetStretchClass(Unicode.Bits.GetCode(text[i]));
            }
            ...
            )
            {
                throw new System.InvalidOperationException(
                    string.Format(
                        "Special glyph required, not in font '{0}'",
                        font.FontIdentity.FullName
                    )
                );
            }
            ...
            {
                this.Add(
                    (Unicode.StretchClass)attributes[count - 1],
                    xEnd - xPos[count - 1] + glue
                );
            }
        }
        ...
        )
        {
            ...
            for (int i = 0; i < count - 1; i++)
            {
                ...
                {
                    this.Add(
                        (Unicode.StretchClass)stretchClassAttributes[i],
                        xPos[i + 1] - xPos[i] + glue
                    );
                }
            }
            ...
            {
                this.Add(
                    (Unicode.StretchClass)stretchClassAttributes[count - 1],
                    xEnd - xPos[count - 1] + glue
                );
            }
        }
        ...
        public double ComputePenalty(double width, double fenceBefore, double fenceAfter)
        {
            ...
            if ((totalWidth == 0) || (totalWidth > width + fenceAfter))
            {
        ...
        public double ComputeScales(double width, out Scales scales)
        {
            ...
            scales.ScaleNoStretch =
                1 + (this.widthNoStretch == 0 ? 0 : force * εNoStretch / this.widthNoStretch);
            scales.ScaleCharacter =
                1 + (this.widthCharacter == 0 ? 0 : force * εCharacter / this.widthCharacter);
            scales.ScaleSpace = 1 + (this.widthSpace == 0 ? 0 : force * εSpace / this.widthSpace);
            scales.ScaleKashida =
                1 + (this.widthKashida == 0 ? 0 : force * εKashida / this.widthKashida);
