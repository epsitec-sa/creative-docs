...
namespace Epsitec.Common.Text.Internal
{
    ...
    internal struct CursorIdArray
    {
        ...
        public void ProcessRemoval(
            ...
            int length,
            int absOrigin,
            bool removalContinuation,
            out CursorInfo[] removed
        )
        {
            ...
            while ((indexAt < indexAfter) && (indexAt < this.length))
            {
                ...
                if (
                    ((length >= 1) && (attachment == CursorAttachment.ToNext))
            ...
            {
                Debug.Assert.IsTrue(
                    this.GetCursorPosition(this.elements[indexAfter].id) >= position + length
                );
        ...
        private int FindElement(Internal.CursorId id)
        {
            ...
            {
                if ((this.cachedIndex < this.length) && (this.elements[this.cachedIndex].id == id))
                {
