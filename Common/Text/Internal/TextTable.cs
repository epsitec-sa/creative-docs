...
namespace Epsitec.Common.Text.Internal
{
    ...
    internal sealed class TextTable
    {
        ...
        public int MoveCursor(Internal.CursorId id, int distance)
        {
            ...
            else if (distance < 0)
            {
                ...
                while (distance > 0)
                {
                    ...
                    this.textChunks[index + 0].RemoveCursor(id);
                    this.textChunks[index - 1]
                        .AddCursor(id, this.textChunks[index - 1].TextLength, record.Attachment);
        ...
        public CursorInfo[] FindCursorsBefore(int position, CursorInfo.Filter filter)
        {
            ...
            for (; ; )
            {
                ...
                if (cursorIndex > -1)
                {
                    ...
                    result[0] = new CursorInfo(
                        cursorId,
                        cursorPos,
                        this.GetCursorDirection(cursorId)
                    );
                    ...
                    for (int i = 1; i < count; i++)
                    {
                        ...
                        result[i] = new CursorInfo(
                            cursorIdI,
                            cursorPosI,
                            this.GetCursorDirection(cursorIdI)
                        );
                    }
        ...
        public CursorInfo[] FindCursors(
            int position,
            int length,
            CursorInfo.Filter filter,
            bool findOnlyFirst
        )
        {
                                ...
                                if (findOnlyFirst)
                                {
                                    return new CursorInfo[]
                                    {
                                        new CursorInfo(
                                            cursorId,
                                            iPos,
                                            cursorInstance == null ? 0 : cursorInstance.Direction
                                        )
                                    };
                                }
        ...
        public CursorInfo[] FindNextCursor(Internal.CursorId id, CursorInfo.Filter filter)
        {
            ...
            {
                if ((infos[i].CursorId == id) && (i + 1 < infos.Length))
                {
        ...
        public void DeleteText(Internal.CursorId cursorId, int length, out CursorInfo[] infos)
        {
            ...
            while ((length > 0) && (index < this.textChunks.Length))
            {
                ...
                if ((infos != null) && (infos.Length > 0))
                {
                    ...
                    for (int i = 0; i < infos.Length; i++)
                    {
                        ...
                        list.Add(
                            new CursorInfo(infos[i].CursorId, infos[i].Position, iCursor.Direction)
                        );
                    }
                }
        ...
        public int TraverseText(
            Internal.CursorId cursorId,
            int distance,
            TextStory.CodeCallback callback
        )
        {
        ...
        )
        {
            ...
            while (count < length)
            {
                ...
                else
                {
                    if (count == 0)
                    {
                        code = Internal.CharMarker.ExtractCoreAndSettings(
                            this.textChunks[index][pos]
                        );
                        next = code;
                    }
                    else
                    {
                        next = Internal.CharMarker.ExtractCoreAndSettings(
                            this.textChunks[index][pos]
                        );
            ...
            while (changed < length)
            {
                ...
                else
                {
                    int count = System.Math.Min(
                        length - changed,
                        this.textChunks[index].TextLength
                    );
        ...
        internal void ReadRawText(System.IO.Stream stream)
        {
            ...
            )
            {
                ...
                {
                    int n =
                        (length + TextTable.TextChunkIdealSize - 1) / TextTable.TextChunkIdealSize;
