...
namespace Epsitec.Common.Text.Internal
{
    ...
    internal sealed class SettingsTable : Common.Support.IXMLSerializable<SettingsTable>
    {
        ...
        )
        {
            ...
            if ((localSettings == null) && (extraSettings == null))
            {
            ...
            else
            {
                ...
                Styles.CoreSettings find = this.FindCore(
                    coreSettings,
                    new CoreMatcher(matcher.FindExactSettings)
                );

                if ((find == null) && (matcher.WasCalled))
                {
                ...
                Internal.CharMarker.SetCoreIndex(ref code, find.CoreIndex);
                Internal.CharMarker.SetLocalIndex(
                    ref code,
                    localSettings == null ? 0 : localSettings.SettingsIndex
                );
                Internal.CharMarker.SetExtraIndex(
                    ref code,
                    extraSettings == null ? 0 : extraSettings.SettingsIndex
                );
            }
        }
        ...
        public void GetCoreAndSettings(
            ulong code,
            out Styles.CoreSettings coreSettings,
            out Styles.LocalSettings localSettings,
            out Styles.ExtraSettings extraSettings
        )
        {
            coreSettings = this.GetCoreFromIndex(Internal.CharMarker.GetCoreIndex(code));

            localSettings =
                (Internal.CharMarker.HasSettings(code) && (coreSettings != null))
                    ? coreSettings.GetLocalSettings(code)
                    : null;
            extraSettings =
                (Internal.CharMarker.HasSettings(code) && (coreSettings != null))
                    ? coreSettings.GetExtraSettings(code)
                    : null;
        }
        ...
        public Styles.CoreSettings FindCore(Styles.CoreSettings coreSettings, CoreMatcher matcher)
        {
            ...
            if ((this.cores == null) || (this.cores.Count == 0))
            {
                return null;
            }
