...
namespace Epsitec.Common.Text
{
    ...
    public sealed class TextFitter
    {
        ...
        {
            get { return this.cursors.Count - this.freeCursors.Count; }
        }
        ...
        public bool CheckNeedsMoreRoomForFullFit()
        {
            ...
            if (cursor != null)
            {
                ...
                if (cursor != null)
                {
                    ...
                    {
                        if (!char.IsWhiteSpace(c) && (c != (char)Unicode.Code.EndOfText))
                        {
                            return true;
                        }
                    }
        ...
        public void GenerateMarks()
        {
            ...
            if ((start == 0) && (end == this.story.TextLength))
            {
                this.ClearAllMarks();
        ...
        public void RenderParagraphInTextFrame(
            ICursor fitterCursor,
        ...
        )
        {
            ...
            Layout.Context layout = new Layout.Context(
                this.story.TextContext,
                text,
                0,
                this.frameList
            );
        ...
        public void RenderTextFrame(ITextFrame frame, ITextRenderer renderer)
        {
            ...
            while (cursor != null)
            {
                ...
                CursorInfo[] cursors = text.FindNextCursor(
                    cursor.CursorId,
        ...
        )
        {
            ...
            for (; ; )
            {
                ...
                while (cursor != null)
                {
                    ...
                    if (
                        this.HitTestParagraphInTextFrame(
                            cursor,
                            frame,
                            x,
                            y,
                    ...
                    {
                        cursor =
                            text.GetCursorInstance(cursors[0].CursorId) as Cursors.FitterCursor;
                    }
                    else
                    {
                        break;
        ...
        public bool HitTestParagraphInTextFrame(
            ICursor fitterCursor,
            ITextFrame frame,
            double x,
            double y,
            bool skipInvisible,
            bool acceptOutOfLine,
            ref int position,
            ref int direction
        )
        {
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                ...
                double lY1 = System.Math.Min(
                    elements[i].LineY1 - dyAfter,
                    lYb + elements[i].LineDescender
                );
                ...
                if ((y >= lY1) && (y <= lY2))
                {
                    ...
                    if ((viewY >= cy1) && (viewY <= cy2))
                    {
                        ...
                        if (viewX < cx1)
                        {
                            ...
                            if ((acceptOutOfLine) && (elements[i].IsTabulation == false))
                            {
                                return true;
                            }
        ...
        public bool GetCursorGeometry(
            ...
            out double x,
            out double y,
            out int paragraphLine,
            out int lineCharacter
        )
        {
                    ...
                    for (int i = 0; i < elements.Length; i++)
                    {
                        ...
                        if (
                            ...
                            && (
                                ...
                                || (
                                    ...
                                    && (
                                        !Internal.Navigator.IsAfterLineBreak(
                                            this.story,
                                            fitterCursor,
                        ...
                        )
                        {
                            ...
                            this.GetCursorGeometry(
                                ...
                                ref y1,
                                ref y2,
                                fitterCursor.SpaceAfterParagraph
                            );
        ...
        public void GetStatistics(out int paragraphCount, out int lineCount)
        {
            ...
            CursorInfo[] infos = this.story.TextTable.FindCursors(
                0,
                this.story.TextLength,
                Cursors.FitterCursor.Filter
            );
            ...
            for (int i = 0; i < infos.Length; i++)
            {
                Cursors.FitterCursor cursor =
                    this.story.TextTable.GetCursorInstance(infos[i].CursorId)
        ...
        private void ExecuteClear(
            Cursors.TempCursor tempCursor,
            int pos,
            ref int length,
            out TextProcessor.Status status
        )
        {
            //	Supprime les marques de découpe de lignes représentées par des
            //	curseurs (instances de Cursors.FitterCursor).

            CursorInfo[] cursors = this.story.TextTable.FindCursors(
                pos,
                length,
                Cursors.FitterCursor.Filter
            );
            ...
            for (; ; )
            {
                ...
                switch (layoutStatus)
                {
                    ...
                    case Layout.Status.OkTabReached:
                        ...
                        layout.TextContext.GetMargins(
                            text[layout.TextOffset - 1],
                            out marginsProperty
                        );
                        layout.TextContext.GetFontSize(
                            text[layout.TextOffset - 1],
                            out fontSizeInPoints
                        );
                        ...
                        double tabDx;
                        bool tabAtLineStart =
                            (!continuation) && (layout.TextOffset == lineStartOffset + 1);
                        bool tabIndents;

                        tabStatus = this.MeasureTabTextWidth(
                            layout,
                            tabProperty,
                            marginsProperty,
                            fontSizeInPoints,
                        ...
                        if (tabStatus == TabStatus.ErrorNeedMoreText)
                        {
                            ...
                            (tabStatus == TabStatus.ErrorNeedMoreRoom)
                            ||
                        ...
                        else
                        {
                            throw new System.NotImplementedException(
                                string.Format("TabStatus.{0}", tabStatus)
                            );
                        }
                    ...
                    default:
                        throw new System.InvalidOperationException(
                            string.Format("Invalid layout status received: {0}.", layoutStatus)
                        );
                }
                ...
                if (nBreaks > 1)
                {
                    ...
                    {
                        double computedPenalty = this.ComputePenalty(
                            result[i].SpacePenalty,
                            result[i].BreakPenalty
                        );
                ...
                if (nBreaks > 0)
                {
                    ...
                    element.LineWidth =
                        (continuation && !tabNewLine)
                            ? result[result.Count - 1].Profile.TotalWidth
                ...
                else
                {
                    ...
                    if (
                        (layoutStatus == Layout.Status.OkFitEnded)
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            this.story.TextContext.GetBreak(
                                layout.LastProcessedCharacter,
                                out breakProperty
                            );

                            if (breakProperty == null)
                            {
                                layout.DefineParagraphStartMode(
                                    Properties.ParagraphStartMode.Anywhere
                                );
                            }
                            else
                            {
        ...
        private int RewindToPreviousParagraph(Cursors.TempCursor cursor, int position, int offset)
        {
            ...
            bool keep1WithPrev =
                (keep1 == null)
                    ? false
                    : (keep1.KeepWithPreviousParagraph == Properties.ThreeState.True);
            bool keep2WithNext =
                (keep2 == null)
                    ? false
                    : (keep2.KeepWithNextParagraph == Properties.ThreeState.True);
            ...
            this.lineSkipBefore =
                ((lead2 == null) || (lead2.SpaceAfter.IsSafeNaN())) ? 0 : lead2.SpaceAfterInPoints;
        ...
        private void GetCursorGeometry(
            int position,
            int cursorOffset,
            Cursors.FitterCursor cursor,
            Cursors.FitterCursor.Element[] elements,
            int i,
            ref double x,
            ref double y,
            ref double y1,
        ...
        private Internal.GeometryRenderer GenerateGeometryInformation(
            ITextFrame frame,
            int position,
            Cursors.FitterCursor cursor,
            Cursors.FitterCursor.Element[] elements,
        ...
        )
        {
            ...
            if (
                (this.geometryCacheVersion.HasAnythingChanged)
                || (this.geometryCacheStart != position)
            )
            {
                Internal.GeometryRenderer renderer = this.GenerateGeometryInformation(
                    frame,
                    position,
                    cursor,
                    elements,
                ...
                if ((n > 1) && (elements[i].IsTabulation) && (elements[i].IsNewLine == false))
                {
                    ...
                    System.Diagnostics.Debug.Assert(
                        renderer[n - 2].Unicode == Unicode.Code.HorizontalTab
                    );
                    ...
                    Internal.GeometryRenderer next = this.GenerateGeometryInformation(
                        frame,
                        nextPos,
                        cursor,
                        elements,
        ...
        )
        {
            ...
            if ((frame == null) || (layout.Frame == frame))
            {
                ...
                )
                {
                    ...
                    if (isTab)
                    {
                        double tabOrigin = elements[i - 1].IsNewLine
                            ? elements[i].LineOriginX
                            : elements[i - 1].LineStartX + elements[i - 1].LineWidth;
                        double tabStop = elements[i].LineStartX;
                        ulong tabCode = this.story.ReadChar(
                            cursor,
                            cursor.GetElementStartPosition(i) - 1
                        );
                        ...
                        bool isTabDefined =
                            tabsProperty == null ? false : tabsProperty.ContainsTabTag(tag);
        ...
        {
            CursorInfo[] info = this.story.TextTable.FindCursorsBefore(
                position,
                Cursors.FitterCursor.Filter
            );
        ...
        private Cursors.FitterCursor GetPreviousFitterCursor(ICursor cursor)
        {
            return (cursor == null)
                ? null
                : this.GetPreviousFitterCursor(this.story.GetCursorPosition(cursor));
        }
        ...
        private TabStatus MeasureTabTextWidth(
            Layout.Context layout,
            Properties.TabProperty tabProperty,
            Properties.MarginsProperty marginsProperty,
            double fontSizeInPoints,
        ...
        )
        {
            ...
            if ((dockingMark != null) && (dockingMark.Length > 0))
            {
                d = 1.0;
            }
            ...
            if ((tabAttr != null) && (tabAttr.Length > 0))
            {
            ...
            switch (tabs.GetTabPositionMode(tabProperty))
            {
                ...
                default:
                    throw new System.NotSupportedException(
                        string.Format(
                            "Tab position mode {0} not supported",
                            tabs.GetTabPositionMode(tabProperty)
                        )
                    );
            }
            ...
            if (d < 0.5)
            {
                ...
                if ((xBefore < roomBefore) && (xBefore > 0))
                {
                    roomBefore = xBefore;
            ...
            if (
                (fitStatus == Layout.Status.OkFitEnded) || (fitStatus == Layout.Status.OkTabReached)
            )
            {
