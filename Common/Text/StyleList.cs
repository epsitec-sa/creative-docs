...
namespace Epsitec.Common.Text
{
    ...
    public sealed class StyleList
    {
        ...
        {
            get { return this.GetTextStyle(name, textStyleClass); }
        }

        #region Internal Properties
        internal Internal.SettingsTable InternalSettingsTable
        {
            get { return this.internalSettings; }
        }
        ...
        internal int InternalStyleCount
        {
            get
            {
                System.Diagnostics.Debug.Assert(
                    this.textStyleList.Count == this.textStyleHash.Count
                );
        ...
        {
            get { return this.internalSettings.GetCore(code); }
        }
        #endregion
        ...
        public TextStyle NewTextStyle(
            ...
            TextStyleClass textStyleClass,
            System.Collections.ICollection properties,
            System.Collections.ICollection parentStyles
        )
        {
            if (name == null)
            {
        ...
        public TextStyle NewMetaProperty(
            string name,
            string metaId,
            int priority,
            params Property[] properties
        ...
        public TextStyle CreateOrGetMetaProperty(
            string metaId,
            int priority,
            params Property[] properties
        )
        ...
        public void DeleteTextStyle(Common.Support.OpletQueue queue, TextStyle style)
        {
            ...
            this.RedefineTextStyle(
                queue,
                style,
                new Property[0],
                new TextStyle[1] { defaultStyle }
            );
        }
        ...
        internal TextStyle GetDefaultTextStyle(TextStyle style)
        {
            ...
            switch (style.TextStyleClass)
            {
                case TextStyleClass.Text:
                    defaultStyle = this.TextContext.DefaultTextStyle;
                    break;
                case TextStyleClass.Paragraph:
                    defaultStyle = this.TextContext.DefaultParagraphStyle;
                    break;

                default:
                    throw new System.NotSupportedException(
                        string.Format(
                            "Cannot delete style {0} of class {1}",
                            style.Name,
        ...
        public static bool IsAutomaticName(string value)
        {
            ...
            if ((value != null) && (value.Length > 4) && (value.StartsWith("#ID#")))
            {
                return true;
            }
            else
        ...
        internal void UpdateTabListUserCount(TextStyle style, int sign)
        {
            Properties.TabsProperty tabs =
                style[Properties.WellKnownType.Tabs] as Properties.TabsProperty;
            Properties.ManagedParagraphProperty mpp =
                style[Properties.WellKnownType.ManagedParagraph]
        ...
        internal static string GetFullName(string name, TextStyleClass textStyleClass)
        {
            ...
            {
                case TextStyleClass.Abstract:
                    return string.Concat("A.", name);
                case TextStyleClass.Paragraph:
                    return string.Concat("P.", name);
                case TextStyleClass.Text:
                    return string.Concat("T.", name);
                case TextStyleClass.Symbol:
                    return string.Concat("S.", name);
                case TextStyleClass.MetaProperty:
                    return string.Concat("M.", name);
        ...
        internal static void SplitFullName(
            string fullName,
            out string name,
            out TextStyleClass textStyleClass
        )
        {
            ...
            {
                case 'A':
                    textStyleClass = TextStyleClass.Abstract;
                    break;
                case 'P':
                    textStyleClass = TextStyleClass.Paragraph;
                    break;
                case 'T':
                    textStyleClass = TextStyleClass.Text;
                    break;
                case 'S':
                    textStyleClass = TextStyleClass.Symbol;
                    break;
                case 'M':
                    textStyleClass = TextStyleClass.MetaProperty;
                    break;
            ...
            foreach (TextStory story in this.context.GetTextStories())
            {
                ...
                if (length > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ManagedParagraphPropertyInfo mppi =
                                this.pendingManagerPatches.Pop() as ManagedParagraphPropertyInfo;

                            story.SetCursorPosition(cursor, mppi.Position);

                            Properties.ManagedParagraphProperty[] mppOld =
                                (mppi.Mpp1 == null)
                                    ? new Properties.ManagedParagraphProperty[0]
                                    : new Properties.ManagedParagraphProperty[1] { mppi.Mpp1 };
                            Properties.ManagedParagraphProperty[] mppNew =
                                (mppi.Mpp2 == null)
                                    ? new Properties.ManagedParagraphProperty[0]
                                    : new Properties.ManagedParagraphProperty[1] { mppi.Mpp2 };

                            Internal.Navigator.HandleManagedParagraphPropertiesChange(
                                story,
                                cursor,
                                0,
                                mppOld,
        ...
        )
        {
            ...
            if (patcher.FindReplacement(code, out replacement) == false)
            {
                ...
                if (isFlagged)
                {
                    ...
                    story.ConvertToStyledText(
                        story.FlattenStylesAndProperties(styles, properties),
                        out replacement
                    );
                }
                else
                ...
                if (replacement != code)
                {
                    ...
                    if (
                        Properties.ManagedParagraphProperty.CompareEqualContents(mpp1, mpp2)
                        == false
                    )
                    {
            ...
            if (replacement != 0)
            {
                for (int i = pos; i < end; i++)
                {
                    ...
                    this.isPatcherOnBreak = Internal.Navigator.IsParagraphSeparator(
                        Unicode.Bits.GetUnicodeCode(buffer[i])
                    );
                }
            ...
            else
            {
                ...
                this.isPatcherOnBreak = Internal.Navigator.IsParagraphSeparator(
                    Unicode.Bits.GetUnicodeCode(buffer[end - 1])
                );
        ...
        private void Attach(TextStyle style)
        {
            ...
            {
                throw new System.ArgumentException(
                    string.Format("TextStyle named {0} ({1}) already exists", name, textStyleClass),
                    "style"
                );
            }
        ...
        private void Detach(TextStyle style)
        {
            ...
            {
                throw new System.ArgumentException(
                    string.Format("TextStyle named {0} ({1}) does not exist", name, textStyleClass),
                    "style"
                );
            }
        }
        ...
        private class Patcher
        {
            ...
            public void DefineManagedParagraphPropertyInfo(
                ulong code,
                ManagedParagraphPropertyInfo mppi
            )
            {
        ...
        private class ManagedParagraphPropertyInfo
        {
            ...
            public ManagedParagraphPropertyInfo(
                Properties.ManagedParagraphProperty mpp1,
                Properties.ManagedParagraphProperty mpp2
            )
            {
        ...
        public class RedefineOplet : Common.Support.AbstractOplet
        {
            ...
            {
                if ((this.style == other.style) && (this.stylist == other.stylist))
                {
                    return true;
                }
        ...
        private class NewOplet : Common.Support.AbstractOplet
        {
            ...
            public override Common.Support.IOplet Undo()
            {
                ...
                this.style.IsDeleted = true;
                this.stylist.RedefineTextStyle(
                    null,
                    this.style,
                    new Property[0],
                    new TextStyle[1] { defaultStyle }
