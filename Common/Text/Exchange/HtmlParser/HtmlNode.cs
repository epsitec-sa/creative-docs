...
namespace Epsitec.Common.Text.Exchange.HtmlParser
{
    ...
    public class HtmlNodeCollection : CollectionBase
    {
        ...
        {
            get { return (HtmlNode)base.InnerList[index]; }
            set
            {
                if (mParent != null)
        ...
        public HtmlNodeCollection FindByName(string name, bool searchChildren)
        {
            ...
            foreach (HtmlNode node in base.List)
            {
                if (node is HtmlElement)
                {
                    ...
                    {
                        foreach (
                            HtmlNode matchedChild in ((HtmlElement)node).Nodes.FindByName(
                                name,
                                searchChildren
                            )
                        )
        ...
        public HtmlNodeCollection FindByAttributeName(string attributeName, bool searchChildren)
        {
            ...
            foreach (HtmlNode node in base.List)
            {
                if (node is HtmlElement)
                {
                    ...
                    {
                        foreach (
                            HtmlNode matchedChild in ((HtmlElement)node).Nodes.FindByAttributeName(
                                attributeName,
                                searchChildren
                            )
                        )
        ...
        public HtmlNodeCollection FindByAttributeNameValue(
        ...
        )
        {
            ...
            foreach (HtmlNode node in base.List)
            {
                if (node is HtmlElement)
                {
                    ...
                    {
                        foreach (
                            HtmlNode matchedChild in (
                                (HtmlElement)node
                            ).Nodes.FindByAttributeNameValue(
                                attributeName,
                                attributeValue,
                                searchChildren
                            )
