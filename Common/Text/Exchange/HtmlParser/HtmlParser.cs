...
namespace Epsitec.Common.Text.Exchange.HtmlParser
{
    ...
    internal class HtmlParser
    {
                    ...
                    )
                    {
                        ...
                        if (index < tokens.Count && "=".Equals(tokens[index]))
                        {
                            ...
                            HtmlAttribute attribute = new HtmlAttribute(
                                attributeName,
                                HtmlEncoder.DecodeValue(attributeValue)
                            );
        ...
        private void MoveNodesDown(
            ref HtmlNodeCollection nodes,
            int nodeIndex,
            HtmlElement newParent
        )
        {
        ...
        private int FindTagOpenNodeIndex(HtmlNodeCollection nodes, string name)
        {
            for (int index = nodes.Count - 1; index >= 0; index--)
            {
                ...
                {
                    if (
                        ((HtmlElement)nodes[index]).Name.ToLower().Equals(name.ToLower())
                        && ((HtmlElement)nodes[index]).Nodes.Count == 0
                        && ((HtmlElement)nodes[index]).IsTerminated == false
                    )
                    {
                        return index;
                    }
        ...
        private string PreprocessScript(string input, string tagName)
        {
            ...
            while (index < input.Length)
            {
                bool omitBody = false;
                if (
                    index + tagNameLen + 1 < input.Length
                    && input.Substring(index, tagNameLen + 1).ToLower().Equals("<" + tagName)
                )
                {
                    ...
                    if (!omitBody)
                    {
                        ...
                        while (
                            index + tagNameLen + 3 < input.Length
                            && !input
                                .Substring(index, tagNameLen + 3)
                                .ToLower()
                                .Equals("</" + tagName + ">")
                        )
                        {
        ...
        private StringCollection GetTokens(string input)
        {
            ...
            while (i < input.Length)
            {
                ...
                else if (status == ParseStatus.ReadStartTag)
                {
                    ...
                    while (
                        i < input.Length && input.Substring(i, 1).IndexOfAny(WHITESPACE_CHARS) != -1
                    )
                    {
                        i++;
                    }
                    ...
                    while (
                        i < input.Length
                        && input.Substring(i, 1).IndexOfAny(" \r\n\t/>".ToCharArray()) == -1
                    )
                    {
                        i++;
                    }
                ...
                else if (status == ParseStatus.ReadAttributeValue)
                {
                    ...
                    else
                    {
                        ...
                        while (
                            i < input.Length
                            && input.Substring(i, 1).IndexOfAny(WHITESPACE_CHARS) != -1
                        )
                        {
                            i++;
                        }
