...
namespace Epsitec.Common.Text.Tests
{
    ...
    public sealed class CheckNavigator
    {
        ...
        private static void TestParagraph()
        {
            ...
            properties.Add(
                new Properties.MarginsProperty(10.0, 10.0, Properties.SizeUnits.Millimeters)
            );
            ...
            Debug.Assert.IsTrue(
                Internal.Navigator.GetParagraphStyles(story, cursor, 0, out styles)
            );
            Debug.Assert.IsTrue(styles.Length == 1);
            Debug.Assert.IsTrue(styles[0].Name == "Normal");

            Debug.Assert.IsTrue(
                Internal.Navigator.GetParagraphStyles(story, cursor, 45, out styles)
            );
            Debug.Assert.IsTrue(styles.Length == 1);
            ...
            Debug.Assert.IsTrue(
                Internal.Navigator.GetParagraphProperties(story, cursor, 0, out props)
            );
            Debug.Assert.IsTrue(props.Length == 0);

            Debug.Assert.IsTrue(
                Internal.Navigator.GetParagraphProperties(story, cursor, 40, out props)
            );
            ...
            Debug.Assert.IsTrue(
                Internal.Navigator.GetParagraphProperties(story, cursor, 5, out props)
            );
            Debug.Assert.IsTrue(props.Length == 0);
