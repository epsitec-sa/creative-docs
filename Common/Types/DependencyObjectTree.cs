...
namespace Epsitec.Common.Types
{
    ...
    public sealed class DependencyObjectTree : DependencyObject
    {
        ...
        public static DependencyObjectTreeSnapshot CreateInheritedPropertyTreeSnapshot(
            DependencyObject root
        )
        {
            IList<DependencyProperty> properties = DependencyObjectTree.FindInheritedProperties(
                root
            );
            return DependencyObjectTree.CreatePropertyTreeSnapshot(root, properties);
        }

        public static DependencyObjectTreeSnapshot CreatePropertyTreeSnapshot(
            DependencyObject root,
        ...
        public static DependencyObjectTreeSnapshot CreatePropertySubtreeSnapshot(
            DependencyObject root,
            DependencyProperty property
        )
        {
        ...
        public static DependencyObject FindFirst(
            DependencyObject root,
        ...
        )
        {
            ...
            while (roots.Count > 0)
            {
                for (int i = 0; i < roots.Count; i++)
                {
                    ...
                    if ((itemName != null) && (regex.IsMatch(itemName)))
                    {
                        return item;
                    }
                }
        ...
        public static DependencyObject[] FindAll(DependencyObject root, string name)
        {
            ...
            while (roots.Count > 0)
            {
                for (int i = 0; i < roots.Count; i++)
                {
                    ...
                    if ((name == "*") || (DependencyObjectTree.GetName(item) == name))
                    {
                        result.Add(item);
                    }
                }
        ...
        public static DependencyObject[] FindAll(
            DependencyObject root,
            System.Text.RegularExpressions.Regex regex
        )
        {
        ...
        public static DependencyObject FindChild(DependencyObject root, string[] path, int start)
        {
            ...
            DependencyObject item = DependencyObjectTree.FindFirst(
                root,
                path[start],
                FindMode.SearchMatchingChildrenOnly
            );
        ...
        public static DependencyObject FindParentDefiningProperty(
        ...
        public static ICollection<DependencyObject> GetChildren(DependencyObject o)
        {
            ...
            {
                return o.GetValue(DependencyObjectTree.ChildrenProperty)
                    as ICollection<DependencyObject>;
            }
        }
        ...
        private static DependencyObject FindFirst(DependencyObject root, string name, FindMode mode)
        {
            ...
            while (roots.Count > 0)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if ((findAll) || (skipRoot) || (DependencyObjectTree.GetName(item) == null))
                    {
                        if (DependencyObjectTree.GetHasChildren(item))
                        {
                            roots.AddRange(DependencyObjectTree.GetChildren(item));
        ...
        private static void FindInheritedProperties(
            DependencyObject item,
            List<DependencyProperty> list
        )
        {
            while (item != null)
            {
