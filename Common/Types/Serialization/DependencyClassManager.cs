...
namespace Epsitec.Common.Types.Serialization
{
    ...
    public sealed class DependencyClassManager
    {
        ...
        {
            get { return DependencyClassManager.current != null; }
        }
        ...
        private void Analyze(Assembly assembly)
        {
            ...
            try
            {
                ...
                foreach (
                    DependencyClassAttribute attribute in DependencyClassAttribute.GetConverterAttributes(
                        assembly
                    )
                )
                {
                    this.converters[attribute.Type] =
                        System.Activator.CreateInstance(attribute.Converter)
                        as ISerializationConverter;
                }

                foreach (
                    DependencyConverterAttribute attribute in DependencyConverterAttribute.GetConverterAttributes(
                        assembly
                    )
                )
                {
