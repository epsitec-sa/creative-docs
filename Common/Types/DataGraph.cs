...
namespace Epsitec.Common.Types
{
    ...
    public class DataGraph : IDataGraph
    {
        ...
        public DataGraph(IDataFolder root)
            : this()
        {
            ...
            for (int i = 0; i < names.Length; i++)
            {
                ...
                if ((item.Classes & DataItemClasses.Folder) == 0)
                {
                    throw new System.ArgumentOutOfRangeException(
                        "path",
                        path,
                        string.Format("Path element {0} does not refer to a folder.", i - 1)
                    );
                }
                ...
                if ((name.Length > 2) && (name[0] == '[') && (name[name.Length - 1] == ']'))
                {
                    ...
                    else
                    {
                        throw new System.ArgumentOutOfRangeException(
                            "path",
                            path,
                            string.Format("Path element {0} (index {1}) is not valid.", i, index)
                        );
                    }
                }
                else
                {
                    ...
                    if (item == null)
                    {
                        throw new System.ArgumentOutOfRangeException(
                            "path",
                            path,
                            string.Format("Path element {0} ({1}) is not valid.", i, name)
                        );
                    }
                }
        ...
        public static bool Equal(IDataItem i1, IDataItem i2)
        {
            ...
            throw new System.InvalidOperationException(
                string.Format(
                    "Cannot compare items of type {0} and {1}.",
                    i1.GetType().Name,
                    i2.GetType().Name
                )
            );
        ...
        {
            if ((src == null) || (dst == null) || (src == dst) || (src.Name != dst.Name))
            {
                return;
            }
