    ...
    public class MultilingualText : System.IEquatable<MultilingualText>
    {
        ...
        public bool ContainsLocalizations
        {
            get
            {
                ...
                {
                    if (
                        this.ContainsLanguage(MultilingualText.DefaultTwoLetterISOLanguageToken)
                        == false
                    )
                    {
                        return true;
        ...
        public FormattedText GetGlobalText()
        {
            ...
            )
            {
                return new FormattedText(
                    this.texts[MultilingualText.DefaultTwoLetterISOLanguageToken]
                );
            }
            ...
            foreach (var prefixedText in prefixedTexts)
            {
                if (prefixedText.Length < 3 || prefixedText[2] != ':')
                {
                    throw new System.FormatException(
                        string.Format("Argument '{0}' does not contain valid prefix", prefixedText)
                    );
                }
        ...
        public static string GetTwoLetterISOLanguageName(FormattedText text, int index)
        {
            if (!text.IsNullOrEmpty())
            {
                ...
                {
                    int i = s.Substring(0, index)
                        .LastIndexOf(MultilingualText.DivBeginWithLanguageAttribute);

                    if (i != -1)
                    {
                        var twoLetter = s.Substring(
                            i + MultilingualText.DivBeginWithLanguageAttribute.Length + 1,
                            2
                        );
        ...
        private static IEnumerable<string> GetLanguageDivSections(string text)
        {
            ...
            {
                yield return MultilingualText.GetDivElement(
                    MultilingualText.DefaultTwoLetterISOLanguageToken,
                    text.Substring(pos)
                );
            }
        }
        ...
        private static string GetDivElement(string twoLetterISOLanguageName, string text)
        {
            return string.Concat(
                MultilingualText.DivBeginWithLanguageAttribute,
                @"""",
                twoLetterISOLanguageName,
                @"""",
                ">",
                text,
                MultilingualText.DivEnd
            );
        }
        ...
        private static IEnumerable<KeyValuePair<string, string>> GetLanguageTexts(string text)
        {
            return MultilingualText
                .GetLanguageDivSections(text)
                .Select(x => MultilingualText.GetLanguageKeyValuePairFromDivSection(x));
        }

        private static KeyValuePair<string, string> GetLanguageKeyValuePairFromDivSection(
            string section
        )
        {
            System.Diagnostics.Debug.Assert(
                section.StartsWith(MultilingualText.DivBeginWithLanguageAttribute)
            );
            ...
            string twoLetterISOLanguageName = section.Substring(
                quotePosBegin,
                quotePosEnd - quotePosBegin
            );
        ...
        static MultilingualText()
        {
...

            {
                ...
                System.Diagnostics.Debug.Assert(
                    t == "<div lang=\"en\">hello_John</div><div lang=\"fr\">bonjour_Jean</div>"
                );
            }

            {
                ...
                System.Diagnostics.Debug.Assert(
                    t
                        == "<div lang=\"en\">John</div><div lang=\"fr\">bonjour_Jean</div><div lang=\"it\">ciao_</div>"
                );
            }

            {
                ...
                System.Diagnostics.Debug.Assert(
                    t
                        == "<div lang=\"en\">hello_John</div><div lang=\"fr\">bonjourbonjour_Jean</div><div lang=\"it\">ciao_</div>"
                );
            }
        }
        #endregion
        ...
        private const string DivBeginWithLanguageAttribute =
            MultilingualText.DivBegin + MultilingualText.LanguageAttribute; // "<div lang="
