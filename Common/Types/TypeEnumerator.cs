...
namespace Epsitec.Common.Types
{
    ...
    public sealed class TypeEnumerator
    {
        private TypeEnumerator()
        {
            ...
            Epsitec.Common.Support.AssemblyLoader.AssemblyLoaded +=
                this.HandleCurrentDomainAssemblyLoaded;
        ...
        {
            get { return TypeEnumerator.instance; }
        }
        ...
        public System.Type FindType(string typeName)
        {
            ...
            else
            {
                throw new System.ArgumentException(
                    string.Format("Ambiguous type name: found {0} matches", count)
                );
            }
        }
        ...
        private void HandleCurrentDomainAssemblyLoaded(
            object sender,
        ...
        private static bool IsForeignAssembly(Assembly assembly)
        {
            ...
            if (
                (name.Contains(", PublicKeyToken=b77a5c561934e089"))
                || //	Core Microsoft dlls
                (name.Contains(", PublicKeyToken=b03f5f7f11d50a3a"))
                || //	Core Microsoft dlls
                (name.Contains(", PublicKeyToken=31bf3856ad364e35"))
                || //	DocumentFormat.OpenXml
                (name.Contains(", PublicKeyToken=64014856190afd81"))
                || //	Microsoft.AspNet.SignalR
                (name.Contains(", PublicKeyToken=5c1f2a6c07aed9bf"))
                || //	Microsoft.Owin
                (name.Contains(", PublicKeyToken=3750abcc3150b00c"))
                || //	FirebirdSql
                (name.Contains(", PublicKeyToken=30ad4fe6b2a6aeed"))
                || //	Newtonsoft
                (name.Contains(", PublicKeyToken=f0ebd12fd5e55cc5"))
                || //	Owin
                (name.Contains(", PublicKeyToken=14313db67a4e0b6a"))
                || //	NancyFx
                (name.Contains(", PublicKeyToken=aa2558b85ebe6c6b"))
                || //	ServiceStack.Text
                (name.Contains(", PublicKeyToken=fd1eb21b62ae805b"))
                || //	ClosedXml
                (name.Contains(", PublicKeyToken=7bb1d2b1829d428e"))
            ) //	Epsitec.Twixclip
            {
                return true;
            }
