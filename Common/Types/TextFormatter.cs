...
namespace Epsitec.Common.Types
{
    public static class TextFormatter
    {
        ...
        {
            get { return TextFormatter.detailLevel; }
        }
        ...
        public static FormattedText DefaultUnknownText
        {
            get
            {
                var text = TextFormatter.GetCurrentCultureText(
                    Epsitec.Common.Types.Res.StringIds.Text.Unknown
                );
        ...
        public static FormattedText GetMonolingualText(
            FormattedText text,
        ...
        )
        {
            if (MultilingualText.IsMultilingual(text))
            {
                ...
                {
                    monolingual = multilingualText.GetTextOrDefault(
                        TextFormatter.CurrentTwoLetterISOLanguageName
                    );
                }
        ...
        public static FormattedText FormatText(params object[] values)
        {
            ...
            return new FormattedText(
                string.Join(
                        FormattedText.HtmlBreak,
                        buffer
                            .ToString()
                            .Split(
                                new string[] { FormattedText.HtmlBreak },
        ...
        {
            return Resources.DefaultManager.GetCaption(
                captionId,
                ResourceLevel.Merged,
                TextFormatter.CurrentCulture
            );
        }
        ...
        {
            return new FormattedText(
                Resources.DefaultManager.GetText(
                    druid,
                    ResourceLevel.Merged,
                    TextFormatter.CurrentCulture
                )
        ...
        public static TResult ExecuteUsingCulture<TResult>(
            CultureInfo culture,
        ...
        {
            TextFormatter.ExecuteUsingCulture(
                Resources.FindSpecificCultureInfo(twoLetterCode),
                action
            );
        }
        ...
        {
            return TextFormatter.ExecuteUsingCulture(
                Resources.FindSpecificCultureInfo(twoLetterCode),
                func
            );
        }
        ...
        public static string ConvertToText(object value)
        {
            ...
            FormattedText formattedText = TextFormatterConverter.ToFormattedText(
                value,
                TextFormatter.CurrentCulture,
                TextFormatter.CurrentDetailLevel
            );
            ...
            if (MultilingualText.IsMultilingual(formattedText))
            {
                ...
                return multilingualText
                    .GetTextOrDefault(TextFormatter.CurrentTwoLetterISOLanguageName)
                    .ToString();
            }
            else
            {
        ...
        public static void DefineDefaultConverter(
            System.Type type,
        ...
        public static FormattedText Join(string separator, IEnumerable<FormattedText> collection)
        {
            return new FormattedText(
                string.Join(
                    separator,
                    collection.Select(x => TextFormatter.ConvertToText(x)).ToArray()
                )
            );
        }

        public static System.IDisposable UsingCulture(
            CultureInfo culture,
        ...
        private static void SetCultureOverride(
            CultureInfo culture,
            TextFormatterDetailLevel detailLevel
        )
        {
        ...
        private static void FormatText(System.Text.StringBuilder buffer, List<string> items)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((prefix.IsPunctuationMark()) && ((lastCharacter == prefix) || emptyItem))
                {
                ...
                skipSpace =
                    (emptyItem && skipSpace)
                    || (!emptyItem && TextFormatter.EndsWithWhiteSpace(text));
            }
        }
        ...
        private static void ProcessTags(List<string> items)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ((command.Length > 0) && (command[0] == Prefix.CommandEscape))
                {
                    if ((command == Command.ClearToMarkIfEmpty) && (i > 0))
                    {
                    ...
                    if ((command == Command.ClearToMarkIfEqual) && (i > 2))
                    {
                    ...
                    if ((command == Command.IfEmpty) && (i > 1))
                    {
                    ...
                    if ((command == Command.IfElseEmpty) && (i > 2))
                    {
        ...
        static TextFormatter()
        {
            ...
            TextFormatter.DefineDefaultConverter<Date>(x =>
                x.ToString("d", DateTimeFormatInfo.CurrentInfo)
            );
            TextFormatter.DefineDefaultConverter<Time>(x =>
                x.ToString("t", DateTimeFormatInfo.CurrentInfo)
            );
            TextFormatter.DefineDefaultConverter<System.DateTime>(x =>
                x.ToString("g", DateTimeFormatInfo.CurrentInfo)
            );
            ...
            TextFormatter.DefineDefaultConverter<Druid>(x =>
                x.IsEmpty
                    ? TextFormatter.DefaultUnknownText.ToString()
                    : EntityInfo.GetStructuredType(x).Caption.DefaultLabelOrName
            );
        }
        ...
        > defaultConverters = new Dictionary<System.Type, System.Func<object, string>>();
    }
}
