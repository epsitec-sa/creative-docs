...
namespace Epsitec.Common.Widgets.Layouts
{
    public sealed class GridLayoutEngine
        : DependencyObject,
            ILayoutEngine,
            Types.IListHost<ColumnDefinition>,
            Types.IListHost<RowDefinition>
    {
        ...
        {
            get { return this.columnMeasures.Length; }
        }
        ...
        public int RowCount
        {
            get { return this.rowMeasures.Length; }
        }
        ...
        void ILayoutEngine.UpdateLayout(
            Visual container,
        ...
        void ILayoutEngine.UpdateMinMax(
            Visual container,
        ...
        private void GenerateColumnOffsets(double rectWidth, double[] x)
        {
            ...
            for (int i = 0; i < this.columnMeasures.Length; i++)
            {
                ...
                GridLength length =
                    (i < this.columnDefinitions.Count)
        ...
        private void GenerateRowOffsets(Drawing.Rectangle rect, double[] y, double[] b)
        {
            ...
            for (int i = 0; i < this.rowMeasures.Length; i++)
            {
                ...
                GridLength length =
                    (i < this.rowDefinitions.Count)
                        ? this.rowDefinitions[i].Height
                        : GridLength.Auto;
        ...
        )
        {
            ...
            if (permeable != null)
            {
                ...
                foreach (
                    PermeableCell cell in permeable.GetChildren(column, row, columnSpan, rowSpan)
                )
                {
                    if (cell.Visual != null)
                    {
        ...
        private double GetColumnSpanWidth(int startColumn, int columnSpan)
        {
            ...
            int stopColumn = System.Math.Min(
            ...
            for (int column = startColumn; column < stopColumn; column++)
            {
                ...
                {
                    if ((column < stopColumn - 1) && (column < this.columnDefinitions.Count))
                    {
        ...
        private double GetRowSpanHeight(int startRow, int rowSpan)
        {
            ...
            for (int row = startRow; row < stopRow; row++)
            {
                ...
                {
                    if ((row + 1 < stopRow) && (row < this.rowDefinitions.Count))
                    {
        ...
        private class MinMaxUpdater
        {
            ...
            {
                get { return this.columnMeasureList; }
            }

            public List<RowMeasure> RowMeasureList
            {
                get { return this.rowMeasureList; }
            }
            ...
            private void ProcessChild(Visual child, int column, int row)
            {
                ...
                if (permeable != null)
                {
                    ...
                    {
                        foreach (
                            PermeableCell cell in permeable.GetChildren(
                                column,
                                row,
                                columnSpan,
                                rowSpan
                ...
                if (columnSpan == 1)
                {
                    ...
                    bool auto =
                        column < this.grid.ColumnDefinitions.Count
                            ? this.grid.ColumnDefinitions[column].Width.IsAuto
                            : true;

                    columnMeasure.UpdateMin(
                        this.passId,
                        (auto ? measureDx.Desired : measureDx.Min) + margins.Width
                    );
                ...
                if (rowSpan == 1)
                {
                    ...
                    bool auto =
                        row < this.grid.RowDefinitions.Count
                            ? this.grid.RowDefinitions[row].Height.IsAuto
                            : true;

                    rowMeasure.UpdateMin(
                        this.passId,
                        (auto ? measureDy.Desired : measureDy.Min) + margins.Height
                    );
            ...
            private double GetColumnSpanWidth(int column, int span)
            {
                ...
                for (int i = 0; i < span; i++)
                {
                    if ((i > 0) && (column + i - 1 < this.grid.columnDefinitions.Count))
                    {
                        dx += this.grid.columnDefinitions[column + i - 1].LeftBorder;
                    }

                    if ((i + 1 < span) && (column + i < this.grid.columnDefinitions.Count))
                    {
            ...
            private double GetRowSpanHeight(int row, int span)
            {
                ...
                for (int i = 0; i < span; i++)
                {
                    if ((i > 0) && (row + i - 1 < this.grid.rowDefinitions.Count))
                    {
                        dy += this.grid.rowDefinitions[row + i - 1].TopBorder;
                    }

                    if ((i + 1 < span) && (row + i < this.grid.rowDefinitions.Count))
                    {
        ...
        private static void NotifyGridPropertyInvalidated(
        ...
        public static readonly DependencyProperty ColumnProperty =
            DependencyProperty.RegisterAttached(
                "Column",
                typeof(int),
                typeof(GridLayoutEngine),
                new DependencyPropertyMetadata(-1, GridLayoutEngine.NotifyGridPropertyInvalidated)
            );
        ...
        public static readonly DependencyProperty ColumnDefinitionsProperty =
            DependencyProperty.RegisterReadOnly(
                ...
                new DependencyPropertyMetadata(
                    GridLayoutEngine.GetRowDefinitionsValue
        ...
        public static readonly DependencyProperty RowDefinitionsProperty =
            DependencyProperty.RegisterReadOnly(
                ...
                typeof(GridLayoutEngine),
                new DependencyPropertyMetadata(
                    GridLayoutEngine.GetColumnDefinitionsValue
                ).MakeReadOnlySerializable()
            );
