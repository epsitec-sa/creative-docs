namespace Epsitec.Common.Widgets.Adorners
{
    ...
    public class LookAqua : AbstractAdorner
    {
        ...
        protected override void RefreshColors()
        {
            ...
            this.colorCaption = Drawing.Color.FromAlphaRgb(
                0.7,
                0.0 / 255.0,
                115.0 / 255.0,
                244.0 / 255.0
        ...
        )
        {
            ...
            switch (type)
            {
                ...
                case GlyphShape.TriangleUp:
                    path.MoveTo(center.X, center.Y + rect.Height * spikeShift);
                    path.LineTo(
                        center.X - rect.Width * baseShiftH * 1.5,
                        center.Y - rect.Height * baseShiftV
                    );
                    path.LineTo(
                        center.X + rect.Width * baseShiftH * 1.5,
                ...
                case GlyphShape.TriangleRight:
                    ...
                    path.LineTo(
                        center.X - rect.Width * baseShiftV,
                        center.Y + rect.Height * baseShiftH * 1.5
                    );
                    path.LineTo(
                        center.X - rect.Width * baseShiftV,
                        center.Y - rect.Height * baseShiftH * 1.5
        ...
        )
        {
            ...
            )
            {
                if ((state & WidgetPaintState.Enabled) != 0)
                {
                    ...
                    {
                        Drawing.Path path = this.PathRoundRectangle(
                            rect,
                            this.RetRadiusButton(rect)
                        );
                        graphics.Rasterizer.AddSurface(path);
                        if (
                            style == ButtonStyle.DefaultAccept
        ...
        )
        {
            ...
            if (dir == Widgets.Direction.Left)
            {
                ...
                Drawing.Path path = this.PathRoundRectangle(
                    frameRect,
                    this.RetRadiusButton(frameRect)
                );
                graphics.Rasterizer.AddSurface(path);
                graphics.RenderSolid(gray);

                graphics.AddLine(
                    frameRect.Left + frameRect.Height / 2,
                    frameRect.Top - 1,
                    frameRect.Right - frameRect.Height / 2,
                    frameRect.Top - 1
                );
            ...
            else
            {
                ...
                graphics.AddLine(
                    frameRect.Left + 1,
                    frameRect.Bottom + frameRect.Width / 2,
                    frameRect.Left + 1,
                    frameRect.Top - frameRect.Width / 2
                );
                graphics.RenderSolid(shadow);
                ...
                if (mainWindow)
                {
                    ...
                    for (double y = windowRect.Bottom; y < windowRect.Top; y += dy)
                    {
                        for (double x = windowRect.Left; x < windowRect.Right; x += dx)
                        {
                            ...
                            {
                                graphics.PaintImage(
                                    this.dynamicReflect ? this.metal2 : this.metal1,
                                    rect,
                                    new Drawing.Rectangle(0, 0, dx, dy)
                                );
                            }
                        }
                    }
                ...
                for (double y = paintRect.Bottom; y < paintRect.Top + h * 2; y += h * 2)
                {
                    ...
                    if (y1 < y2)
                    {
                        graphics.AddFilledRectangle(
                            new Drawing.Rectangle(paintRect.Left, y1, paintRect.Width, y2 - y1)
                        );
                    }
                }
        ...
        protected void PaintRectShadow(
            Drawing.Graphics graphics,
            Drawing.Rectangle rect,
            int deep,
            double alphaTop,
        ...
        )
        {
            ...
            for (int i = 0; i < deep; i++)
            {
                ...
                graphics.AddLine(rect.Left, rect.Top - 0.5, rect.Right, rect.Top - 0.5);
                if (hole)
                    graphics.RenderSolid(Drawing.Color.FromAlphaRgb(alphaTop, 0, 0, 0));
                else
                    graphics.RenderSolid(Drawing.Color.FromAlphaRgb(alphaTop, 1, 1, 1));
        ...
        public override Drawing.Color ColorBorder
        {
            ...
            {
                if (this.metalRenderer)
                    return Drawing.Color.FromBrightness(0.55);
        ...
        public override Drawing.Color ColorTextDisplayMode(TextFieldDisplayMode mode)
        {
            switch (mode)
            {
                ...
                case TextFieldDisplayMode.OverriddenValue:
                    return Drawing.Color.FromRgb(197.0 / 255.0, 231.0 / 255.0, 255.0 / 255.0);
                case TextFieldDisplayMode.InheritedValue:
                    return Drawing.Color.FromRgb(249.0 / 255.0, 223.0 / 255.0, 187.0 / 255.0);
            }
            return Drawing.Color.Empty;
        }

        public override double AlphaMenu
        {
            get { return 0.9; }
        }
        ...
        {
            get { return new Drawing.Margins(5, 5, 0, 7); }
        }
        public override Drawing.Margins GeometryMenuMargins
        {
            get { return new Drawing.Margins(1, 1, 6, 6); }
        }
