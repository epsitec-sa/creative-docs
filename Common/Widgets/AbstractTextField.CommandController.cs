    ...
    public abstract partial class AbstractTextField
    {
        private sealed class CommandController
        {
            ...
            public void CommandSuperscript(CommandDispatcher dispatcher, CommandEventArgs e)
            {
                ...
                this.host.navigator.SelectionSuperscript = !this.host
                    .navigator
            ...
            private void UpdateCommandStatesEnable(bool focused)
            {
                ...
                commandContext.GetCommandState(ApplicationCommands.Cut).Enable =
                    focused && isReadWrite;
                commandContext.GetCommandState(ApplicationCommands.Paste).Enable =
                    focused && isReadWrite;
                commandContext.GetCommandState(ApplicationCommands.Delete).Enable =
                    focused && isReadWrite;
                ...
                commandContext.GetCommandState(ApplicationCommands.MultilingualEdition).Enable =
                    canFormat;
            }
            ...
            private void UpdateCommandStatesActiveState()
            {
                ...
                if (canFormat)
                {
                    commandContext.GetCommandState(ApplicationCommands.Bold).ActiveState = this.host
                        .navigator
                        .SelectionBold
                        ? ActiveState.Yes
                        : ActiveState.No;
                    commandContext.GetCommandState(ApplicationCommands.Italic).ActiveState =
                        this.host.navigator.SelectionItalic ? ActiveState.Yes : ActiveState.No;
                    commandContext.GetCommandState(ApplicationCommands.Underlined).ActiveState =
                        this.host.navigator.SelectionUnderline ? ActiveState.Yes : ActiveState.No;
                    commandContext.GetCommandState(ApplicationCommands.Subscript).ActiveState =
                        this.host.navigator.SelectionSubscript ? ActiveState.Yes : ActiveState.No;
                    commandContext.GetCommandState(ApplicationCommands.Superscript).ActiveState =
                        this.host.navigator.SelectionSuperscript ? ActiveState.Yes : ActiveState.No;
                }
            }
