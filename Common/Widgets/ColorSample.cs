...
namespace Epsitec.Common.Widgets
{
    ...
    public partial class ColorSample : AbstractButton, Behaviors.IDragBehaviorHost
    {
        ...
        public RichColor Color
        {
            get
            {
                return UndefinedValue.GetValue<RichColor>(
                    this.GetValue(ColorSample.ColorProperty),
                    RichColor.Empty,
                    true
                );
            }
        ...
        {
            get { return (bool)this.GetValue(ColorSample.DragSourceFrameProperty); }
            set { this.SetValue(ColorSample.DragSourceFrameProperty, value); }
        }

        public bool DragSourceEnable
        {
            get { return (bool)this.GetValue(ColorSample.DragSourceEnableProperty); }
            set
            {
                if (value)
                {
        ...
        {
            get { return this.GetValue(ColorSample.DragHostProperty) as ColorSample; }
            set
            {
                if (value == null)
                {
        ...
        public void OnDragEnd()
        {
            ...
            if (this.dragInfo.Target != null)
            {
                ...
                {
                    if (
                        Message.CurrentState.IsShiftPressed || Message.CurrentState.IsControlPressed
                    )
                    {
        ...
        protected override bool FilterTabNavigationSibling(
            Widget sibling,
            TabNavigationDir dir,
            TabNavigationMode mode
        )
        {
            if ((mode == TabNavigationMode.ActivateOnTab) && (sibling != this))
            {
        ...
        protected override void PaintBackgroundImplementation(Graphics graphics, Rectangle clipRect)
        {
            ...
            if (this.DragSourceFrame && this.ActiveState == ActiveState.Yes)
            {
                Rectangle r = rect;
            ...
            if (this.IsEnabled)
            {
                ...
                if (this.IsFocused && this.DragHost == null && this.dragInfo == null)
                {
                ...
                if (this.dragInfo != null && this.dragInfo.Color.IsValid)
                {
        ...
        private static Color GetHiliteColor(Color color)
        {
            ...
            if ((s * color.A < 0.2) && (v > 0.8 * color.A))
            {
        ...
        private void UpdateSwapping()
        {
            ...
            if (this.dragInfo != null)
            {
                ...
                {
                    bool swap =
                        (Message.CurrentState.IsShiftPressed)
        ...
        private ColorSample FindDropTarget(Point mouse)
        {
            ...
            return this.Window.Root.FindChild(
        ...
        {
            add { this.AddUserEventHandler(ColorSample.ColorChangedEvent, value); }
            remove { this.RemoveUserEventHandler(ColorSample.ColorChangedEvent, value); }
        }

        public static readonly DependencyProperty DragHostProperty =
            DependencyProperty<ColorSample>.Register<ColorSample>(
                x => x.DragHost,
                new DependencyPropertyMetadata().MakeNotSerializable()
            );
        public static readonly DependencyProperty ColorProperty =
            DependencyProperty<ColorSample>.Register<RichColor>(
                x => x.Color,
        ...
        public static readonly DependencyProperty DragSourceFrameProperty =
            DependencyProperty<ColorSample>.Register<bool>(
                x => x.DragSourceFrame,
                new Helpers.VisualPropertyMetadata(
                    false,
                    Helpers.VisualPropertyMetadataOptions.AffectsDisplay
