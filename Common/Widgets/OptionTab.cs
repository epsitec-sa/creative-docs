...
namespace Epsitec.Common.Widgets
{
    public class OptionTab : Widget
    {
        ...
        {
            get { return this.hiliteCloseGlyph; }
            set
            {
        ...
        protected override void ProcessMessage(Message message, Point pos)
        {
            if (message.IsMouseType)
            {
                ...
                if ((message.MessageType != MessageType.MouseLeave) & (closeBounds.Contains(pos)))
                {
        ...
        private Rectangle GetCloseGlyphBounds()
        {
            ...
            var center = new Point(
                frame.Right - 12,
                System.Math.Max(System.Math.Floor(frame.Center.Y), frame.Top - 16)
            );
