...
namespace Epsitec.Common.Support.ResourceAccessors
{
    ...
    public class CommandResourceAccessor : CaptionResourceAccessor
    {
        ...
        )
        {
            ...
            if (
                !Types.UndefinedValue.IsUndefinedValue(
                    data.GetValue(Res.Fields.ResourceCommand.Statefull)
                )
            )
            {
                Widgets.Command.SetStatefull(
                    caption,
                    (bool)data.GetValue(Res.Fields.ResourceCommand.Statefull)
                );
            }
            if (
                !Types.UndefinedValue.IsUndefinedValue(
                    data.GetValue(Res.Fields.ResourceCommand.DefaultParameter)
                )
            )
            {
                Widgets.Command.SetDefaultParameter(
                    caption,
                    (string)data.GetValue(Res.Fields.ResourceCommand.DefaultParameter)
                );
            }
            if (
                !Types.UndefinedValue.IsUndefinedValue(
                    data.GetValue(Res.Fields.ResourceCommand.Group)
                )
            )
            {
                Widgets.Command.SetGroup(
                    caption,
                    (string)data.GetValue(Res.Fields.ResourceCommand.Group)
                );
            }

            IList<StructuredData> shortcuts =
                data.GetValue(Res.Fields.ResourceCommand.Shortcuts) as IList<StructuredData>;
            ...
            if (shortcuts != null)
            {
                ...
                foreach (StructuredData item in shortcuts)
                {
                    ...
                    shortcut.SetValue(
                        Widgets.Shortcut.KeyCodeProperty,
                        item.GetValue(Res.Fields.Shortcut.KeyCode) as string
                    );
                    target.Add(shortcut);
                }
            }
        ...
        )
        {
            ...
            ObservableList<StructuredData> shortcuts =
                data.GetValue(Res.Fields.ResourceCommand.Shortcuts)
            ...
            if (containsShortcuts)
            {
                ...
                foreach (Widgets.Shortcut captionShortcut in captionShortcuts)
                {
                    ...
                    shortcut.SetValue(
                        Res.Fields.Shortcut.KeyCode,
                        captionShortcut.GetValue(Widgets.Shortcut.KeyCodeProperty)
                    );
            ...
            object defaultParameterValue = caption.GetValue(
                Widgets.Command.DefaultParameterProperty
            );
        ...
        protected override bool IsEmptyCaption(StructuredData data)
        {
            if (base.IsEmptyCaption(data))
            {
                ...
                string defaultParameter =
                    data.GetValue(Res.Fields.ResourceCommand.DefaultParameter) as string;
                ...
                if (
                    (UndefinedValue.IsUndefinedValue(statefull) || ((bool)statefull == false))
        ...
        )
        {
            ...
            AbstractCaptionResourceAccessor.CopyDeltaValue(
                rawData,
                patchData,
                Res.Fields.ResourceCommand.DefaultParameter
            );
            AbstractCaptionResourceAccessor.CopyDeltaValue(
                rawData,
                patchData,
                Res.Fields.ResourceCommand.Group
            );
            AbstractCaptionResourceAccessor.CopyDeltaValue(
                rawData,
                patchData,
                Res.Fields.ResourceCommand.Shortcuts
            );
            AbstractCaptionResourceAccessor.CopyDeltaValue(
                rawData,
                patchData,
                Res.Fields.ResourceCommand.Statefull
            );
#else
        ...
        )
        {
            ...
            {
                ShortcutListener listener = Listener.FindListener<ShortcutListener>(
                    container,
                    fieldId
                );
        ...
        protected class ShortcutListener : Listener
        {
            public ShortcutListener(
                CommandResourceAccessor accessor,
                CultureMap item,
                StructuredData data
            )
            ...
            public override void HandleCollectionChanging(object sender)
            {
                if (this.SaveField(Res.Fields.ResourceCommand.Shortcuts))
                {
                    ...
                    IList<StructuredData> shortcuts =
                        this.Data.GetValue(Res.Fields.ResourceCommand.Shortcuts)
