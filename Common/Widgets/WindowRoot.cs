...
namespace Epsitec.Common.Widgets
{
    ...
    /// en général une instance de WindowRoot en appelant PlatformWindow.Root.
    /// </summary>
    public sealed class WindowRoot : AbstractGroup
    {
        ...
        {
            get { return this.treeChangeCounter; }
        }

        public ModifierKeys ActiveModifierKeys
        {
            get { return this.activeModifierKeys; }
        }
        ...
        internal void RefreshFocusChain()
        {
            ...
            if ((this.focusChain.Count == 0) && (this.window != null))
            {
        ...
        private bool InternalShortcutHandler(Shortcut shortcut, bool executeFocused)
        {
            if (this.window != null)
            {
                ...
                if (execute)
                {
                    ...
                    List<Command> commands = Types.Collection.ToList(
                        Widgets.Command.FindAll(shortcut)
                    );

                    if (commands.Count > 0)
                    {
                        CommandDispatcherChain dispatcherChain = CommandDispatcherChain.BuildChain(
                            focused ?? this
                        );
                        ...
                        if (dispatcherChain != null)
                        {
                            ...
                            window.QueueCommand(widget, bestCommand, shortcut);
                            return true;
                        }
                    }
                }
                ...
                switch (shortcut.KeyCodeOnly)
                {
                    case KeyCode.Tab:
                        ...
                        dir = Message.CurrentState.IsShiftPressed
                            ? TabNavigationDir.Backwards
                ...
                if ((mode != TabNavigationMode.None) && (dir != TabNavigationDir.None))
                {
        ...
        )
        {
            ...
            else
            {
                ...
                adorner.PaintWindowBackground(
                    graphics,
                    this.Client.Bounds,
                    rect,
                    WidgetPaintState.None
                );
            }
        }
        ...
        {
            add { this.AddUserEventHandler("WindowStylesChanged", value); }
            remove { this.RemoveUserEventHandler("WindowStylesChanged", value); }
        }
        public event EventHandler WindowTypeChanged
        {
            add { this.AddUserEventHandler("WindowTypeChanged", value); }
            remove { this.RemoveUserEventHandler("WindowTypeChanged", value); }
        }
