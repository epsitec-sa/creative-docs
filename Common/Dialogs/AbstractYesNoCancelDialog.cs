...
namespace Epsitec.Common.Dialogs
{
    ...
    {
        public AbstractYesNoCancelDialog(
            string dialogTitle,
            string yesText,
            string noText,
            string cancelText,
        ...
        protected override Window CreateWindow()
        {
            ...
            button1.SetManualBounds(
                new Drawing.Rectangle(
                    dialogWindow.ClientSize.Width - 3 * 75 - 3 * 8,
                    16,
                    75,
                    button1.PreferredHeight
                )
            );
            button1.FormattedText = this.yesText.IsNullOrEmpty()
                ? Widgets.Res.Strings.Dialog.Button.Yes
                : this.yesText;
            ...
            button2.FormattedText = this.noText.IsNullOrEmpty()
                ? Widgets.Res.Strings.Dialog.Button.No
                : this.noText;
            ...
            if (this.hideCancel == false)
            {
                ...
                button3.SetManualBounds(
                    new Drawing.Rectangle(
                        dialogWindow.ClientSize.Width - 1 * 75 - 1 * 8,
                        16,
                        75,
                        button3.PreferredHeight
                    )
                );
                button3.FormattedText = this.cancelText.IsNullOrEmpty()
            ...
            AbstractMessageDialog.LayoutButtons(
                dialogWindow.ClientSize.Width,
                button1,
                button2,
                button3
            );
        ...
        protected void CommandValidateDialogYes()
        {
            ...
            if (this.commandYesTemplate != null)
            {
                this.DispatchWindow.QueueCommand(
                    this.DialogWindow,
                    string.Format(this.commandYesTemplate, this.CommandArgs)
                );
            }
        ...
        protected void CommandValidateDialogNo()
        {
            ...
            if (this.commandNoTemplate != null)
            {
                this.DispatchWindow.QueueCommand(
                    this.DialogWindow,
                    string.Format(this.commandNoTemplate, this.CommandArgs)
                );
            }
