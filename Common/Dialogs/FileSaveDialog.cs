...
namespace Epsitec.Common.Dialogs
{
    ...
    public class FileSaveDialog : Helpers.IFilterCollectionHost, IDialog
    {
            ...
            this.dialog.FileOk += (sender, e) =>
            {
                ...
                if (this.PromptForOverwriting)
                {
                    if (System.IO.File.Exists(name))
                    {
                        ...
                        var message = string.Format(
                            Res.Strings.Dialog.File.Message.PromptForOverwriting.ToSimpleText(),
                            System.IO.Path.GetFileName(name)
                        );
                        var result = System.Windows.Forms.MessageBox.Show(
                            this.OwnerWindow == null ? null : this.OwnerWindow.PlatformWindowObject,
                            message,
                            title,
                ...
                if (this.PromptForCreation)
                {
                    if (!System.IO.File.Exists(name))
                    {
                        var title = Res.Strings.Dialog.File.Message.SaveTitle.ToSimpleText();
                        var message = string.Format(
                            Res.Strings.Dialog.File.Message.PromptForCreation.ToSimpleText(),
                            System.IO.Path.GetFileName(name)
                        );
                        var result = System.Windows.Forms.MessageBox.Show(
                            this.OwnerWindow == null ? null : this.OwnerWindow.PlatformWindowObject,
        ...
        {
            get { return this.FileNames[0]; }
            set { this.dialog.FileName = value; }
        }

        public string[] FileNames
        {
            get { return this.GetFixedFileNames(); }
        }
        ...
        public bool PromptForCreation { get; set; }

        public bool PromptForOverwriting { get; set; }
        ...
        private string FixFileName(string name)
        {
            ...
            if (forceExt != null && forceExt != fileExt)
            {
