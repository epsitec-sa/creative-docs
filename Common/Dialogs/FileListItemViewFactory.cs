...
namespace Epsitec.Common.Dialogs
{
    ...
    class FileListItemViewFactory : AbstractItemViewFactory
    {
        ...
        protected override Widget CreateElement(
            string name,
            ItemPanel panel,
            ItemView view,
            ItemViewShape shape
        )
        {
            ...
            {
                case "icon":
                    return FileListItemViewFactory.CreateFileIcon(item, shape, iconSize);
                case "name":
                    return FileListItemViewFactory.CreateFileName(item, shape);
                case "info":
                    return FileListItemViewFactory.CreateFileInfo(item, shape);
                case "date":
                    return FileListItemViewFactory.CreateFileDate(item, shape);
                case "size":
                    return FileListItemViewFactory.CreateFileSize(item, shape);
            }
        ...
        private static Widget CreateFileIcon(FileListItem item, ItemViewShape shape, double size)
        {
            ...
            string iconUri = item.GetIconUri(
                size < 32 ? FileInfoIconSize.Small : FileInfoIconSize.Large
            );
        ...
        private static Widget CreateFileName(FileListItem item, ItemViewShape shape)
        {
            ...
            {
                text = string.Concat(
                    Common.Dialogs.Res.Strings.Dialog.File.Header.FileName,
                    ": <b>",
                    text,
                    "</b>"
                );
                fileName.TextBreakMode =
        ...
        private static Widget CreateFileDate(FileListItem item, ItemViewShape shape)
        {
            ...
            {
                text = string.Concat(
                    Common.Dialogs.Res.Strings.Dialog.File.Header.Date,
                    ": ",
                    text
                );
            }
        ...
        private static Widget CreateFileSize(FileListItem item, ItemViewShape shape)
        {
            ...
            {
                text = string.Concat(
                    Common.Dialogs.Res.Strings.Dialog.File.Header.Size,
                    ": ",
                    text
                );
