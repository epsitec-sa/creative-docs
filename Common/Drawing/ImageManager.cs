    ...
    public sealed class ImageManager : System.IDisposable
    {
        ...
        public bool HasPendingWork
        {
            get
            {
                ...
                {
                    if ((this.pendingWorkQueue.Count > 0) || (this.processingWorkQueue.Count > 0))
                    {
                        return true;
                    }
                    else
        ...
        {
            get { return this.memoryPressureThreshold; }
            set
            {
        ...
        {
            return this.WaitProcessingQueueEvent(
                timeout,
                delegate()
                {
                    lock (this.localExclusion)
        ...
        public void AddMemoryPressure(long size)
        {
            ...
            int pressure = System.Threading.Interlocked.Add(
                ref this.localMemoryPressure,
                (int)size
            );
        ...
        public void TrimCaches(CacheClearing mode, long targetMemoryPressure, bool skipPending)
        {
            lock (this.cacheExclusion)
            {
                ...
                foreach (ImageData image in images)
                {
                    ...
                    {
                        if ((skipPending == false) || (image.QueueStatus == QueueStatus.Inactive))
                        {
                            if (mode == CacheClearing.ReleaseEverything)
                            {
                                ImageManager.ClearImageCache(
                                    CacheClearing.ReleaseLargeBuffers,
                                    image
                                );
                                ImageManager.ClearImageCache(
                                    CacheClearing.ReleaseMediumBuffers,
                                    image
                                );
                                ImageManager.ClearImageCache(
                                    CacheClearing.ReleaseSmallBuffers,
                                    image
                                );
                            }
                            else
        ...
        public static string GetKey(string imageFilePath, System.DateTime imageFileDate)
        {
            return string.Concat(
                imageFilePath.ToLowerInvariant(),
                "|",
                imageFileDate.Ticks.ToString(System.Globalization.CultureInfo.InvariantCulture)
            );
        }
        ...
        {
            get { return this.sampleImageSize; }
            set
            {
        ...
        {
            get { return ImageManager.imageStore; }
        }
            ...
            if (callback != null)
            {
                ...
                finally
                {
                    ...
                    lock (this.localExclusion)
                    {
                        ...
                        {
                            foreach (
                                System.Threading.EventWaitHandle handle in this.waitProcessingQueueEvents
                            )
                            {
                                handle.Set();
                            }
                ...
                lock (this.cacheExclusion)
                {
                    if (this.isCacheTrimmingRequested)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine(
                            "Trimming... VM = "
                                + (process.VirtualMemorySize64 / (1024 * 1024L))
                                + "MB"
                        );
        ...
        private Dictionary<string, ByteReaderCallback> protocols =
            new Dictionary<string, ByteReaderCallback>();
        private List<ImageData> imageList = new List<ImageData>();
        private List<System.Threading.EventWaitHandle> waitProcessingQueueEvents =
            new List<System.Threading.EventWaitHandle>();
