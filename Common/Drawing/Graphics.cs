...
namespace Epsitec.Common.Drawing
{
    ...
    public class Graphics : System.IDisposable, IPaintPort
    {
        ...
        {
            get { return this.SaveClippingRectangle(); }
        }

        public Point ClipOffset
        {
            get { return new Point(this.clipOx, this.clipOy); }
            set
            {
                this.clipOx = value.X;
                this.clipOy = value.Y;
        ...
        {
            get { return RichColor.FromColor(this.originalColor); }
            set
            {
        ...
        {
            get { return RichColor.FromColor(this.FinalColor); }
            set { this.FinalColor = value.Basic; }
        }
        ...
        {
            get { return this.imageFilter; }
            set { this.imageFilter = value; }
        }

        public Margins ImageCrop
        {
            get { return this.imageCrop; }
            set { this.imageCrop = value; }
        }

        public Size ImageFinalSize
        {
            get { return this.imageFinalSize; }
            set { this.imageFinalSize = value; }
        }

        public FillMode FillMode
        {
            get { return this.Rasterizer.FillMode; }
            set { this.Rasterizer.FillMode = value; }
        }
        ...
        public void PaintDashedOutline(
            Path path,
            double width,
            double dash,
            double gap,
        ...
        )
        {
            if (this.transform.OnlyScaleOrTranslate)
            {
                ...
                if ((scaleX < 2) || (scaleX > 25) || (scaleY < 2) || (scaleY > 25))
                {
                    return false;
                }
        ...
        )
        {
            ...
            {
                if ((infos[i].Scale != 1.00) && (infos[i].GlyphClass == GlyphClass.Space))
                {
        ...
        )
        {
            ...
            vectorOx = this.ApplyTransformDirect(vectorOx) - vectorOo;
            Point vectorOy = new Point(0, 1);
            vectorOy = this.ApplyTransformDirect(vectorOy) - vectorOo;
            ...
            double sx =
                (ix2 - ix1 <= 1)
                    ? (Graphics.AlmostInfinite)
                    : (
                        (fillWidth > adjust / fixX)
                            ? (fillWidth - adjust / fixX) / (ix2 - ix1 - adjust)
                            : 1.0
                    );
            double sy =
                (iy2 - iy1 <= 1)
        ...
        {
            this.transform = this.transform.MultiplyByPostfix(
                Drawing.Transform.CreateScaleTransform(sx, sy, cx, cy)
            );
            this.UpdateTransform();
        }
        ...
        {
            this.transform = this.transform.MultiplyByPostfix(
                Drawing.Transform.CreateRotationDegTransform(angle, cx, cy)
            );
            this.UpdateTransform();
        }
        ...
        {
            this.transform = this.transform.MultiplyByPostfix(
                Drawing.Transform.CreateRotationRadTransform(angle, cx, cy)
            );
            this.UpdateTransform();
        }
        ...
        {
            this.transform = this.transform.MultiplyByPostfix(
                Drawing.Transform.CreateTranslationTransform(ox, oy)
            );
            this.UpdateTransform();
        }
        ...
        public Drawing.Rectangle SaveClippingRectangle()
        {
            if (this.hasClipRect)
            {
                return new Drawing.Rectangle(
                    this.clipX1,
                    this.clipY1,
                    this.clipX2 - this.clipX1,
                    this.clipY2 - this.clipY1
                );
            }
        ...
        public bool HasEmptyClippingRectangle
        {
            get
            {
                if (this.hasClipRect)
                {
                    if ((this.clipX1 >= this.clipX2) || (this.clipY1 >= this.clipY2))
                    {
                        return true;
                    }
                }
