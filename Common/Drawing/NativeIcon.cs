    ...
    public static class NativeIcon
    {
        ...
        public static byte[] CreateIcon(byte[] imageBytes, int pitch, int dx, int dy)
        {
            return NativeIcon.CreateIcon(
                Platform.NativeBitmap.CreateFromPremultipliedArgb32(imageBytes, pitch, dx, dy)
            );
        }
            ...
            foreach (int bpp in new int[] { 8, 32 })
            {
                foreach (NativeBitmap image in images)
                {
                    ...
                    if (image.Width <= 64)
                    {
                        ...
                        {
                            case 4:
                                System.Drawing.Bitmap bitmapMask =
                                    SystemDrawingBitmapHelper.Generate1BppBitmapMask(image);
                                System.Drawing.Bitmap bitmap4bpp =
                                    SystemDrawingBitmapHelper.Generate4BppBitmapWithFixedPalette(
                                        image
                                    );
                            ...
                            case 8:
                                System.Drawing.Bitmap bitmap8bpp =
                                    SystemDrawingBitmapHelper.Generate8BppBitmapWithTransparentColor(
                                        image
                                    );
                            ...
                            case 32:
                                System.Drawing.Bitmap bitmap32bpp =
                                    SystemDrawingBitmapHelper.Generate32BppBitmap(image);
