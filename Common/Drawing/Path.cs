...
namespace Epsitec.Common.Drawing
{
    public class Path
    {
        ...
        public void MoveToRelative(double x, double y)
        {
            if (this.hasCurrentPoint == false)
            {
                throw new System.InvalidOperationException(
                    "No current point defined: cannot use relative coordinates"
                );
            }
        ...
        )
        {
            ...
            this.CurveTo(
                this.currentPoint.X + xC1,
                this.currentPoint.Y + yC1,
                /**/this.currentPoint.X + xC1 + xC2,
                this.currentPoint.Y + yC2 + yC2,
                /**/this.currentPoint.X + xC1 + xC2 + x,
                this.currentPoint.Y + yC2 + yC2 + y
            );
        }
        ...
        public void CurveToRelative(double xC, double yC, double x, double y)
        {
            ...
            this.CurveTo(
                this.currentPoint.X + xC,
                this.currentPoint.Y + yC,
                /**/this.currentPoint.X + xC + x,
                this.currentPoint.Y + yC + y
            );
        }
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                ...
                switch (elements[i] & PathElement.MaskCommand)
                {
                    case PathElement.MoveTo:
                        ...
                        buffer.Append(
                            System.String.Format("({0:0.00}, {1:0.00})", points[i].X, points[i].Y)
                        );
                        break;
                    case PathElement.LineTo:
        ...
        private bool SurfaceContainsPointWithoutCurves(double x, double y, double approximationZoom)
        {
            ...
            for (int i = 0; i < elements.Length; i++)
            {
                switch (elements[i] & PathElement.MaskCommand)
                {
                    ...
                    case PathElement.Curve4:
                        throw new System.InvalidOperationException(
                            "Flattened path still contains curves"
                        );
