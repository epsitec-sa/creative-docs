    ...
    public class ImageStore
    {
        ...
        public ImageStore()
        {
            ...
            this.cacheDir = System.IO.Path.Combine(
                ...
                @"OPaC bright ideas\ImageManager.Cache"
            );
        ...
        )
        {
            ...
            if ((image.BitsPerPixel > 8) && (image.IsTransparent == false))
            {
                memory = image.SaveToMemory(
                    new BitmapFileFormat() { Type = BitmapFileType.Jpeg, Quality = 90 }
                );
                extension = ".jpg";
            }
            ...
            if ((memory == null) && (image.BitsPerPixel == 32))
            {
        ...
        {
            this.ScrubCache(
                System.TimeSpan.FromDays(ImageStore.maximumDiskCacheAgeInDays),
                ImageStore.maximumDiskCacheSize
            );
        }
        ...
        )
        {
            ...
            lock (this.records)
            {
                ...
                foreach (ImageRecord record in this.records)
                {
                    if ((record.PathHash == hash) && (record.Date == date))
                    {
        ...
        private struct ImageRecord
        {
            ...
            public ImageRecord(string source)
            {
                ...
                this.date = new System.DateTime(
                    long.Parse(values[1], System.Globalization.CultureInfo.InvariantCulture),
                    System.DateTimeKind.Utc
                );
                this.cacheFileName = values[2];
                this.pixels = int.Parse(
                    values[3],
                    System.Globalization.CultureInfo.InvariantCulture
                );
                this.sourceWidth = int.Parse(
                    values[4],
                    System.Globalization.CultureInfo.InvariantCulture
                );
                this.sourceHeight = int.Parse(
                    values[5],
                    System.Globalization.CultureInfo.InvariantCulture
                );
                this.extra =
                    values.Length > 6 ? string.Join(";", values, 4, values.Length - 4) : null;
            }

            public string PathHash
            {
                get { return this.pathHash; }
            }
            ...
            {
                get { return this.sourceWidth; }
            }

            public int SourceHeight
            {
                get { return this.sourceHeight; }
            }
