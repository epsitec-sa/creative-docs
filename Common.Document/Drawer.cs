...
namespace Epsitec.Common.Document
{
    ...
    public class Drawer
    {
        ...
        )
        {
            ...
            foreach (Shape shape in shapes)
            {
                if (
                    ...
                    || shape.Aspect == Aspect.OnlyDetect
                )
                    continue;
                ...
                {
                    if (!shape.IsVisible || shape.Path.IsEmpty)
                        continue;
                ...
                if (shape.Type == Type.Text)
                {
                    ...
                    {
                        if (!shape.IsVisible && mode != DrawShapesMode.OnlyText)
                            continue;
                    ...
                    if (
                        ...
                        && (
                            !drawingContext.DrawImageFilter(
                                new DrawingContext.DrawImageFilterInfo(shape.Object, "image")
                            )
                        )
                    )
                    {
        ...
        public static void DrawPathDash(
            Graphics graphics,
            DrawingContext drawingContext,
            Path path,
            double width,
        ...
        public bool Detect(Point pos, DrawingContext drawingContext, params Shape[] shapes)
        {
            ...
            foreach (Shape shape in shapes)
            {
                if (shape == null || shape.Path == null || shape.Path.IsEmpty)
                {
                    continue;
                }
                ...
                if (shape.Type == Type.Surface)
                {
                    if (!shape.IsVisible)
                        continue;
                    if (Geometry.DetectSurface(shape.Path, pos))
                        return true;
                }

                if (shape.Type == Type.Stroke)
                {
                    if (!shape.IsVisible && shape.IsMisc)
                        continue;
                    double width = 0.0;
                    if (shape.PropertyStroke != null)
                        width = shape.PropertyStroke.Width / 2;
                    width = System.Math.Max(width, drawingContext.MinimalWidth);
                    if (Geometry.DetectOutline(shape.Path, width, pos))
                        return true;
                }
            }
        ...
        public int DetectOutline(Point pos, DrawingContext drawingContext, params Shape[] shapes)
        {
        ...
        )
        {
            ...
            {
                flat = (System.Math.Abs(surface.Sx) < 0.01 && System.Math.Abs(surface.Sy) < 0.01);
            }
            else
            {
                flat = (System.Math.Abs(surface.Sx) < 0.01 || System.Math.Abs(surface.Sy) < 0.01);
            ...
            else // dégradé ?
            {
                ...
                Point corner = sa.ToAbs(
                    new Point(surface.Cx + surface.Sx, surface.Cy + surface.Sy)
                );
        ...
        public static void RenderMotif(
            IPaintPort port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            Path path,
        ...
        protected static void RenderHatch(
            IPaintPort port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            Path path,
        ...
        )
        {
            ...
            {
                if (surface.GetHatchWidth(i) == 0.0 || surface.GetHatchDistance(i) == 0.0)
                    continue;
                ...
                double width = System.Math.Min(
                    surface.GetHatchWidth(i),
                    surface.GetHatchDistance(i)
                );
        ...
        protected static void RenderDots(
            IPaintPort port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            Path path,
        ...
        protected static void RenderSquares(
            IPaintPort port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            Path path,
        ...
        protected static void RenderMotifColor(
            IPaintPort port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            Properties.Gradient surface,
        ...
        )
        {
            if (obj != null && port is PDF.Port)
            {
                ...
                if (type == PDF.Type.TransparencyPattern)
                {
                    ...
                    pdfPort.SetColoredComplexSurface(
                        color,
                        id,
                        firstColor
                            ? PDF.PdfComplexSurfaceType.ExtGStateP1
                            : PDF.PdfComplexSurfaceType.ExtGStateP2
                    );
                    return;
                }
            }
        ...
        protected static void MinRectMotif(
            Properties.Gradient surface,
            SurfaceAnchor sa,
            int i,
            out Point p1,
        ...
        */
        ...
        )
        {
            ...
            if (shape.PropertySurface == null || shape.PropertyStroke == null)
                return;
