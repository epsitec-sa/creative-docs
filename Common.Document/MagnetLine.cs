...
namespace Epsitec.Common.Document
{
    ...
    public class MagnetLine
    {
        ...
        public static Point[] Intersect(MagnetLine line1, MagnetLine line2)
        {
        ...
        public bool Snap(ref Point pos, double margin)
        {
            ...
            Point proj = this.Projection(pos);
            if (Point.Distance(proj, pos) > margin)
                return false;
            pos = proj;
            return true;
        }
        ...
        public bool Compare(MagnetLine line)
        {
            ...
            if (this.handleRank != line.handleRank)
                return false;
            ...
            else // quelconque ?
            {
                ...
                if (System.Math.Abs(delta1 - delta2) > 0.00001)
                    return false;

                double start1 =
                    ((this.p1.Y - this.p2.Y) * this.p1.X / (this.p2.X - this.p1.X)) + this.p1.Y;
                double start2 =
                    ((line.p1.Y - line.p2.Y) * line.p1.X / (line.p2.X - line.p1.X)) + line.p1.Y;
                if (System.Math.Abs(start1 - start2) > 0.00001)
                    return false;
            }
            return true;
        }
