...
namespace Epsitec.Common.Document.TextPanels
{
    ...
    {
        public Font(Document document, bool isStyle, StyleCategory styleCategory)
            : base(document, isStyle, styleCategory)
        {
            ...
            this.fontFace.ComboOpening += new EventHandler<CancelEventArgs>(
                this.HandleFontFaceComboOpening
            );
            ...
            this.fontFeatures = this.CreateIconButton(
                Misc.Icon("FontFeatures"),
                Res.Strings.TextPanel.Font.Tooltip.Features,
                this.HandleFeaturesClicked
            );

            if (Command.IsDefined("Settings"))
            {
                this.buttonSettings = this.CreateIconButton(
                    Misc.Icon("Settings"),
                    Res.Strings.Action.Settings,
                    this.HandleButtonSettingsClicked,
                    false
                );
            }

            this.fontSize = this.CreateTextFieldLabel(
                Res.Strings.TextPanel.Font.Tooltip.Size,
                Res.Strings.TextPanel.Font.Short.Size,
                Res.Strings.TextPanel.Font.Long.Size,
                0,
                0,
                0,
                0,
                Widgets.TextFieldLabel.Type.TextFieldUnit,
            ...
            this.buttonSizeMenu = this.CreateComboButton(
                null,
                Res.Strings.TextPanel.Font.Tooltip.Size,
                this.HandleButtonSizeMenuClicked
            );
            this.buttonSizeMenu.Name = "FontSize Menu";

            this.fontColor = this.CreateColorSample(
                Res.Strings.Action.FontColor,
                this.HandleFieldColorClicked,
                this.HandleFieldColorChanged
            );

            this.fontGlue = this.CreateTextFieldLabelPercent(
                Res.Strings.TextPanel.Font.Tooltip.Glue,
                Res.Strings.TextPanel.Font.Short.Glue,
                Res.Strings.TextPanel.Font.Long.Glue,
                -50.0,
                200.0,
                0.0,
                5.0,
                this.HandleGlueValueChanged
            );
        ...
        public override void OriginColorSelect(int rank)
        {
            ...
            if (rank != -1)
            {
                ...
                if (rank == 0)
                    this.originFieldColor = this.fontColor;
            }
        ...
        protected override void UpdateAfterChanging()
        {
            ...
            this.fontFace.Text = TextLayout.ConvertToTaggedText(
                Misc.FaceInvariantToInvariant(face, style)
            );
            this.fontStyle.Text = TextLayout.ConvertToTaggedText(
                Misc.StyleInvariantToLocale(face, style)
            );
        ...
        protected override void UpdateClientGeometry()
        {
            ...
            if (this.isExtendedSize)
            {
                ...
                this.checkBold.Visibility =
                    (this.styleCategory == StyleCategory.Paragraph) ? false : true;
                r.Offset(0, -18);
                this.checkItalic.SetManualBounds(r);
                this.checkItalic.Visibility =
                    (this.styleCategory == StyleCategory.Paragraph) ? false : true;
            }
            else
            {
        ...
        protected VMenu BuildFeaturesMenu(string face, string style, string[] features)
        {
            ...
            for (int i = 0; i < defs.Length; i++)
            {
                ...
                this.BuildFeaturesMenu(
                    ...
                    active,
                    valid,
                    this.HandleFeaturesMenu
                );
            }

            for (int i = 0; i < supported.Length; i++)
            {
                if (Misc.IsInsideList(defs, supported[i]))
                    continue;
        ...
        protected void BuildFeaturesMenu(
            VMenu menu,
            OpenType.Font font,
            string text,
            string feature,
        ...
        )
        {
            ...
            {
                if (table.LookupType == OpenType.LookupType.Alternate)
                    return;
            }
        ...
        private void HandleFontFaceComboOpening(object sender, CancelEventArgs e)
        {
            ...
            System.Collections.ArrayList fontList = Misc.MergeFontList(
                Misc.GetFontList(false),
                this.document.Settings.QuickFonts,
                quickOnly,
                selectedFontFace,
                out quickCount
            );
        ...
        private void HandleFieldColorClicked(object sender, MessageEventArgs e)
        {
            ...
            if (this.originFieldColor == this.fontColor)
        ...
        private void HandleCheckBoldActiveStateChanged(object sender)
        {
            ...
            )
            {
                ...
                this.TextWrapper.Defined.InvertBold = value;
                this.TextWrapper.DefineOperationName(
                    Res.Commands.FontBold.CommandId,
                    Res.Strings.Action.FontBold
                );
                this.TextWrapper.ResumeSynchronizations();
                this.ActionMade();
            }
        ...
        private void HandleCheckItalicActiveStateChanged(object sender)
        {
            ...
            )
            {
                ...
                this.TextWrapper.Defined.InvertItalic = value;
                this.TextWrapper.DefineOperationName(
                    Res.Commands.FontItalic.CommandId,
                    Res.Strings.Action.FontItalic
                );
                this.TextWrapper.ResumeSynchronizations();
                this.ActionMade();
            }
        ...
        protected VMenu CreateMenu()
        {
            ...
            return Menus.FontSizeMenu.CreateFontSizeMenu(
                size,
                percent ? "%" : "",
                factor,
                isPercent,
