...
namespace Epsitec.Common.Document
{
    ...
    public class Geometry
    {
        public static double GetSliderPosition(Point sliderStarting, Point sliderEnding, Point pos)
        {
            ...
            {
                return Point.Distance(sliderStarting, pos)
                    / Point.Distance(sliderStarting, sliderEnding);
            }
            else
            {
        ...
        public static Point ComputeArrowExtremity(
            Point p1,
            Point p2,
            double para,
            double perp,
        ...
        public static Objects.SelectedSegment.Type PathExtract(
            Path path,
            int rank,
            out Point pp1,
            out Point ss1,
        ...
        public static int DetectOutlineRank(
            Path path,
            double width,
            Point pos,
            out Point bp1,
        ...
        protected static bool PathToCurveContinues(Point[] points, int start, int current)
        {
            ...
            else
            {
                ...
                r = Geometry.PathToCurveRapport(
                    points[current - 1] + points[start] - points[start + 1],
                    points[start + 1],
                    points[current]
                );
                return r < 0.5;
        ...
        public static Point[] Intersect(Point a, double ra, Point b, double rb)
        {
            ...
            {
                double sqr = (
                    (ra * ra - rb * rb + a.X * a.X + b.X * b.X - 2 * a.X * b.X) / (2 * (b.X - a.X))
                );
            ...
            double B = 2 * (b.Y - a.Y);
            double C =
                System.Math.Pow(b.X - a.X, 2) + System.Math.Pow(b.Y - a.Y, 2) - rb * rb + ra * ra;
            double delta =
                System.Math.Pow(2 * A * C, 2) - 4 * (A * A + B * B) * (C * C - B * B * ra * ra);
            ...
            if (B == 0)
            {
                ...
                double sqr2 = System.Math.Sqrt(
                    rb * rb - System.Math.Pow((2 * C - A * A) / 2 * A, 2)
                );
                p.Y = a.Y + B / 2 + sqr2;
                q.Y = a.Y + B / 2 - sqr2;
        ...
        public static bool IsRectangular(Point p0, Point p1, Point p2, Point p3)
        {
            ...
            if (!Geometry.IsRight(p3, p0, p2))
                return false;
            if (!Geometry.IsRight(p0, p2, p1))
                return false;
            if (!Geometry.IsRight(p2, p1, p3))
                return false;
            if (!Geometry.IsRight(p1, p3, p0))
                return false;
            return true;
        }
        ...
        public static Point ArcBezierDeg(
            Path path,
            Stretcher stretcher,
            Point c,
            double rx,
        ...
        )
        {
            ...
            if (ccw)
            {
                ...
                while (a1 < a2)
                {
                    ...
                    path.CurveTo(
                        stretcher.Transform(s1),
                        stretcher.Transform(s2),
                        stretcher.Transform(p2)
                    );
                    a1 = aa;
                }
            }
