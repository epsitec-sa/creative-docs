...
namespace Epsitec.Common.Document.Menus
{
    public class LeadingMenu : System.IComparable
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    this.text = string.Format(
                        "{0} {1}",
                        document.Modifier.RealToString(this.value),
                        document.Modifier.ShortNameUnitDimension
                    );
                }
            }
        }
        ...
        public int CompareTo(object obj)
        {
            ...
            if (double.IsNaN(this.value))
                return -1; // "Aucun" toujours au début
            if (double.IsNaN(that.value))
                return 1;

            if (this.units != that.units)
            {
                if (this.units == "%")
                    return -1; // les % toujours au début
                if (that.units == "%")
                    return 1;
            }

            if (System.Math.Abs(this.value - that.value) < 0.00001)
                return 0;
        ...
        public static VMenu CreateLeadingMenu(
            Document document,
            double currentValue,
            string currentUnits,
            bool isDefault,
