...
namespace Epsitec.Common.Document.Widgets
{
    ...
    public class MiniatureFrame : FrameBox, Common.Widgets.Behaviors.IDragBehaviorHost
    {
        ...
        public bool IsLeftRightPlacement
        {
            ...
            get { return this.isLeftRightPlacement; }
            set { this.isLeftRightPlacement = value; }
        }

        public bool IsBefore
        {
            //	Si le widget a été utilisé comme destination d'un drag & drop, indique la moitié visée.
            get { return this.isBefore; }
        }

        public bool DragSourceFrame
        {
            get { return (bool)this.GetValue(MiniatureFrame.DragSourceFrameProperty); }
            set { this.SetValue(MiniatureFrame.DragSourceFrameProperty, value); }
        }

        public bool DragSourceEnable
        {
            get { return (bool)this.GetValue(MiniatureFrame.DragSourceEnableProperty); }
            set
            {
                if (value)
                {
        ...
        {
            get { return this.GetValue(MiniatureFrame.DragHostProperty) as MiniatureFrame; }
            set
            {
                if (value == null)
                {
        ...
        private PartialHilite TargetHilite
        {
            ...
            set
            {
                if (this.targetHilite != value)
                {
                    ...
                    {
                        this.isBefore = (
                            this.targetHilite == PartialHilite.Left
                            || this.targetHilite == PartialHilite.Bottom
                        );
                    }
        ...
        private MiniatureFrame FindDropTarget(Point mouse)
        {
            ...
            Widget widget = this.Window.Root.FindChild(
                this.MapClientToRoot(mouse),
        ...
        private class DragInfo
        {
            public DragInfo(Point cursor, MiniatureFrame widget, Size size)
            {
                ...
                this.target = null;
                this.origin = widget.DragHost.MapClientToScreen(
                    new Point(
                        -System.Math.Floor(size.Width / 2),
                        -System.Math.Floor(size.Height / 2)
                    )
                );
        ...
        {
            add { this.AddUserEventHandler("DragAndDropDoing", value); }
            remove { this.RemoveUserEventHandler("DragAndDropDoing", value); }
        }

        private void OnDragAndDropDoing(MiniatureFrame dst)
        {
            EventHandler<MiniatureFrameEventArgs> handler =
                this.GetUserEventHandler<MiniatureFrameEventArgs>("DragAndDropDoing");
        ...
        public static readonly DependencyProperty DragHostProperty = DependencyProperty.Register(
            "DragHost",
            typeof(MiniatureFrame),
            typeof(MiniatureFrame),
            new DependencyPropertyMetadata().MakeNotSerializable()
        );
        public static readonly DependencyProperty DragSourceFrameProperty =
            DependencyProperty.Register(
                "DragSourceFrame",
                typeof(bool),
                typeof(MiniatureFrame),
                new Common.Widgets.Helpers.VisualPropertyMetadata(
                    false,
                    Common.Widgets.Helpers.VisualPropertyMetadataOptions.AffectsDisplay
                )
            );
