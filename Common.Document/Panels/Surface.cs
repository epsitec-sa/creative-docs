...
namespace Epsitec.Common.Document.Panels
{
    ...
    {
        public Surface(Document document)
            : base(document)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                this.fieldFactor[i] = new Widgets.TextFieldLabel(
                    this,
                    Widgets.TextFieldLabel.Type.TextFieldReal
                );
                this.fieldFactor[i].LabelShortText = string.Format(
                    Res.Strings.Panel.Surface.Short.Factor,
                    i + 1
                );
            ...
            for (int i = 0; i < 2; i++)
            {
                this.fieldScalar[i] = new Widgets.TextFieldLabel(
                    this,
                    Widgets.TextFieldLabel.Type.TextFieldReal
                );
                this.fieldScalar[i].LabelShortText = string.Format(
                    Res.Strings.Panel.Surface.Short.Scalar,
                    i + 1
                );
        ...
        protected void AddRadioIcon(Properties.SurfaceType type, bool endOfLine)
        {
            this.grid.AddRadioIcon(
                Misc.Icon(Properties.Surface.GetIconText(type)),
                Properties.Surface.GetName(type),
                (int)type,
                endOfLine
            );
        }
        ...
        public override double DefaultHeight
        {
            ...
            get { return (this.isExtendedSize ? this.LabelHeight + 124 : this.LabelHeight + 30); }
        }
