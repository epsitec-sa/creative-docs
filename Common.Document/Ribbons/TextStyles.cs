...
namespace Epsitec.Common.Document.Ribbons
{
    ...
    {
        public TextStyles(DocumentType type, InstallType install, DebugMode debugMode)
            : base(type, install, debugMode)
        {
            ...
            ToolTip.Default.SetToolTip(
                ...
                Res.Strings.Panel.Style.ParagraphDefinition
            );
            ...
            ToolTip.Default.SetToolTip(
                ...
                Res.Strings.Panel.Style.CharacterDefinition
            );
        ...
        public override void NotifyTextStylesChanged(System.Collections.ArrayList textStyleList)
        {
            ...
            foreach (Text.TextStyle textStyle in textStyleList)
            {
                ...
                for (int i = 0; i < max; i++)
                {
                    ...
                    if (rank >= this.comboStyle.Items.Count)
                        break;

                    IconButtonsCombo.Item item =
                        this.comboStyle.Items[rank] as IconButtonsCombo.Item;
        ...
        protected void UpdateAfterTextStyleListChanged()
        {
            ...
            Text.TextStyle[] styles = this.document.TextStyles(
                this.characterMode ? StyleCategory.Character : StyleCategory.Paragraph
            );
            string tt = this.characterMode
                ? Res.Strings.Action.Apply.Tooltip.StyleCharacter
                : Res.Strings.Action.Apply.Tooltip.StyleParagraph;
            foreach (Text.TextStyle style in styles)
            {
                ...
                string parameter = string.Concat(
                    style.Name,
                    '\t',
                    this.characterMode ? "Character" : "Paragraph"
                );
        ...
        protected void UpdateSelectedStyle()
        {
            ...
            if (this.document.Wrappers.TextFlow != null)
            {
                ...
                {
                    if (
                        style.TextStyleClass == Common.Text.TextStyleClass.Paragraph
                        && !this.characterMode
                    )
                    {
                        this.ignoreChange = true;
                    ...
                    if (
                        style.TextStyleClass == Common.Text.TextStyleClass.Text
                        && this.characterMode
                    )
                    {
                        this.ignoreChange = true;
        ...
        protected void UpdateMode()
        {
            ...
            this.buttonParagraph.ActiveState = this.characterMode
                ? ActiveState.No
                : ActiveState.Yes;
            this.buttonCharacter.ActiveState = this.characterMode
                ? ActiveState.Yes
                : ActiveState.No;
        }
        ...
        protected int RibbonStyleFirst
        {
            ...
            get
            {
                if (this.characterMode)
                    return this.document.Wrappers.RibbonCharacterStyleFirst;
                else
                    return this.document.Wrappers.RibbonParagraphStyleFirst;
            }
            set
            {
                if (this.characterMode)
                    this.document.Wrappers.RibbonCharacterStyleFirst = value;
                else
                    this.document.Wrappers.RibbonParagraphStyleFirst = value;
            }
        }
        ...
        private void HandleSelectedIndexChanged(object sender)
        {
            ...
            Text.TextStyle style = this.document.TextContext.StyleList.GetTextStyle(
                name,
                this.characterMode
