        ...
        private void WriteObject(string objectName, string ending, string type)
        {
            ...
            if (type == "D") // définition ?
            {
                ...
                System.Diagnostics.Debug.Assert(
                    !obj.Defined,
                    "PDF.Writer: Attempt to redefine a object"
                );
                System.Diagnostics.Debug.Assert(
                    obj.Id == 1 || objectName != "Root",
                    "PDF.Writer: Root objet must have number 1"
                );
                obj.Defined = true;
            }
                ...
                switch (part.Type)
                {
                    ...
                    default:
                        throw new System.NotSupportedException(
                            string.Format("Part type {0} not supported here", part.Type)
                        );
                }
            }
        ...
        public void Finish()
        {
            ...
            this.FileWriteLine(
                string.Format(
                    ...
                    "xref 0 {0}",
                    Writer.ToString(this.dictionary.Count + 1)
                )
            );
            ...
            for (int i = 0; i < this.dictionary.Count; i++)
            {
                ...
                this.FileWriteLine(
                    string.Format(
                        CultureInfo.InvariantCulture,
                        "{0} 00000 n",
                        PDF.Writer.ToStringD10(obj.Offset)
                    )
                );
            }
            this.FileWriteLine(
                string.Format(
                    CultureInfo.InvariantCulture,
                    "trailer << /Size {0} /Root 1 0 R /Info {1} 0 R >>",
                    Writer.ToString(this.dictionary.Count + 1),
                    this.GetObjectId("Info")
                )
            );
