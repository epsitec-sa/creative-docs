...
namespace Epsitec.Common.Document.PDF
{
    ...
    public sealed class Export : System.IDisposable
    {
        ...
        private Writer CreateWriter(string path)
        {
            ...
            this.documentTitle =
                Support.Globals.Properties.GetProperty<string>("PDF:Title")
            ...
            string author = Export.GetEscapedString(
                Support.Globals.Properties.GetProperty<string>("PDF:Author")
            );
            string creator = Export.GetEscapedString(
                Support.Globals.Properties.GetProperty<string>("PDF:Creator")
            );
            string producer = Export.GetEscapedString(
                Support.Globals.Properties.GetProperty<string>("PDF:Producer")
                    ?? string.Format("{0}, {1}", defaultProducer, defaultCopyright)
            );
            string creationDate = Export.GetDateString(
                Support.Globals.Properties.GetProperty<System.DateTime>(
                    "PDF:CreationDate",
                    System.DateTime.Now
                )
            );
        ...
        private void EmitHeaderPages(
            Writer writer,
            PdfExportPageRange pageRange,
            IEnumerable<int> pageList
        )
        {
        ...
        )
        {
            ...
            foreach (int page in pageList)
            {
                this.report.DefineProgress(
                    0,
                    string.Format(Res.Strings.Export.PDF.Progress.Content, page + 1)
                );
                ...
                this.EmitSinglePageContents(
                    writer,
                    port,
                    drawingContext,
                    offset.GetPageOffset(rank),
                    page
                );
            }
        }
        ...
        private void EmitPageObjects(Writer writer, Modifier modifier, IEnumerable<int> pageList)
        {
            ...
            foreach (int page in pageList)
            {
                ...
                switch (
                    Support.Globals.Properties.GetProperty<string>(
                        "PDF:MediaBoxDefinition",
                        "MediaBox"
                    )
                )
                {
        ...
        private void EmitPageObjectResources(Writer writer, IEnumerable<int> pageList)
        {
            ...
            foreach (int page in pageList)
            {
                this.report.DefineProgress(
                    0,
                    string.Format(Res.Strings.Export.PDF.Progress.Ressource, page + 1)
                );
                if (this.report.Canceled)
                {
                    throw new PdfExportException(Res.Strings.Export.PDF.Progress.Canceled);
                ...
                if (tcs > 0 || this.imageSurfaces.Count > 0)
                {
                    ...
                    writer.WriteString(
                        Export.GetComplexSurfaceShortName(0, PdfComplexSurfaceType.ExtGState)
                    );
                    writer.WriteObjectRef(
                        Export.GetComplexSurfaceName(0, PdfComplexSurfaceType.ExtGState)
                    );
                    for (int index = 0; index < tcs; index++)
                    {
                        ...
                        if (
                            cs.Type == Type.TransparencyRegular
                            || cs.Type == Type.TransparencyGradient
                            || cs.IsSmooth
                        )
                        {
                            writer.WriteString(
                                Export.GetComplexSurfaceShortName(
                                    cs.Id,
                                    PdfComplexSurfaceType.ExtGState
                                )
                            );
                            writer.WriteObjectRef(
                                Export.GetComplexSurfaceName(cs.Id, PdfComplexSurfaceType.ExtGState)
                            );
                        }
                        if (cs.Type == Type.TransparencyPattern)
                        {
                            writer.WriteString(
                                Export.GetComplexSurfaceShortName(
                                    cs.Id,
                                    PdfComplexSurfaceType.ExtGStateP1
                                )
                            );
                            writer.WriteObjectRef(
                                Export.GetComplexSurfaceName(
                                    cs.Id,
                                    PdfComplexSurfaceType.ExtGStateP1
                                )
                            );
                            ...
                            writer.WriteString(
                                Export.GetComplexSurfaceShortName(
                                    cs.Id,
                                    PdfComplexSurfaceType.ExtGStateP2
                                )
                            );
                            writer.WriteObjectRef(
                                Export.GetComplexSurfaceName(
                    ...
                    for (int index = 0; index < tcs; index++)
                    {
                        ComplexSurface cs = this.GetComplexSurface(page, index);
                        System.Diagnostics.Debug.Assert(cs != null);
                        if (cs.Type == Type.OpaqueGradient || cs.Type == Type.TransparencyGradient)
                        {
                            writer.WriteString(
                                Export.GetComplexSurfaceShortName(
                                    cs.Id,
                                    PdfComplexSurfaceType.ShadingColor
                                )
                            );
                            writer.WriteObjectRef(
                                Export.GetComplexSurfaceName(
                    ...
                    for (int index = 0; index < tcs; index++)
                    {
                        ComplexSurface cs = this.GetComplexSurface(page, index);
                        System.Diagnostics.Debug.Assert(cs != null);
                        if (cs.Type == Type.OpaquePattern || cs.Type == Type.TransparencyPattern)
                        {
                            writer.WriteString(
                                Export.GetComplexSurfaceShortName(
                                    cs.Id,
                                    PdfComplexSurfaceType.XObject
                                )
                            );
                            writer.WriteObjectRef(
                                Export.GetComplexSurfaceName(cs.Id, PdfComplexSurfaceType.XObject)
                            );
                        }
                    }
                    foreach (ImageSurface image in this.imageSurfaces)
                    {
                        ...
                        writer.WriteObjectRef(
                            Export.GetComplexSurfaceName(image.Id, PdfComplexSurfaceType.XObject)
                        );
                    }
                    writer.WriteString(">> ");
                }
                ...
                if (!info.TextCurve)
                {
                    ...
                    foreach (System.Collections.DictionaryEntry dict in this.fontHash)
                    {
                        ...
                        int totalPages =
                            (font.CharacterCount + Export.charPerFont - 1) / Export.charPerFont;
                        for (int fontPage = 0; fontPage < totalPages; fontPage++)
                        {
                            writer.WriteString(Export.GetFontShortName(font.Id, fontPage));
                            writer.WriteObjectRef(
                                Export.GetFontName(font.Id, fontPage, PdfFontType.Base)
                            );
                        }
                    }
                    writer.WriteString(">> ");
        ...
        private void EmitSinglePageContents(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
        ...
        {
            foreach (
                System.Reflection.AssemblyCopyrightAttribute attribute in typeof(Export).Assembly.GetCustomAttributes(
        ...
        private static string GetDefaultProducerInformation()
        {
            ...
            foreach (
                System.Diagnostics.ProcessModule module in System
        ...
        private static string GetDateString(System.DateTime dateTime)
        {
            ...
            return string.Format(
                CultureInfo.InvariantCulture,
                "{0:D4}{1:D2}{2:D2}{3:D2}{4:D2}{5:D2}Z",
                utc.Year,
                utc.Month,
                utc.Day,
                utc.Hour,
        ...
        private void PreProcessComplexSurfaces(
            Port port,
            DrawingContext drawingContext,
            IEnumerable<int> pageList
        )
                ...
                foreach (Objects.Layer layer in layers)
                {
                    ...
                    foreach (Objects.Abstract obj in this.document.Deep(layer))
                    {
                        ...
                        this.report.DefineProgress(
                            0,
                            string.Format(
                                Res.Strings.Export.PDF.Progress.Surface,
                                page + 1,
                                objIndex++
                            )
                        );
        ...
        private void PreProcessSurfaceOfObjectProperties(
            Port port,
            int page,
            Objects.Layer layer,
            ...
            for (int rank = 0; rank < list.Count; rank++)
            {
                ...
                if (gradient != null)
                {
                    Properties.Line stroke =
                        (gradient.Type == Properties.Type.LineColor) ? obj.PropertyLineMode : null;

                    this.complexSurfaces.Add(
                        new ComplexSurface(page, port, layer, obj, gradient, stroke, rank, id++)
                    );
                }
                ...
                {
                    this.complexSurfaces.Add(
                        new ComplexSurface(page, port, layer, obj, font, null, rank, id++)
                    );
                }
            }
        }

        private void PreProcessSurfaceOfImageObjects(
            Port port,
            DrawingContext drawingContext,
            Objects.Abstract obj,
            ref int id
        ...
        private void PreProcessCharactersOfTextObjects(
            Port port,
            DrawingContext drawingContext,
            Objects.Abstract obj
        )
            ...
            for (int i = 0; i < this.complexSurfaces.Count; i++)
            {
                ...
                switch (cs.Type)
                {
                    case PDF.Type.OpaqueRegular:
                        this.CreateComplexSurfaceTransparencyGradientOrSmooth(
                            writer,
                            port,
                            drawingContext,
                            cs
                    ...
                    case PDF.Type.TransparencyRegular:
                        this.CreateComplexSurfaceTransparencyRegular(
                            writer,
                            port,
                            drawingContext,
                            cs
                    ...
                    case PDF.Type.TransparencyPattern:
                        this.CreateComplexSurfaceTransparencyPattern(
                            writer,
                            port,
                            drawingContext,
                            cs
        ...
        private void CreateComplexSurfaceTransparencyRegular(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        ...
        private void CreateComplexSurfaceTransparencyPattern(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        ...
        private void CreateComplexSurfaceAlpha(
            Writer writer,
            double alpha,
            int id,
            PdfComplexSurfaceType type
        )
        {
        ...
        private void CreateComplexSurfaceTransparencyGradientOrSmooth(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        )
        {
            ...
            if (cs.IsSmooth)
            {
                writer.WriteString("/ExtGState << ");
                writer.WriteString(
                    Export.GetComplexSurfaceShortName(cs.Id, PdfComplexSurfaceType.ExtGStateSmooth)
                );
                writer.WriteObjectRef(
                    Export.GetComplexSurfaceName(cs.Id, PdfComplexSurfaceType.ExtGStateSmooth)
                );
                writer.WriteString(">> ");
            }
            ...
            writer.WriteString("/Shading << ");
            writer.WriteString(
                Export.GetComplexSurfaceShortName(cs.Id, PdfComplexSurfaceType.ShadingGray)
            );
            writer.WriteObjectRef(
                Export.GetComplexSurfaceName(cs.Id, PdfComplexSurfaceType.ShadingGray)
            );
            writer.WriteString(">> ");
            ...
            var pdf = port.GetPDF();
            writer.WriteLine(
                string.Format(CultureInfo.InvariantCulture, "{0} >>", Port.StringLength(pdf.Length))
            );
            writer.WriteLine("stream");
            writer.WriteString(pdf);
            writer.WriteLine("endstream endobj");
            ...
            if (cs.Type == PDF.Type.OpaqueGradient || cs.Type == PDF.Type.TransparencyGradient)
            {
        ...
        private void CreateComplexSurfaceSmooth(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        )
        {
            ...
            writer.WriteString("<< /SMask << /S /Luminosity /G "); // voir [*] page 521
            writer.WriteObjectRef(
                Export.GetComplexSurfaceName(cs.Id, PdfComplexSurfaceType.XObjectSmooth)
            );
            writer.WriteLine(">> >> endobj");
        ...
        private void CreateComplexSurfaceOpaqueGradient(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        ...
        private void MatrixComplexSurfaceGradient(ComplexSurface cs)
        {
            ...
            if (
                gradient.FillType == Properties.GradientFillType.Diamond
        ...
        private void CreateComplexSurfaceGradient(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs,
        ...
        )
        {
            ...
            PdfComplexSurfaceType tcs =
                (nbColors == 0)
                    ? PdfComplexSurfaceType.ShadingGray
                    : PdfComplexSurfaceType.ShadingColor;
            ...
            if (
                gradient.FillType == Properties.GradientFillType.Linear
                ...
                else
                {
                    ...
                    writer.WriteObjectRef(
                        Export.GetFunctionName(
                            cs.Id,
                            (nbColors == 0)
                                ? PdfFunctionType.SampledAlpha
                                : PdfFunctionType.SampledColor
                        )
                    );
                    port.Reset();
                }
            ...
            )
            {
                ...
                double domainMinX = System.Math.Min(
                    System.Math.Min(p1.X, p2.X),
                    System.Math.Min(p3.X, p4.X)
                );
                double domainMaxX = System.Math.Max(
                    System.Math.Max(p1.X, p2.X),
                    System.Math.Max(p3.X, p4.X)
                );
                double domainMinY = System.Math.Min(
                    System.Math.Min(p1.Y, p2.Y),
                    System.Math.Min(p3.Y, p4.Y)
                );
                double domainMaxY = System.Math.Max(
                    System.Math.Max(p1.Y, p2.Y),
                    System.Math.Max(p3.Y, p4.Y)
                );
                ...
                for (int i = 0; i < totalColors; i++)
                {
                    ...
                    writer.WriteLine(
                        string.Format(
                            CultureInfo.InvariantCulture,
                            "<< /FunctionType 4 /Range [0 1] {0} {1} >>",
                            domain,
                            Port.StringLength(fonctions[i].Length)
                        )
                    );
                    writer.WriteLine("stream");
        ...
        private static void FunctionColorSampled(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs,
        ...
        )
        {
            ...
            writer.WriteObjectDef(
                Export.GetFunctionName(
                    cs.Id,
                    (nbColors == 0) ? PdfFunctionType.SampledAlpha : PdfFunctionType.SampledColor
                )
            );
            writer.WriteLine(
                string.Format(
                    CultureInfo.InvariantCulture,
                    "<< /FunctionType 0 /Domain [0 1] /Range [{0}] /Size [256] /BitsPerSample 8 /Filter /ASCIIHexDecode {1} >>",
                    range,
                    Port.StringLength(table.Length)
                )
            );
            writer.WriteLine("stream");
        ...
        private static string FunctionGradientDiamond(
        ...
        )
        {
            ...
            else
            {
                ...
                if (gradient.Repeat > 1)
                {
                    ...
                    builder.Append(
                        " mul dup dup truncate sub exch cvi 2 mod 1 eq { 1 exch sub } if "
                    );
                }
        ...
        private static string FunctionGradientConic(
            Properties.Gradient gradient,
            double colorStart,
            double colorEnd
        )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    builder.Append(
                        " div dup exch dup truncate sub exch cvi 2 mod 1 eq { 1 exch sub } if "
                    );
                }
        ...
        private static void FunctionMiddleAdjust(
        ...
        private static void FunctionColorAdjust(
            System.Text.StringBuilder builder,
            double colorStart,
            double colorEnd
        )
        ...
        private void CreateComplexSurfaceOpaquePattern(
            Writer writer,
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        ...
        private int ComplexSurfaceGradientNbColors(
            Port port,
            DrawingContext drawingContext,
            ComplexSurface cs
        )
        ...
        private void EmitImageSurfaces(Writer writer, Port port)
        {
            ...
            foreach (ImageSurface image in this.imageSurfaces)
            {
                this.report.DefineProgress(
                    0,
                    string.Format(Res.Strings.Export.PDF.Progress.Image, image.Id)
                );
                ...
                if (
                    this.CreateImageSurface(
                        writer,
                        image,
                        PdfComplexSurfaceType.XObject,
                        PdfComplexSurfaceType.XObjectMask
                    )
                )
                {
                    this.CreateImageSurface(
        ...
        private bool CreateImageSurface(
            Writer writer,
            ImageSurface image,
            PdfComplexSurfaceType baseType,
            PdfComplexSurfaceType maskType
        )
        {
            ...
            writer.WriteLine(
                string.Format(
                    CultureInfo.InvariantCulture,
                    " {0} >>",
                    Port.StringLength(image.ImageStream.StreamLength)
                )
            );
            writer.WriteLine("stream");
        ...
        private static NativeBitmap PrepareImage(ImageSurface image)
        {
            System.Diagnostics.Debug.WriteLine(
                string.Format(
                    "PrepareImage, image:\n{0}",
                    image == null ? "<null>" : image.GetDebugInformation()
                )
            );
            ...
            System.Diagnostics.Debug.WriteLine(
                string.Format(
                    "PrepareImage, before crop and resizing:\n{0}",
                    fi == null ? "<null>" : fi.ToString()
                )
            );
            fi = Export.CropImage(image, fi);
            System.Diagnostics.Debug.WriteLine(
                string.Format(
                    "PrepareImage, After crop:\n{0}",
                    fi == null ? "<null>" : fi.ToString()
                )
            );
            ...
            System.Diagnostics.Debug.WriteLine(
                string.Format(
                    "PrepareImage: after crop & resize, Size={0}x{1}\n{2}",
                    dx,
                    dy,
                    fi == null ? "<null>" : fi.ToString()
                )
        ...
        private ImageSurface ProcessImageAndCreatePdfStream(
            ImageSurface image,
            PdfComplexSurfaceType baseType,
        ...
        )
        {
            ...
            System.Diagnostics.Debug.WriteLine(
                string.Format(
                    "ProcessImageAndCreatePdfStream> SurfaceType={0}, ImageCompression={1}, ColorConversion={2}, Quality={3}",
                    baseType,
                    compression,
        ...
        private static ImageSurface LaunchProcessImageAndCreatePdfStream(ImageSurface image)
        {
            ...
            try
            {
                System.IO.File.WriteAllText(
                    path,
                    ImageSurface.Serialize(image),
                    System.Text.Encoding.UTF8
                );

                string program = "Common.Document.ExportEngine.exe";

                string exe1 = System.IO.Path.Combine(
                    Epsitec.Common.Support.Globals.Directories.Executable,
                    program
                );
                string exe2 = System.IO.Path.Combine(
                    Epsitec.Common.Support.Globals.Directories.InitialDirectory,
                    program
                );
                string exe3 = System.IO.Path.Combine(
                    System.IO.Path.GetDirectoryName(typeof(Export).Assembly.Location),
                    program
                );

                string exe = System.IO.File.Exists(exe1)
                    ? exe1
                    : System.IO.File.Exists(exe2)
                        ? exe2
                        : exe3;
                ...
                return ImageSurface.Deserialize(
        ...
        private static void InternalProcessImageAndCreatePdfStream(ImageSurface image)
        {
            ...
            using (NativeBitmap fi = Export.PrepareImage(image))
            {
                ...
                {
                    image.ImageStream = Export.EmitJpegImageSurface(
                        baseType,
                        fi,
                        colorConversion,
                        jpegQuality
                ...
                {
                    image.ImageStream = Export.EmitLosslessImageSurface(
                        baseType,
                        image,
                        compression,
                        fi,
        ...
        private static void EmitImageHeader(
            Writer writer,
            PdfComplexSurfaceType baseType,
            ImageSurface image,
            ImageCompression compression,
        ...
        )
        {
            ...
            System.Diagnostics.Debug.WriteLine(
                string.Format("EmitHeader: {0} XML={1}", baseType, image.GetDebugInformation())
            );
                    ...
                    switch (image.ColorType)
                    {
                        ...
                        default:
                            throw new System.InvalidOperationException(
                                string.Format("ColorType.{0} not recognized", image.ColorType)
                            );
                    }
                }
            }
        ...
        private static PdfImageStream EmitLosslessImageSurface(
            PdfComplexSurfaceType baseType,
            ImageSurface image,
            ImageCompression compression,
            NativeBitmap fi,
        ...
        )
        {
            ...
            if (bpp == 1)
            {
                data = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Grayscale,
                    image.Filter
                );
            }
            ...
            if (bpp == 3)
            {
                byte[] bufferRed = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Red,
                    image.Filter
                );
                byte[] bufferGreen = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Green,
                    image.Filter
                );
                byte[] bufferBlue = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Blue,
                    image.Filter
                );
            ...
            if (bpp == 4)
            {
                byte[] bufferCyan = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Cyan,
                    image.Filter
                );
                byte[] bufferMagenta = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Magenta,
                    image.Filter
                );
                byte[] bufferYellow = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Yellow,
                    image.Filter
                );
                byte[] bufferBlack = Export.CreateImageSurfaceChannel(
                    fi,
                    BitmapColorChannel.Black,
                    image.Filter
                );
        ...
        private static PdfImageStream EmitJpegImageSurface(
            PdfComplexSurfaceType baseType,
            NativeBitmap fi,
            ColorConversion colorConversion,
            double jpegQuality
        )
        {
            ...
            if (
                (colorConversion == PDF.ColorConversion.ToGray)
        ...
        /// </summary>
        private static NativeBitmap ResizeImage(
            ImageSurface image,
            NativeBitmap fi,
            double imageMinDpi,
            double imageMaxDpi,
        ...
        )
        {
            ...
            //  Modified Code:
            System.Diagnostics.Debug.WriteLine(
                "Export.ResizeImage: USING TEMPORARY WORKAROUND !!!!!"
            );
            ...
            if (usedCurrentDpi != usedFinalDpi)
            {
                ...
                resizeRequired = true;
                System.Diagnostics.Debug.WriteLine(
                    "Dpi are different, resize is required:, now, dx = {0}, dy = {1}",
                    dx,
                    dy
                );
            }
        ...
        private static byte[] CreateImageSurfaceChannel(
            NativeBitmap fi,
            BitmapColorChannel channel,
            ImageFilter filter
        )
        {
        ...
        private void CreateFontToUnicode(Writer writer, FontList font, int fontPage, int count)
        {
            ...
            builder.Append(
                "/CIDSystemInfo << /Registry (Epsitec) /Ordering (Identity-H) /Supplement 0 >> def "
            );
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                builder.Append(
                    string.Format(CultureInfo.InvariantCulture, "<{0}> <{0}> <{1}> ", hex1, hex2)
                );
            }
            ...
            writer.WriteLine(
                string.Format(
                    CultureInfo.InvariantCulture,
                    "<< {0} >>",
                    Port.StringLength(builder.Length)
                )
            );
            writer.WriteLine("stream");
        ...
        private void CreateFontCharacter(
            Writer writer,
            FontList font,
            int fontPage,
            CharacterList cl
