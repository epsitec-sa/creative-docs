...
namespace Epsitec.Common.Document.PDF
{
    ...
    public class Port : IPaintPort
    {
        ...
        public Port(
            IEnumerable<ComplexSurface> complexSurfaceList,
        ...
        )
        {
            ...
            this.SetTransform(this.transform);
            this.PutCommand(
                Export.GetComplexSurfaceShortName(image.Id, PdfComplexSurfaceType.XObject)
            );
        ...
        protected void SetStrokeColor(RichColor color)
        {
            ...
            )
            {
                ...
                if (this.complexSurfaceId != -1)
                {
                    ...
                    if (cs.Type == Type.TransparencyRegular || cs.Type == Type.TransparencyGradient)
                    {
                        this.PutCommand(
                            Export.GetComplexSurfaceShortName(
                                this.complexSurfaceId,
                                this.complexType
                            )
                        );
                        this.PutCommand("gs "); // graphic state, voir [*] page 189
        ...
        protected void DoFill(Path path)
        {
            ...
            else
            {
                ...
                else if (cs.Type == Type.OpaquePattern || cs.Type == Type.TransparencyPattern)
                {
                    this.PutPath(path);
                    this.PutCommand("q "); // save, voir [*] page 189
                    this.PutCommandStroke();
        ...
        protected void PutStrokingColorSpace(RichColor color)
        {
            ...
            if (this.colorForce == ColorForce.Nothing)
                return;
            if (this.colorForce != ColorForce.Default)
                return;

            if (color.ColorSpace == ColorSpace.Rgb)
                this.PutCommand("/DeviceRGB CS ");
            if (color.ColorSpace == ColorSpace.Cmyk)
                this.PutCommand("/DeviceCMYK CS ");
            if (color.ColorSpace == ColorSpace.Gray)
                this.PutCommand("/DeviceGray CS ");
        }
        ...
        protected void PutStrokingColor(RichColor color)
        {
            ...
            if (this.colorForce == ColorForce.Default)
                this.PutCommand("SC ");
            if (this.colorForce == ColorForce.Rgb)
                this.PutCommand("RG ");
            if (this.colorForce == ColorForce.Cmyk)
                this.PutCommand("K ");
            if (this.colorForce == ColorForce.Gray)
                this.PutCommand("G ");
        }
        ...
        public static string StringBBox(string cmd, Rectangle bbox)
        {
            ...
            return string.Format(
                ...
                "{0} [{1} {2} {3} {4}] ",
                cmd,
                Port.StringValue(bbox.Left),
                Port.StringValue(bbox.Bottom),
                Port.StringValue(bbox.Right),
                Port.StringValue(bbox.Top)
            );
        }
        ...
        public int SearchComplexSurface(Objects.Abstract obj, Properties.Abstract property)
        {
            ...
            if (this.complexSurfaceList == null)
                return -1;
        ...
        public ImageSurface SearchImageSurface(
            string filename,
            Size size,
            Margins crop,
            ImageFilter filter
        )
        {
            ...
            {
                this.lastImageSurface = ImageSurface.Search(
                    this.imageSurfaceList,
                    filename,
                    size,
                    crop,
                    filter
