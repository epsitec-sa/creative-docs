...
namespace Epsitec.Common.Document
{
    ...
    public class Misc
    {
        ...
        public static void AutoFocus(Widget widget, bool state)
        {
        ...
        public static void ConvertStringToDouble(
            out double value,
            out Text.Properties.SizeUnits units,
            string text,
            double min,
            double max,
            double defaultValue
        ...
        public static double ConvertStringToDouble(
            string text,
            double min,
            double max,
            double defaultValue
        ...
        public static string ConvertDoubleToString(double value, double fracDigits)
        {
            //	Conversion d'un nombre réel en chaîne.
            if (fracDigits > 0)
            {
        ...
        public static Path GetHatchPath(Rectangle rect, double distance, Point reference)
        {
        ...
        public static string GetColorNiceName(RichColor color)
        {
            ...
            else if (color.R == color.G && color.R == color.B) // gris ?
            {
                ...
                if (br == 0.0)
                    builder.Append(Res.Strings.Color.Black);
                else if (br == 1.0)
                    builder.Append(Res.Strings.Color.White);
                else if (br < 0.3)
                    builder.Append(Res.Strings.Color.DarkGray);
                else if (br > 0.7)
                    builder.Append(Res.Strings.Color.LightGray);
                else
                    builder.Append(Res.Strings.Color.Gray);
            }
            else
            {
                ...
                if (h < 30 + 60 * 0)
                    builder.Append(Res.Strings.Color.Red);
                else if (h < 30 + 60 * 1)
                    builder.Append(Res.Strings.Color.Yellow);
                else if (h < 30 + 60 * 2)
                    builder.Append(Res.Strings.Color.Green);
                else if (h < 30 + 60 * 3)
                    builder.Append(Res.Strings.Color.Cyan);
                else if (h < 30 + 60 * 4)
                    builder.Append(Res.Strings.Color.Blue);
                else if (h < 30 + 60 * 5)
                    builder.Append(Res.Strings.Color.Magenta);
                else
                    builder.Append(Res.Strings.Color.Red);
            }
        ...
        public static string FaceInvariantToInvariant(string face, string style)
        {
        ...
        public static string FaceInvariantToLocale(string face, string style)
        {
        ...
        public static string StyleInvariantToLocale(string face, string style)
        {
        ...
        public static System.Collections.ArrayList GetFontList(bool enableSymbols)
        {
        ...
        public static bool IsExistingFont(OpenType.FontName fontName)
        {
        ...
        public static string DefaultFontStyle(string face)
        {
        ...
        public static OpenType.FontIdentity DefaultFontIdentityStyle(string face)
        {
            //	Cherche le FontStyle par défaut pour un FontFace donné.
            ...
            {
                if (id.InvariantStyleName == "Regular" || id.InvariantStyleName == "Normal")
                {
                    return id;
                }
            }
        ...
        public static bool IsExistingFontStyle(string face, string style)
        {
            ...
            foreach (OpenType.FontIdentity id in list)
            {
                if (id.InvariantStyleName == style)
                    return true;
            }
        ...
        public static string GetUnicodeName(int code, string fontFace, string fontStyle)
        {
        ...
        public static string GetUnicodeName(int code)
        {
            ...
            if (!minus) // aucune minuscule dans le texte ?
            {
                ...
                text = string.Format(
                    ...
                    text.Substring(1, text.Length - 1).ToLower()
                );
            }
        ...
        public static string GetFeatureText(string feature)
        {
        ...
        public static bool IsInsideList(string[] list, string text)
        {
        ...
        public static int IndexOf(string text, string value, int startIndex, StringSearch mode)
        {
            int count;
            ...
            if ((mode & StringSearch.WholeWord) != 0) // mot entier ?
            {
                if ((mode & StringSearch.EndToStart) != 0) // en arrière ?
                {
                    ...
                    while (true)
                    {
                        ...
                        if (startIndex == -1)
                            return -1;
                        if (Misc.IsWholeWord(text, startIndex, value.Length))
                            return startIndex;
                        startIndex--;
        ...
        protected static bool IsWholeWord(string text, int index, int count)
        {
        ...
        public static string Resume(string text, int max)
        {
        ...
        public static string FontSize(string text, double size)
        {
            //	Retourne le texte dans une autre taille.
            return string.Format(
                "<font size=\"{1}%\">{0}</font>",
                text,
                (size * 100).ToString(System.Globalization.CultureInfo.InvariantCulture)
            );
        }

        public static string Image(string icon)
        {
        ...
        public static string ImageDyn(string name, string parameter)
        {
        ...
        public static string ImageDyn(string name, string parameter, double verticalOffset)
        {
            //	Retourne le texte pour mettre une image dynamique dans un texte.
            return string.Format(
                @"<img src=""{0}"" voff=""{1}""/>",
                Misc.IconDyn(name, parameter),
                verticalOffset.ToString(System.Globalization.CultureInfo.InvariantCulture)
            );
        }

        public static Size IconPreferredSize(string iconSize)
        {
        ...
        public static string Icon(string icon)
        {
            ...
            return string.Format("manifest:Common.DocumentEditor/Images/{0}.icon", icon);
        }

        public static string IconDyn(string name, string parameter)
        {
            //	Retourne le nom complet d'une icône dynamique.
            return string.Format("dyn:{0}/{1}", name, parameter);
        }

        public static string GetShortcut(Command command)
        {
        ...
        public static Command CreateStructuredCommandWithName(string commandName)
        {
            return null;
        }

        public static string FullName(string filename, bool dirtySerialize)
        {
        ...
        public static string ExtractName(string filename, bool dirtySerialize)
        {
        ...
        public static string ExtractName(string filename)
        {
            ...
            return TextLayout.ConvertToTaggedText(
        ...
        public static string CopyName(string name, string copy, string of)
        {
            //	Retourne la copie d'un nom.
            //	copy = "Copie" ou "Copy"
            //	of = "de" ou "of"
            ...
            if (name.StartsWith(string.Concat(copy, " ", of, " ")))
            {
                return string.Concat(
                    ...
                    of,
                    " ",
                    name.Substring(copy.Length + of.Length + 2)
                );
            }
        ...
        public static void Swap(ref Size a, ref Size b)
        {
            Size t = a;
            a = b;
            b = t;
