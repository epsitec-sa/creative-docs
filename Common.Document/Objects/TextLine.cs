...
namespace Epsitec.Common.Document.Objects
{
    ...
    public class TextLine : Objects.Abstract, Support.IXMLSerializable<TextLine>
    {
        ...
        public override DetectEditType DetectEdit(Point pos)
        {
            ...
            DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
            Shape[] shapes = this.ShapesBuild(null, context, false);
            if (context.Drawer.DetectOutline(pos, context, shapes) != -1)
                return DetectEditType.Body;

            if (this.DetectTextCurve(pos))
                return DetectEditType.Body;
            return DetectEditType.Out;
        }
        ...
        protected void AdaptPrimaryLine(int rankPrimary, int rankSecondary, out int rankExtremity)
        {
            ...
            if (rankExtremity < 0)
                rankExtremity = this.handles.Count - 2;
            if (rankExtremity >= this.handles.Count)
                rankExtremity = 1;
        ...
        )
        {
            ...
            if (this.IsHilite && drawingContext != null && drawingContext.IsActive && !this.edited)
            {
                pathHilite = pathBbox;
            }
            ...
            while (true)
            {
                ...
                else // courbe ?
                {
                    ...
                    while (true)
                    {
                        ...
                        if (t2 >= 1.0)
                            break; // fin de cette portion de courbe ?
                        t1 = t2;
                        l1 = l2;
                        next1 = next2;
                    }
        ...
        protected bool AdvanceInit()
        {
            ...
            for (int i = 0; i < this.advanceCharArray.Length; i++)
            {
                ...
                width +=
                    this.AdvanceString(this.advanceCharArray[i].Font, s)
                    * this.advanceCharArray[i].FontSize;
                width += justif.Add * this.advanceCharArray[i].FontSize;

                this.advanceMaxAscender = System.Math.Max(
                    this.advanceMaxAscender,
                    this.advanceCharArray[i].Font.Ascender * this.advanceCharArray[i].FontSize
                );
            }
        ...
        )
        {
            ...
            if (i >= this.advanceCharArray.Length)
                return false;
            ...
            if (justif.Offset > 0.0)
            {
                pos = Point.Move(
                    this.advanceP1,
                    this.advanceP2,
                    this.advanceMaxAscender * justif.Offset
                );
        ...
        protected int DetectTextCurveRank(Point mouse)
        {
            ...
            )
            {
                ...
                inside.AddLine(ptl, pbl);
                if (inside.IsInside())
                    return this.advanceRank - 1; // dans la moitié gauche ?
                ...
                if (inside.IsInside())
                    return this.advanceRank; // dans la moitié droite ?
            }
            return -1;
        }
        ...
        public override void DrawText(IPaintPort port, DrawingContext drawingContext)
        {
            ...
            )
            {
                ...
                if (port is Graphics && this.edited && rank == this.textNavigator.Context.CursorTo)
                {
                    Graphics graphics = port as Graphics;
