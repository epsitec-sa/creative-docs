...
namespace Epsitec.Common.Document.Objects
{
    ...
    public class Dimension : Objects.Abstract
    {
        ...
        public Dimension(Document document, Objects.Abstract model, bool floating)
            : base(document, model)
        {
        ...
        protected override bool ExistingProperty(Properties.Type type)
        {
            ...
            if (type == Properties.Type.LineMode)
                return true;
            if (type == Properties.Type.LineDimension)
                return true;
            if (type == Properties.Type.LineColor)
                return true;
            if (type == Properties.Type.DimensionArrow)
                return true;
            if (type == Properties.Type.Dimension)
                return true;
            ...
            if (rank < this.handles.Count) // poignée de l'objet ?
            {
                ...
                if (handle.PropertyType == Properties.Type.None)
                {
                    ...
                    else if (rank == 4) // milieu ?
                    {
                        ...
                        {
                            drawingContext.ConstrainAddLine(
                                this.Handle(4).Position,
                                this.Handle(4).Position
                                    + (this.Handle(0).Position - this.Handle(2).Position),
                                false,
                                -1
                            );
                        }
        ...
        public override void MoveHandleProcess(int rank, Point pos, DrawingContext drawingContext)
        {
            ...
            if (rank == 0) // extrémité gauche ?
            {
                if (this.IsRight)
                {
                    double ai = Point.ComputeAngleDeg(
                        this.Handle(0).Position,
                        this.Handle(1).Position
                    );
                    double a2 = Point.ComputeAngleDeg(
                        this.Handle(0).Position,
                        this.Handle(2).Position
                    );
                    double a3 = Point.ComputeAngleDeg(
                        this.Handle(1).Position,
                    ...
                    double af = Point.ComputeAngleDeg(
                        this.Handle(0).Position,
                        this.Handle(1).Position
                    );
                    this.Handle(2).Position = Transform.RotatePointDeg(
                        this.Handle(0).Position,
                        a2 + (af - ai),
                        this.Handle(0).Position + new Point(l2, 0)
                    );
                    this.Handle(3).Position = Transform.RotatePointDeg(
                        this.Handle(1).Position,
                        a3 + (af - ai),
                        this.Handle(1).Position + new Point(l3, 0)
                    );
                    this.Handle(4).Position = Point.Scale(
                        this.Handle(0).Position,
                        this.Handle(1).Position,
                        0.5
                    );
            ...
            if (rank == 2) // support gauche ?
            {
                ...
                {
                    this.Handle(2).Position = pos;
                    double angle = Point.ComputeAngleDeg(
                        this.Handle(0).Position,
                        this.Handle(2).Position
                    );
        ...
        )
        {
            ...
            Point pp1 = this.PropertyDimensionArrow.PathExtremity(
                pathStart,
                0,
                w,
                cap,
            ...
            Point pp2 = this.PropertyDimensionArrow.PathExtremity(
                pathEnd,
                1,
                w,
                cap,
            ...
            if (cf == Properties.DimensionForm.Auto)
            {
                cf =
                    (textWidth < length)
                        ? Properties.DimensionForm.Inside
                        : Properties.DimensionForm.Outside;
            }
            ...
            else // ->|-12-|<- ?
            {
                pathStart = new Path();
                pathEnd = new Path();
                pp1 = this.PropertyDimensionArrow.PathExtremity(
                    pathStart,
                    0,
                    w,
                    cap,
                ...
                pp2 = this.PropertyDimensionArrow.PathExtremity(
                    pathEnd,
                    1,
                    w,
                    cap,
                ...
                if (justif == 1)
                {
                    ...
                    if (dimension.FontOffset > 0.0)
                    {
                        pathLine.MoveTo(pp1);
                        pathLine.LineTo(
                            Point.Move(p2, p1, -(lex1 + dimension.OutLength + textWidth))
                        );
                    }
                    else
                    {
                ...
                if (justif == -1)
                {
                    ...
                    if (dimension.FontOffset > 0.0)
                    {
                        pathLine.MoveTo(pp2);
                        pathLine.LineTo(
                            Point.Move(p1, p2, -(lex2 + dimension.OutLength + textWidth))
                        );
                    }
                    else
                    {
