...
namespace Epsitec.Common.Document.Objects
{
    ...
    public class Free : Objects.Abstract
    {
        ...
        public Free(Document document, Objects.Abstract model, bool floating)
            : base(document, model)
        {
        ...
        protected override bool ExistingProperty(Properties.Type type)
        {
            ...
            if (type == Properties.Type.PolyClose)
                return true;
            if (type == Properties.Type.Tension)
                return true;
            ...
            if (family == "Free")
            {
                if (this.IsShaperHandleSelected())
                {
                    ...
                    for (int i = 0; i < total; i++)
                    {
                        ...
                        if (
                            this.IsCyclingHandleSharp(i, true)
                            && this.IsCyclingHandleSharp(i, false)
                        )
                        {
            ...
            if (family == "Segment")
            {
                if (this.selectedSegments != null && this.selectedSegments.Count != 0)
                {
                    ...
                    for (int i = 0; i < this.selectedSegments.Count; i++)
                    {
                        ...
                        if (
                            this.IsCyclingHandleSharp(rank, false)
                            && this.IsCyclingHandleSharp(rank + 1, true)
                        )
                        {
        ...
        public override void CreateMouseMove(Point pos, DrawingContext drawingContext)
        {
            ...
            {
                double len = Point.Distance(
                    this.Handle(total - 1).Position,
                    this.Handle(total - 2).Position
                );
        ...
        public override void CreateProcessMessage(Common.Widgets.Message message, Point pos)
        {
            ...
            if (message.MessageType == Common.Widgets.MessageType.KeyDown)
            {
                ...
                if (
                    message.KeyCode == Epsitec.Common.Widgets.KeyCode.ArrowUp
                ...
                if (
                    message.KeyCode == Epsitec.Common.Widgets.KeyCode.ArrowDown
                ...
                if (
                    message.KeyCode == Epsitec.Common.Widgets.KeyCode.NumericMultiply
        ...
        )
        {
            ...
            if (this.isCreating && total >= 2)
            {
                ...
                pathBox.AppendRectangle(
                    center - new Point(this.spacing, this.spacing),
                    new Size(this.spacing * 2, this.spacing * 2)
                );
        ...
        )
        {
            ...
            this.vp1 = this.PropertyArrow.PathExtremity(
                pathStart,
                0,
                w,
                cap,
            ...
            this.vp2 = this.PropertyArrow.PathExtremity(
                pathEnd,
                1,
                w,
                cap,
            ...
            else
            {
                ...
                for (int i = 0; i < total; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (
                            (i == 1 && !this.PropertyPolyClose.BoolValue)
                            || this.IsCyclingHandleSharp(i - 1, false)
                        )
                        {
                        ...
                        {
                            s1 = this.ComputeSecondary(
                                this.GetCyclingHandlePosition(i),
                                this.GetCyclingHandlePosition(i - 1),
                                this.GetCyclingHandlePosition(i - 2),
                                tension
                            );
                        }

                        if (
                            (i == total - 1 && !this.PropertyPolyClose.BoolValue)
                            || this.IsCyclingHandleSharp(i, true)
                        )
                        {
        ...
        private void SetCyclingHandleSharp(int rank, bool before, bool sharp)
        {
            ...
            if (before)
            {
                if (sharp)
                {
                    this.Handle(rank).ConstrainType = this.IsCyclingHandleSharp(rank, !before)
                        ? HandleConstrainType.SharpSharp
                        : HandleConstrainType.SharpRound;
                }
                else
                {
                    this.Handle(rank).ConstrainType = this.IsCyclingHandleSharp(rank, !before)
                        ? HandleConstrainType.RoundSharp
                        : HandleConstrainType.Symmetric;
                }
            }
            else
            {
        ...
        protected Free(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
