...
namespace Epsitec.Common.Document.Objects
{
    ...
    public class TextBox2 : Objects.AbstractText, Text.ITextRenderer
    {
        ...
        protected override void UpdateTextFrame()
        {
            ...
            if (frame.Width != width || frame.Height != height || frame.OriginY != p4.Y)
            {
                frame.OriginY = p4.Y;
                frame.Width = width;
                frame.Height = height;
        ...
        )
        {
            ...
            {
                Point pbl = this.transform.TransformInverse(
                    this.drawingContext.Viewer.RedrawArea.BottomLeft
                );
                Point pbr = this.transform.TransformInverse(
                    this.drawingContext.Viewer.RedrawArea.BottomRight
                );
                Point ptl = this.transform.TransformInverse(
                    this.drawingContext.Viewer.RedrawArea.TopLeft
                );
                Point ptr = this.transform.TransformInverse(
                    this.drawingContext.Viewer.RedrawArea.TopRight
                );
        ...
        )
        {
            ...
            else
            {
                throw new System.InvalidOperationException(
                    string.Format("InternalOperation.{0} not handled", this.internalOperation)
                );
            }
        }

        private void RenderTextBoundingBox(
            Epsitec.Common.OpenType.Font font,
            double size,
            ushort[] glyphs,
            double[] x,
        ...
        )
        {
            ...
            if (drawingFont != null)
            {
                ...
                if (!mergedBounds.IsSurfaceZero)
                {
                    ...
                    {
                        this.mergingBoundingBox.MergeWith(
                            this.transform.ScaleOrTranslateDirect(mergedBounds)
                        );
                    }
                    else
                    {
                        this.mergingBoundingBox.MergeWith(
                            this.transform.TransformDirect(mergedBounds.BottomLeft)
                        );
                        this.mergingBoundingBox.MergeWith(
                            this.transform.TransformDirect(mergedBounds.BottomRight)
                        );
                        this.mergingBoundingBox.MergeWith(
                            this.transform.TransformDirect(mergedBounds.TopLeft)
                        );
                        this.mergingBoundingBox.MergeWith(
                            this.transform.TransformDirect(mergedBounds.TopRight)
                        );
                    }
                }
            }
        ...
        private void RenderTextPainting(
            Epsitec.Common.OpenType.Font font,
            double size,
            ushort[] glyphs,
            SpaceType[] spaces,
        ...
        private void RenderTextPaintSpecialCharacters(
            Epsitec.Common.OpenType.Font font,
            double size,
            ushort[] glyphs,
            SpaceType[] spaces,
        ...
        )
        {
            for (int i = 0; i < glyphs.Length; i++)
            {
                ...
                if (spaces[i] == SpaceType.NewFrame || spaces[i] == SpaceType.NewPage) // saut ?
                {
                    Text.SimpleTextFrame frame = this.textFrame as Text.SimpleTextFrame;
