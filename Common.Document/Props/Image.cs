...
namespace Epsitec.Common.Document.Properties
{
    ...
    public class Image : Abstract
    {
        ...
        public Image(Document document, Type type)
            : base(document, type) { }
        ...
        public static bool DisablePopupInterface { get; set; }
        ...
        public bool InsideDoc
        {
            ...
            get { return this.insideDoc; }
            set { this.insideDoc = value; }
        }

        public bool FromClipboard
        {
            //	Indique si les données de l'images proviennent du clipboard.
            get { return this.fromClipboard; }
            set { this.fromClipboard = value; }
        }
        ...
        public int FilterCategory
        {
            ...
            get { return this.filterCategory; }
            set
            {
        ...
        public override string SampleText
        {
            ...
            get { return Misc.ExtractName(this.filename); }
        }
        ...
        public override bool IsHandleVisible(Objects.Abstract obj, int rank)
        {
            ...
            {
                double sum =
                    this.cropMargins.Left
                    + this.cropMargins.Right
                    + this.cropMargins.Bottom
                    + this.cropMargins.Top;
            ...
            if (obj.IsSelected && !obj.IsGlobalSelected && !Image.DisablePopupInterface) // objet sélectionné ?
            {
                ...
                if (viewer.Window != null)
                {
                    if (obj.PopupInterfaceFrame == null)
                    {
                        ...
                        viewer.OpenPopupInterface(
                            obj.PopupInterfaceFrame,
                            obj,
                            this.GetPopupInterfacePosition
                        );
        ...
        private void UpdatePopupInterface(Objects.Abstract obj)
        {
            ...
            if (this.cropLogic.IsFillMode)
            {
                ...
                ToolTip.Default.SetToolTip(
                    this.popupInterfaceFillModeButton,
                    Res.Strings.Action.ImageShowAll
                );
            }
            else
            {
                ...
                ToolTip.Default.SetToolTip(
                    this.popupInterfaceFillModeButton,
                    Res.Strings.Action.ImageFillMode
                );
            }
        ...
        private Widget CreatePopupInterface(Objects.Abstract obj)
        {
            ...
            ToolTip.Default.SetToolTip(
                this.popupInterfaceRotateButton,
                Res.Strings.Action.ImageRotateLeft
            );
            ...
            if (filename != null)
            {
                using (
                    this.document.Modifier.OpletQueueBeginAction(
                        Res.Strings.Action.ImageImport,
                        "ChangeImageFilename"
                    )
                )
                {
        ...
        private void PopupInterfaceSwapFill(Objects.Abstract obj)
        {
            ...
            using (
                this.document.Modifier.OpletQueueBeginAction(
                    isFill ? Res.Strings.Action.ImageShowAll : Res.Strings.Action.ImageFillMode,
                    "ChangeImageZoom"
                )
            )
            {
        ...
        public override bool Compare(Abstract property)
        {
            ...
            if (p.date != this.date)
                return false;
            if (p.shortName != this.shortName)
                return false;
            if (p.insideDoc != this.insideDoc)
                return false;
            if (p.fromClipboard != this.fromClipboard)
                return false;
            if (p.rotation != this.rotation)
                return false;
            if (p.mirrorH != this.mirrorH)
                return false;
            if (p.mirrorV != this.mirrorV)
                return false;
            if (p.homo != this.homo)
                return false;
            if (p.filterCategory != this.filterCategory)
                return false;
            if (p.cropMargins != this.cropMargins)
                return false;
        ...
        public static BitmapFileFormat FilterQualityToMode(double quality)
        {
            ...
            return new BitmapFileFormat() { Type = BitmapFileType.Jpeg, Quality = n };
        }
        ...
        public static string FilterIndexToName(int index)
        {
            ...
            switch (index)
            {
                ...
                case 1:
                    return "Bilinear"; // filtrage simple et rapide
                case 2:
                    return "Blackman"; // normal
                case 3:
                    return "Bicubic"; // flou
                case 4:
                    return "Sinc"; // accentué
                default:
                    return null;
            }
        }
        ...
        public static string FilterNameToText(string name)
        {
            ...
            switch (name)
            {
                ...
                case "Blackman":
                    return Res.Strings.Panel.Image.Filter.Blackman;
                case "Bicubic":
                    return Res.Strings.Panel.Image.Filter.Bicubic;
                case "Sinc":
                    return Res.Strings.Panel.Image.Filter.Sinc;
                default:
                    return string.Format(Res.Strings.Panel.Image.Filter.Other, name);
            }
        }

        public static ImageFilter CategoryToFilter(
            ...
            switch (name)
            {
                ...
                case "Bilinear":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingBilinear
                            : ImageFilteringMode.Bilinear
                    );
                case "Bicubic":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingBicubic
                            : ImageFilteringMode.Bicubic
                    );
                case "Spline16":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingSpline16
                            : ImageFilteringMode.Spline16
                    );
                case "Spline36":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingSpline36
                            : ImageFilteringMode.Spline36
                    );
                case "Kaiser":
                    return new ImageFilter(
                        resampling ? ImageFilteringMode.ResamplingKaiser : ImageFilteringMode.Kaiser
                    );
                case "Quadric":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingQuadric
                            : ImageFilteringMode.Quadric
                    );
                case "Catrom":
                    return new ImageFilter(
                        resampling ? ImageFilteringMode.ResamplingCatrom : ImageFilteringMode.Catrom
                    );
                case "Gaussian":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingGaussian
                            : ImageFilteringMode.Gaussian
                    );
                case "Bessel":
                    return new ImageFilter(
                        resampling ? ImageFilteringMode.ResamplingBessel : ImageFilteringMode.Bessel
                    );
                case "Mitchell":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingMitchell
                            : ImageFilteringMode.Mitchell
                    );
                case "Sinc":
                    return new ImageFilter(
                        resampling ? ImageFilteringMode.ResamplingSinc : ImageFilteringMode.Sinc,
                        1.0
                    );
                case "Lanczos":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingLanczos
                            : ImageFilteringMode.Lanczos,
                        1.0
                    );
                case "Blackman":
                    return new ImageFilter(
                        resampling
                            ? ImageFilteringMode.ResamplingBlackman
                            : ImageFilteringMode.Blackman,
                        1.0
                    );
                default:
                    return new ImageFilter(ImageFilteringMode.None);
