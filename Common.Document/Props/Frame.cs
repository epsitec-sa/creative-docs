...
namespace Epsitec.Common.Document.Properties
{
    ...
    public class Frame : Abstract, Support.IXMLSerializable<Frame>
    {
        ...
        {
            get { return this.marginConvexity; }
            set
            {
                if (this.marginConvexity != value)
                {
                    this.NotifyBefore();
                    this.marginConvexity = value;
                    this.NotifyAfter();
                }
            }
        }
        ...
        {
            get { return this.shadowConvexity; }
            set
            {
                if (this.shadowConvexity != value)
                {
                    this.NotifyBefore();
                    this.shadowConvexity = value;
                    this.NotifyAfter();
                }
            }
        }
        ...
        public override string SampleText
        {
            ...
            get { return Frame.GetName(this.frameType); }
        }
        ...
        public static FrameType ConvType(int index)
        {
            ...
            switch (index)
            {
                case 0:
                    ...
                    break;
                case 1:
                    type = FrameType.OnlyFrame;
                    break;
                case 2:
                    type = FrameType.FrameAndShadow;
                    break;
                case 3:
                    type = FrameType.OnlyShadow;
                    break;
            }
            return type;
        }
        ...
        public static string GetName(FrameType type)
        {
            ...
            {
                case FrameType.None:
                    name = Res.Strings.Property.Frame.None;
                    break;
                case FrameType.OnlyFrame:
                    name = Res.Strings.Property.Frame.OnlyFrame;
                    break;
                case FrameType.FrameAndShadow:
                    name = Res.Strings.Property.Frame.FrameAndShadow;
                    break;
                case FrameType.OnlyShadow:
                    name = Res.Strings.Property.Frame.OnlyShadow;
                    break;
            }
            return name;
        }
        ...
        public static string GetIconText(FrameType type)
        {
            ...
            switch (type)
            {
                case FrameType.None:
                    return "FrameNone";
                case FrameType.OnlyFrame:
                    return "FrameOnlyFrame";
                case FrameType.FrameAndShadow:
                    return "FrameFrameAndShadow";
                case FrameType.OnlyShadow:
                    return "FrameOnlyShadow";
            }
            return "";
        }
        ...
        public override bool IsHandleVisible(Objects.Abstract obj, int rank)
        {
            ...
            {
                if (
                    this.frameType == Properties.FrameType.OnlyFrame
            ...
            {
                if (
                    this.frameType == Properties.FrameType.OnlyShadow
                    || this.frameType == Properties.FrameType.FrameAndShadow
                )
                {
                    return true;
        ...
        public override Point GetHandlePosition(Objects.Abstract obj, int rank)
        {
            ...
            if (polygon != null && polygon.Count >= 2)
            {
                if (rank == 0) // margin width ?
                {
                    polygon = polygon.InflateAndMakeConvexe(this.marginWidth, 0);
                ...
                if (rank == 1) // shadow inflate ?
                {
                    ...
                    polygon = polygon.InflateAndMakeConvexe(
                        this.marginWidth + this.shadowInflate,
                        0
                    );
                ...
                if (rank == 2) // shadow size ?
                {
                    polygon = polygon.Move(this.shadowOffsetX, this.shadowOffsetY);
                    polygon = polygon.InflateAndMakeConvexe(
                        this.marginWidth + this.shadowInflate + this.shadowSize,
                        0
                    );
                    return polygon.GetPoint(0);
                }
        ...
        )
        {
            ...
            if (polygon != null)
            {
                ...
                )
                {
                    var center = polygon.Center;
                    var pa = new Point(
                        center.X + this.shadowOffsetX,
                        center.Y + this.shadowOffsetY
                    );
                    ...
                    if (Point.Distance(center, pa) > radius)
                    {
                        ...
                        graphics.AddLine(
                            pa,
                            Geometry.ComputeArrowExtremity(center, pa, 0.4, 0.2, 0)
                        );
                        graphics.AddLine(
                            pa,
                            Geometry.ComputeArrowExtremity(center, pa, 0.4, 0.2, 1)
                        ); // flèche
                ...
                )
                {
                    ...
                    {
                        polygon = polygon.InflateAndMakeConvexe(this.shadowSize, 0);
                        Drawer.DrawPathDash(
                            graphics,
                            drawingContext,
                            polygon.PolygonPath,
        ...
        public override void CopyTo(Abstract property)
        {
            ...
            p.marginConvexity = this.marginConvexity;
            p.backgroundColor = this.backgroundColor;
            p.shadowInflate = this.shadowInflate;
            p.shadowConvexity = this.shadowConvexity;
        ...
        public override bool Compare(Abstract property)
        {
            ...
            if (p.frameType != this.frameType)
                return false;
            if (p.frameWidth != this.frameWidth)
                return false;
            if (p.frameColor != this.frameColor)
                return false;
            if (p.marginWidth != this.marginWidth)
                return false;
            if (p.marginConvexity != this.marginConvexity)
                return false;
            if (p.backgroundColor != this.backgroundColor)
                return false;
            if (p.shadowInflate != this.shadowInflate)
                return false;
            if (p.shadowConvexity != this.shadowConvexity)
                return false;
            if (p.shadowSize != this.shadowSize)
                return false;
            if (p.shadowOffsetX != this.shadowOffsetX)
                return false;
            if (p.shadowOffsetY != this.shadowOffsetY)
                return false;
            if (p.shadowColor != this.shadowColor)
                return false;
        ...
        )
        {
            ...
            {
                var pp = Polygon.InflateAndMakeConvexe(
                    polygons,
                    this.marginWidth,
                    this.marginConvexity
                );
                ...
                {
                    var pp1 = Polygon.InflateAndMakeConvexe(
                        polygons,
                        this.marginWidth + this.shadowInflate,
                        this.shadowConvexity
                    );
                    var pp2 = Polygon.Move(pp1, this.shadowOffsetX, this.shadowOffsetY);
                    var shadowPath = Polygon.GetPolygonPathCorner(
                        drawingContext,
                        pp2,
                        corner,
                        false
                ...
                )
                {
                    if (this.frameWidth > 0)
                    {
                        var shape = new Shape();
                        shape.Path = path;
                        shape.SetPropertyStroke(
                            port,
                            this.GetPropertyFrameStroke(obj),
                            this.GetPropertyFrameSurface(obj)
                        );
                        shapes.Add(shape);
                    }
                }
        ...
        public override void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("MarginConvexity", this.marginConvexity, typeof(double));
            info.AddValue("BackgroundColor", this.backgroundColor);

            info.AddValue("ShadowInflate", this.shadowInflate, typeof(double));
            info.AddValue("ShadowConvexity", this.shadowConvexity, typeof(double));
            ...
            : base(info, context)
        {
            ...
            if (this.document.IsRevisionGreaterOrEqual(3, 0, 4))
            {
                try
                {
                    ...
                    this.marginConvexity = (double)info.GetValue("MarginConvexity", typeof(double));
                    this.backgroundColor = (RichColor)
                        info.GetValue("BackgroundColor", typeof(RichColor));

                    this.shadowInflate = (double)info.GetValue("ShadowInflate", typeof(double));
                    this.shadowConvexity = (double)info.GetValue("ShadowConvexity", typeof(double));
