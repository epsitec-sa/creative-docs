...
namespace Epsitec.Cresus.Assets.Server.Engine
{
	...
	public class TestMandatFactory : AbstractMandatFactory
	{
		...
		{
			this.amortizations.Preview (range, objectGuid);
		...
		protected override void AddReports()
		{
			...
			this.accessor.Mandat.Reports.Add (new MCH2SummaryParams (null, dateRange, Guid.Empty, 1, Guid.Empty, summaryType: MCH2SummaryType.Indirect, skipHiddenRows: true));

			{
				var group  = this.GetGroup ("Catégories MCH2");
				var filter = this.GetGroup ("Patrimoine administratif");
				this.accessor.Mandat.Reports.Add (new MCH2SummaryParams ("Patrimoine administratif MCH2 &lt;DATE&gt; &lt;SUMMARYTYPE&gt;", dateRange, group.Guid, 1, filter.Guid, summaryType: MCH2SummaryType.Indirect, skipHiddenRows: true));
			}

			{
				var group  = this.GetGroup ("Catégories MCH2");
				var filter = this.GetGroup ("Patrimoine financier");
				this.accessor.Mandat.Reports.Add (new MCH2SummaryParams ("Patrimoine financier MCH2 &lt;DATE&gt; &lt;SUMMARYTYPE&gt;", dateRange, group.Guid, 1, filter.Guid, summaryType: MCH2SummaryType.Indirect, skipHiddenRows: true));
			}
