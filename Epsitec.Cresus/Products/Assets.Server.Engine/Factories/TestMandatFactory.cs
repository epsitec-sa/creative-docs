...
namespace Epsitec.Cresus.Assets.Server.Engine
{
	...
	public class TestMandatFactory : AbstractMandatFactory
	{
		...
		protected override void AddReports()
		{
			...
			this.accessor.Mandat.Reports.Add (new MCH2SummaryParams (null, dateRange, Guid.Empty, 1, Guid.Empty, indirect: false));

			{
				var group  = this.GetGroup ("Catégories MCH2");
				var filter = this.GetGroup ("Patrimoine administratif");
				this.accessor.Mandat.Reports.Add (new MCH2SummaryParams ("Patrimoine administratif MCH2 &lt;DATE&gt;", dateRange, group.Guid, 1, filter.Guid, indirect: false));
			}

			{
				var group  = this.GetGroup ("Catégories MCH2");
				var filter = this.GetGroup ("Patrimoine financier");
				this.accessor.Mandat.Reports.Add (new MCH2SummaryParams ("Patrimoine financier MCH2 &lt;DATE&gt;", dateRange, group.Guid, 1, filter.Guid, indirect: false));
			}
