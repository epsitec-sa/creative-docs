...
namespace Epsitec.Cresus.Assets.App.Widgets
{
	...
	public class TimelineRowGlyphs : AbstractTimelineRow, Epsitec.Common.Widgets.Helpers.IToolTipHost
	{
		...
		private static void PaintCellBackground(Graphics graphics, Rectangle rect, TimelineCellGlyph cell, bool isHover, int index, bool isLocked)
		{
			...
			if (isLocked)
			{
				TimelineRowGlyphs.PaintHash (graphics, rect);
			}
		}
		...
		#region Hash painting
		private static void PaintHash(Graphics graphics, Rectangle rect)
		{
			...
			for (double y=d; y<=rect.Height; y+=d)
			{
				...
				var end = TimelineRowGlyphs.GetHashEnd (start, rect);
		...
		private static Point GetHashEnd(Point start, Rectangle rect)
		{
