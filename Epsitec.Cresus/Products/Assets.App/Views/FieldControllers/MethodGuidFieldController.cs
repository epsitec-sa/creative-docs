...
{
	public class MethodGuidFieldController : AbstractFieldController
	{
		public MethodGuidFieldController(DataAccessor accessor)
			: base (accessor)
		{
		}
		...
		public override void CreateUI(Widget parent)
		{
			...
			this.CreateGotoExpressionButton ();
			this.UpdateError ();
			this.UpdatePropertyState ();
		...
		private void CreateGotoExpressionButton()
		{
			this.gotoButton = this.CreateGotoButton ();

			ToolTip.Default.SetToolTip (this.gotoButton, Res.Strings.FieldControllers.Method.Goto.ToString ());

			this.gotoButton.Clicked += delegate
			{
				if (!this.value.IsEmpty)
				{
					var viewState = MethodsView.GetViewState (this.value, ObjectField.Unknown);
		...
		private void UpdateButtons()
		{
			if (this.button != null)
			{
				var text = MethodsLogic.GetSummary (this.accessor, this.value);
		...
		{
			MethodsPopup.Show (this.button, this.accessor, this.Value, delegate (Guid guid)
			{
				this.Value = guid;
				this.UpdateError ();
				this.OnValueEdited (this.Field);
		...
		{
			return MethodsLogic.GetSummary (this.accessor, guid);
		}
