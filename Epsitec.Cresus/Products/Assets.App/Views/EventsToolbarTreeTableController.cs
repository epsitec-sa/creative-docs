	...
	public class EventsToolbarTreeTableController : AbstractToolbarTreeTableController
	{
		...
		private void UpdateCategoriesContent(int firstRow, int count, int selection, bool crop = true)
		{
			...
			for (int i=0; i<count; i++)
			{
				...
				var glyph  = TimelineData.TypeToGlyph (eventType);
				var type   = StaticDescriptions.GetEventDescription (eventType);
				var taux   = ObjectCalculator.GetObjectPropertyDecimal (this.obj, timestamp, ObjectField.TauxAmortissement);
				var typeAm = ObjectCalculator.GetObjectPropertyString (this.obj, timestamp, ObjectField.TypeAmortissement);
				var period = ObjectCalculator.GetObjectPropertyString (this.obj, timestamp, ObjectField.Périodicité);
				var residu = ObjectCalculator.GetObjectPropertyDecimal (this.obj, timestamp, ObjectField.ValeurRésiduelle);
				var nom    = ObjectCalculator.GetObjectPropertyString (this.obj, timestamp, ObjectField.Nom);
				var numéro = ObjectCalculator.GetObjectPropertyString (this.obj, timestamp, ObjectField.Numéro);
