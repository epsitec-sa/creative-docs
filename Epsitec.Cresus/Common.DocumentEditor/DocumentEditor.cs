...
//		[Command ("SelectAll")]
		...
		protected CommandState CreateCommandState(string commandName, bool statefull, params Shortcut[] shortcuts)
		{
			return this.CreateCommandState(commandName, null, commandName, statefull, shortcuts);
		}

		protected CommandState CreateCommandState(string commandName, string icon, bool statefull, params Shortcut[] shortcuts)
		{
			return this.CreateCommandState(commandName, icon, commandName, statefull, shortcuts);
		}

		protected CommandState CreateCommandState(string commandName, string icon, params Shortcut[] shortcuts)
		{
			return this.CreateCommandState(commandName, icon, commandName, false, shortcuts);
		}

		protected CommandState CreateCommandState(string commandName, string icon, string tooltip, params Shortcut[] shortcuts)
		{
			return this.CreateCommandState(commandName, icon, tooltip, false, shortcuts);
		}

		protected CommandState CreateCommandState(string commandName, string icon, string tooltip, bool statefull, params Shortcut[] shortcuts)
		{
		...
		private void HandleOriginChanged()
		{
			...
			if ( this.HasCurrentDocument )
			{
				...
				this.zoomPageState.ActiveState = context.IsZoomPage ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
				this.zoomPageWidthState.ActiveState = context.IsZoomPageWidth ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
				this.zoomDefaultState.ActiveState = context.IsZoomDefault ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
			}
			else
			{
		...
		private void HandleZoomChanged()
		{
			...
			if ( this.HasCurrentDocument )
			{
				...
				this.zoomPageWidthState.Enable = true;
				this.zoomDefaultState.Enable = true;
				this.zoomMinState.ActiveState = (context.Zoom <= this.CurrentDocument.Modifier.ZoomMin) ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
				this.zoomPageState.ActiveState = context.IsZoomPage ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
				this.zoomPageWidthState.ActiveState = context.IsZoomPageWidth ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
				this.zoomDefaultState.ActiveState = context.IsZoomDefault ? Common.Widgets.ActiveState.Yes : Common.Widgets.ActiveState.No;
		...
		protected void StartCheck(bool always)
		{
			...
			this.checker = new VersionChecker (typeof (Common.DocumentEditor.Application).Assembly);
