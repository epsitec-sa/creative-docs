	...
	public sealed class UnitTestLockManager
	{
		...
		public void GetLocksTest2()
		{
			using (DbInfrastructure dbinfrastructure = DbInfrastructureHelper.ConnectToTestDatabase ())
			{
				...
				this.CheckLock (locks1["lock1"], c1, "lock1", System.DateTime.Now);
				this.CheckLock (locks1["lock2"], c1, "lock2", System.DateTime.Now);
				this.CheckLock (locks1["lock3"], c2, "lock3", System.DateTime.Now);
				this.CheckLock (locks1["lock4"], c3, "lock4", System.DateTime.Now);

				var locks2 = lockManager.GetLocks (new List<string> () { "lock1", "lock2", }).ToDictionary (l => l.Name);

				Assert.AreEqual (2, locks2.Count);
				this.CheckLock (locks2["lock1"], c1, "lock1", System.DateTime.Now);
				this.CheckLock (locks2["lock2"], c1, "lock2", System.DateTime.Now);

				var locks3 = lockManager.GetLocks (new List<string> () { "lock3", }).ToDictionary (l => l.Name);
				
				Assert.AreEqual (1, locks3.Count);
				this.CheckLock (locks3["lock3"], c2, "lock3", System.DateTime.Now);

				var locks4 = lockManager.GetLocks (new List<string> () { "lock4", }).ToDictionary (l => l.Name);

				Assert.AreEqual (1, locks4.Count);
				this.CheckLock (locks4["lock4"], c3, "lock4", System.DateTime.Now);
