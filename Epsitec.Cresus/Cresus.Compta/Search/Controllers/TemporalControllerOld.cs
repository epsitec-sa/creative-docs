...
namespace Epsitec.Cresus.Compta.Search.Controllers
{
	...
	/// </summary>
	public class TemporalControllerOld
	{
		public TemporalControllerOld(AbstractController controller, TemporalData data)
		{
		...
		private void CreateMainUI()
		{
			...
			TemporalControllerOld.InitTemporalDataDurationCombo (this.durationField);

			//	Connexion des événements.
			this.durationField.SelectedItemChanged += delegate
			{
				this.data.Duration = TemporalControllerOld.TemporalDataDurationToType (this.durationField.FormattedText);
		...
		private void UpdateButtons()
		{
			using (this.ignoreChanges.Enter ())
			{
				...
				this.durationField.FormattedText = TemporalControllerOld.TemporalDataDurationToString (this.data.Duration);
		...
		private int DescriptionBestFitWidth
		{
			...
			get
			{
				...
				{
					width = TemporalControllerOld.GetDescriptionBestFitWidth (this.data.Duration);
		...
		private static int GetDescriptionBestFitWidth(TemporalDataDuration duration)
		{
			...
			int max = 0;
			foreach (var dr in TemporalControllerOld.GetDateRangeSamples (duration))
			{
		...
		private static void InitTemporalDataDurationCombo(TextFieldCombo combo)
		{
			...
			foreach (var type in TemporalControllerOld.TemporalDataDurations)
			{
				combo.Items.Add (TemporalControllerOld.TemporalDataDurationToString (type));
			}
		}
		...
		private static TemporalDataDuration TemporalDataDurationToType(FormattedText text)
		{
			foreach (var type in TemporalControllerOld.TemporalDataDurations)
			{
				if (TemporalControllerOld.TemporalDataDurationToString (type) == text)
				{
					return type;
				}
			}
