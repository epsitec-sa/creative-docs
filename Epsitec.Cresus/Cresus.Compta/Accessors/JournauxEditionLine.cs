...
namespace Epsitec.Cresus.Compta.Accessors
{
	...
	public class JournauxEditionLine : AbstractEditionLine
	{
		public JournauxEditionLine(AbstractController controller)
			: base (controller)
		{
			this.dataDict.Add (ColumnType.Titre,   new EditionData (this.controller, this.ValidateTitle));
			this.dataDict.Add (ColumnType.Libellé, new EditionData (this.controller));
		...
		private void ValidateTitle(EditionData data)
		{
			...
			if (data.HasText)
			{
				var journal = this.compta.Journaux.Where (x => x.Nom == data.Text).FirstOrDefault ();
				if (journal == null)
				{
					return;
				}
		...
		private void ValidatePièce(EditionData data)
		{
			...
			if (data.HasText)
			{
				var pièce = JournauxDataAccessor.GetPiècesGenerator (this.compta, data.Text);
				if (pièce == null)
				{
		...
		public override void DataToEntity(AbstractEntity entity)
		{
			...
			journal.PiècesGenerator = JournauxDataAccessor.GetPiècesGenerator (this.compta, this.GetText (ColumnType.Pièce));
		}
	}
}