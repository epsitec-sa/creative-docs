	...
	public static class LambdaComputer
	{
		...
		private class ComputableExpressionFinder : ExpressionVisitor
		{
			...
			public override Expression Visit(Expression expression)
			{
				...
				// method, we can't return a bool which would have been very useful. That's why we
				// use a stack of bools to store the values that should have been returned if we
				// could have done so.

				// We start by assuming that the current expression can be computed if we are said
				// so by the callback that we are given.
				...
				{
					// Either the current expression or at least one of its children cannot be
					// computed, which implies that the parent expression cannot be computed.
				...
				{
					// The current expression as well as all its children can be computed, so we add
					// it to the list of expressions that can be computed.
