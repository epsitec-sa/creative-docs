...
{
	/// <summary>
	/// La classe ContainerStyles contient tous les panneaux des styles.
	/// </summary>
	[SuppressBundleSupport]
	public class ContainerStyles : AbstractContainer
	{
		public ContainerStyles(Document document) : base(document)
		{
		...
		public override void Hilite(AbstractObject hiliteObject)
		{
			...
			for ( int i=0 ; i<total ; i++ )
			{
				...
				bool hilite = AbstractContainer.IsObjectUseByProperty(property, hiliteObject);
		...
		{
			AbstractProperty property = this.document.PropertiesStyle[row] as AbstractProperty;
			...
			st.Text = AbstractProperty.Text(property.Type);
		...
		protected void UpdatePanel()
		{
			...
			if ( sel != -1 )
			{
				AbstractProperty property = this.document.PropertiesStyle[sel] as AbstractProperty;
				...
				this.panelStyleName = new PanelStyleName(this.document);
		...
		private void HandleTableFlyOverChanged(object sender)
		{
			...
			if ( rank != -1 )
			{
				property = this.document.PropertiesStyle[rank] as AbstractProperty;
			}
		...
		protected VMenu CreateMenu(Point pos)
		{
			...
			foreach ( int value in System.Enum.GetValues(typeof(PropertyType)) )
			{
				...
				if ( !AbstractProperty.StyleAbility(type) )  continue;

				if ( back != -1 && back != AbstractProperty.BackgroundIntensity(type) )
				{
					menu.Items.Add(new MenuSeparator());
				}
				back = AbstractProperty.BackgroundIntensity(type);

				MenuItem item = new MenuItem("StyleCreate", "", AbstractProperty.Text(type), "", AbstractProperty.TypeName(type));
		...
		private void HandleMenuPressed(object sender, MessageEventArgs e)
		{
			...
			int sel = this.document.PropertiesStyle.Selected;
			PropertyType type = AbstractProperty.TypeName(item.Name);
		...
		protected PanelStyleName		panelStyleName;
