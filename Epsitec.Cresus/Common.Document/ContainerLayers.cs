...
{
	/// <summary>
	/// La classe ContainerLayers contient tous les panneaux des calques.
	/// </summary>
	[SuppressBundleSupport]
	public class ContainerLayers : AbstractContainer
	{
		public ContainerLayers(Document document) : base(document)
		{
			...
			this.panelModColor = new PanelModColor(this.document);
			...
			this.panelLayerName = new PanelLayerName(this.document);
		...
		protected override void DoUpdateObject(AbstractObject obj)
		{
			ObjectLayer layer = obj as ObjectLayer;
			DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
			ObjectPage page = context.RootObject(1) as ObjectPage;
		...
		protected void UpdateTable()
		{
			...
			for ( int i=0 ; i<rows ; i++ )
			{
				...
				ObjectLayer layer = page.Objects[ii] as ObjectLayer;
				this.TableFillRow(i);
				this.TableUpdateRow(i, layer);
			}
		}
		...
		protected void TableUpdateRow(int row, ObjectLayer layer)
		{
			DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
			StaticText st;
			...
			st.Text = ObjectLayer.LayerPositionName(n, context.TotalLayers());
		...
		protected void UpdatePanel()
		{
			...
			int sel = context.CurrentLayer;
			ObjectPage page = context.RootObject(1) as ObjectPage;
			ObjectLayer layer = page.Objects[sel] as ObjectLayer;
		...
		private void HandleRadioClicked(object sender, MessageEventArgs e)
		{
			using ( this.document.Modifier.OpletQueueBeginAction() )
			{
				...
				if ( sender == this.radioShow   )  type = LayerType.Show;
				if ( sender == this.radioDimmed )  type = LayerType.Dimmed;
				if ( sender == this.radioHide   )  type = LayerType.Hide;
				int sel = context.CurrentLayer;
				ObjectPage page = context.RootObject(1) as ObjectPage;
				ObjectLayer layer = page.Objects[sel] as ObjectLayer;
		...
		private void HandleButtonClicked(object sender, MessageEventArgs e)
		{
			using ( this.document.Modifier.OpletQueueBeginAction() )
			{
				DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
				LayerType type = LayerType.None;
				if ( sender == this.buttonShow   )  type = LayerType.Show;
				if ( sender == this.buttonDimmed )  type = LayerType.Dimmed;
				if ( sender == this.buttonHide   )  type = LayerType.Hide;
				ObjectPage page = context.RootObject(1) as ObjectPage;
				int total = context.TotalLayers();
				for ( int i=0 ; i<total ; i++ )
				{
					ObjectLayer layer = page.Objects[i] as ObjectLayer;
					layer.Type = type;
				}
				this.UpdateRadio();
		...
		protected PanelLayerName		panelLayerName;
		...
		protected PanelModColor			panelModColor;
