...
namespace Epsitec.Common.Document
{
	...
	public abstract class AbstractPanel : Widgets.Widget
	{
		public AbstractPanel(Document document)
		{
		...
		// Container éventuel propriétaire du panneau.
		...
		private void HandleMouseEntered(object sender, MessageEventArgs e)
		{
			...
			{
				AbstractObject obj = this.property.Owners[i] as AbstractObject;
		...
		public AbstractProperty Property
		{
			...
			{
				this.property = value;
				this.backgroundIntensity = AbstractProperty.BackgroundIntensity(this.property.Type);
		...
		public virtual void OriginColorChange(Color color)
		{
		}

		// Donne la couleur d'origine.
		public virtual Color OriginColorGet()
		{
		...
		private void ExtendedButtonClicked(object sender, MessageEventArgs e)
		{
			...
			{
				ContainerProperties cp = this.container as ContainerProperties;
				if ( cp != null )
				{
					cp.IsPropertiesExtended(this.property.Type, this.isExtendedSize);
				}
			}
		}
		...
		protected override void PaintBackgroundImplementation(Graphics graphics, Rectangle clipRect)
		{
			...
			color = Color.FromRGB(color.R*this.backgroundIntensity, color.G*this.backgroundIntensity, color.B*this.backgroundIntensity);
#else
			Color cap = adorner.ColorCaption;
			Color color = Color.FromARGB(1.0-this.backgroundIntensity, 0.5+cap.R*0.5, 0.5+cap.G*0.5, 0.5+cap.B*0.5);
#endif
