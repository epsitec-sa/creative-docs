...
namespace Epsitec.Common.Document
{
	...
	{
		public PropertyArrow(Document document) : base(document)
		{
		...
		public override Point GetHandlePosition(AbstractObject obj, int rank)
		{
			...
			if ( obj is ObjectBezier )
			{
				ObjectBezier bezier = obj as ObjectBezier;
				Point a, b;
		...
		{
			return new PanelArrow(document);
		}
								   ...
								   Point p1, Point p2)
		{
			double len = this.length[extremity];
			double ef1 = this.effect1[extremity];
			Point pa = PropertyArrow.Extremity(p1, p2, len, len*ef1);
		...
		protected PropertyArrow(SerializationInfo info, StreamingContext context) : base(info, context)
		{
