...
namespace Epsitec.Common.Document.Panels
{
    ...
    {
        public ModColor(Document document)
            : base(document)
        {
            this.fieldArray = new Widgets.TextFieldLabel[7];
            for (int i = 0; i < 7; i++)
            {
                this.fieldArray[i] = new Widgets.TextFieldLabel(
                    this,
                    Widgets.TextFieldLabel.Type.TextFieldReal
                );

                if (i == 0)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.H;
                if (i == 1)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.S;
                if (i == 2)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.V;
                if (i == 3)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.R;
                if (i == 4)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.G;
                if (i == 5)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.B;
                if (i == 6)
                    this.fieldArray[i].LabelShortText = Res.Strings.Panel.ModColor.Short.A;
                ...
                if (i == 0)
                {
                    this.document.Modifier.AdaptTextFieldRealAngle(
                        this.fieldArray[i].TextFieldReal
                    );
                }
                else
                {
                    this.document.Modifier.AdaptTextFieldRealScalar(
                        this.fieldArray[i].TextFieldReal
                    );
        ...
        public override double DefaultHeight
        {
            ...
            get { return (this.isExtendedSize ? this.LabelHeight + 79 : this.LabelHeight + 30); }
        }
        ...
        protected override void UpdateClientGeometry()
        {
            ...
            r.Left =
                rect.Left
                + Widgets.TextFieldLabel.ShortWidth
                + Widgets.TextFieldLabel.DefaultLabelWidth
                + Widgets.TextFieldLabel.DefaultMarginWidth;
