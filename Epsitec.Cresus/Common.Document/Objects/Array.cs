...
{
	/// <summary>
	/// La classe Array est la classe de l'objet graphique "tableau".
	/// </summary>
	[System.Serializable()]
	public class Array : Objects.Abstract
	{
		...
		public Array(Document document, Objects.Abstract model) : base(document, model)
		{
		...
		protected override bool ExistingProperty(Properties.Type type)
		{
			...
			if ( type == Properties.Type.BackColor )  return true;
		...
		{
			return new Array(document, model);
		}
		...
		public override Abstract GetProperty(Type type)
		{
			if ( type == Type.Name     ||
				 type == Type.TextFont )
			{
			...
			if ( type == Type.LineMode  ||
				 type == Type.LineColor )  // trait ?
			{
			...
			found:
			Abstract copy = Abstract.NewProperty(this.document, type);
		...
		public override void SetProperty(Abstract property)
		{
			this.onlyBase = true;
			base.SetProperty(property);
			this.onlyBase = false;

			if ( property.Type == Type.LineMode  ||
				 property.Type == Type.LineColor )  // trait ?
			{
				for ( int c=0 ; c<this.columns ; c++ )
				{
					for ( int r=0 ; r<this.rows ; r++ )
		...
		protected bool PropertySelected(Properties.Abstract property, int c, int r, OutlinePos pos)
		{
			if ( this.selected )
			{
			...
			Properties.Abstract actual = this.GetProperty(property.Type, c,r, pos);
			if ( actual == null )  return false;
			return ( actual == property );
		}

		protected bool PropertySelected(Properties.Abstract property, int c, int r)
		{
			if ( this.selected )
			{
			...
			Properties.Abstract actual = this.Cell(c,r).Property(property.Type, 0);
			if ( actual == null )  return false;
			return ( actual == property );
		}
		...
		protected Properties.Abstract GetPropertySel(Properties.Type type, int c, int r, OutlinePos pos)
		{
		...
		protected Properties.Abstract GetProperty(Properties.Type type, int c, int r, OutlinePos pos)
		{
			switch ( pos  )
			{
		...
		protected void SetProperty(Properties.Abstract property, int c, int r, OutlinePos pos)
		{
			switch ( pos  )
			{
		...
		protected void LookHeader1()
		{
			...
			for ( int c=c1 ; c<=c2 ; c++ )
			{
				for ( int r=r1 ; r<=r2 ; r++ )
				{
					...
					this.Cell(c,r).BackColor.ColorValue = color;
		...
		protected void LookAlter2()
		{
			...
			for ( int c=c1 ; c<=c2 ; c++ )
			{
				...
				{
					Drawing.Color color;
					if ( r%2 ==0 )  color = Color.FromRGB(0.9, 1.0, 1.0);  // bleuté
		...
		protected void LookSerie(string arg)
		{
			...
			for ( int c=c1 ; c<=c2 ; c++ )
			{
				...
				{
					this.Cell(c,r).Content = Array.Serie(arg, rank++);
				}
			}
		}
		...
		public override bool EditRulerLink(TextRuler ruler, DrawingContext drawingContext)
		{
			...
			Properties.Justif justif = this.Cell(c,r).TextJustif;
			left  += justif.MarginH;
		...
		protected void DrawDraft(Graphics graphics, DrawingContext drawingContext)
		{
			...
			if ( this.draftStep == 0 )  // choix taille d'une cellule ?
			{
				...
				graphics.RenderSolid(cell.BackColor.ColorValue);

				graphics.Rasterizer.AddOutline(path, cell.LeftLine.Width);
				graphics.RenderSolid(cell.LeftColor.ColorValue);
			}
		...
		protected void DrawCellText(IPaintPort port, DrawingContext drawingContext, int c, int r)
		{
			...
			Properties.JustifVertical   jv = this.Cell(c,r).TextJustif.Vertical;
			Properties.JustifHorizontal jh = this.Cell(c,r).TextJustif.Horizontal;

			if ( jv == Properties.JustifVertical.Top )
			{
				     if ( jh == Properties.JustifHorizontal.Center )  textLayout.Alignment = ContentAlignment.TopCenter;
				else if ( jh == Properties.JustifHorizontal.Right  )  textLayout.Alignment = ContentAlignment.TopRight;
				else                                       textLayout.Alignment = ContentAlignment.TopLeft;
			}
			if ( jv == Properties.JustifVertical.Center )
			{
				     if ( jh == Properties.JustifHorizontal.Center )  textLayout.Alignment = ContentAlignment.MiddleCenter;
				else if ( jh == Properties.JustifHorizontal.Right  )  textLayout.Alignment = ContentAlignment.MiddleRight;
				else                                       textLayout.Alignment = ContentAlignment.MiddleLeft;
			}
			if ( jv == Properties.JustifVertical.Bottom )
			{
				     if ( jh == Properties.JustifHorizontal.Center )  textLayout.Alignment = ContentAlignment.BottomCenter;
				else if ( jh == Properties.JustifHorizontal.Right  )  textLayout.Alignment = ContentAlignment.BottomRight;
				else                                       textLayout.Alignment = ContentAlignment.BottomLeft;
			}

			     if ( jh == Properties.JustifHorizontal.Justif )  textLayout.JustifMode = TextJustifMode.AllButLast;
			else if ( jh == Properties.JustifHorizontal.All    )  textLayout.JustifMode = TextJustifMode.All;
		...
		protected void DrawCellSurface(IPaintPort port, DrawingContext drawingContext, int c, int r)
		{
			...
			if ( port is Graphics )
			{
				Graphics graphics = port as Graphics;
				graphics.Rasterizer.AddSurface(path);
				graphics.RenderSolid(drawingContext.AdaptColor(this.Cell(c,r).BackColor.ColorValue));
			}
			else
			{
				port.Color = this.Cell(c,r).BackColor.ColorValue;
		...
		protected void DrawCellOutline(IPaintPort port, DrawingContext drawingContext, int c, int r)
		{
			...
			{
				Graphics graphics = port as Graphics;
				line.DrawPath(graphics, drawingContext, path, color.ColorValue);
			}
			else
			{
				Printing.PrintPort pp = port as Printing.PrintPort;
				pp.Color = color.ColorValue;
		...
		protected Array(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		#endregion
