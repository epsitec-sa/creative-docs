...
namespace Epsitec.Common.Document.Objects
{
	...
	public class Ellipse : Objects.Abstract
	{
		...
		protected void TextInfoModif()
		{
			Point p1 = this.Handle(0).Position;
			Point p2 = this.Handle(1).Position;
			double width = System.Math.Abs(p1.X-p2.X)/2.0/this.document.Modifier.RealScale;
			double height = System.Math.Abs(p1.Y-p2.Y)/2.0/this.document.Modifier.RealScale;
			string text = string.Format("rx={0}, ry={1}", width.ToString("F1"), height.ToString("F1"));
