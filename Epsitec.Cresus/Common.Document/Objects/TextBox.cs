    ...
    public class TextBox : Objects.Abstract
    {
        ...
        protected override bool ExistingProperty(Properties.Type type)
        {
            ...
            if (type == Properties.Type.TextJustif)
                return true;
            if (type == Properties.Type.TextFont)
                return true;
            return false;
        }
        ...
        public string SimpleText
        {
            ...
            get { return TextLayout.ConvertToSimpleText(this.textLayout.Text); }
        }
        ...
        public override void MoveHandleProcess(int rank, Point pos, DrawingContext drawingContext)
        {
            ...
            )
            {
                ...
                if (rank == 0)
                {
                    this.Handle(2).Position = Point.Projection(
                        this.Handle(2).Position,
                        this.Handle(1).Position,
                        pos
                    );
                    this.Handle(3).Position = Point.Projection(
                        this.Handle(3).Position,
                        this.Handle(1).Position,
                        pos
                    );
                ...
                if (rank == 2)
                {
                    this.Handle(0).Position = Point.Projection(
                        this.Handle(0).Position,
                        this.Handle(3).Position,
                        pos
                    );
                    this.Handle(1).Position = Point.Projection(
                        this.Handle(1).Position,
                        this.Handle(3).Position,
                        pos
                    );
        ...
        )
        {
            ...
            Point p3 = new Point();
            Point p4 = new Point();
            if (!this.InitTextLayout(ref p1, ref p2, ref p3, ref p4, null))
                return;
        ...
        public override void DrawText(IPaintPort port, DrawingContext drawingContext)
        {
            ...
            Point p3 = new Point();
            Point p4 = new Point();
            if (!this.InitTextLayout(ref p1, ref p2, ref p3, ref p4, drawingContext))
                return;
            ...
            {
                if (jh == Properties.JustifHorizontal.Center)
                    this.textLayout.Alignment = ContentAlignment.TopCenter;
                else if (jh == Properties.JustifHorizontal.Right)
                    this.textLayout.Alignment = ContentAlignment.TopRight;
                else
            ...
            if (jv == Properties.JustifVertical.Center)
            {
                if (jh == Properties.JustifHorizontal.Center)
                    ...
                    this.textLayout.Alignment = ContentAlignment.MiddleRight;
                else
            ...
            {
                if (jh == Properties.JustifHorizontal.Center)
                    this.textLayout.Alignment = ContentAlignment.BottomCenter;
                else if (jh == Properties.JustifHorizontal.Right)
                    this.textLayout.Alignment = ContentAlignment.BottomRight;
                else
                    this.textLayout.Alignment = ContentAlignment.BottomLeft;
            }

            if (jh == Properties.JustifHorizontal.Justif)
                this.textLayout.JustifMode = TextJustifMode.AllButLast;
            else if (jh == Properties.JustifHorizontal.All)
                this.textLayout.JustifMode = TextJustifMode.All;
            else
                this.textLayout.JustifMode = TextJustifMode.None;
            ...
            )
            {
                ...
                int from = System.Math.Min(
                    this.textNavigator.Context.CursorFrom,
                    this.textNavigator.Context.CursorTo
                );
                int to = System.Math.Max(
                    this.textNavigator.Context.CursorFrom,
                    this.textNavigator.Context.CursorTo
                );
                TextLayout.SelectedArea[] areas = this.textLayout.FindTextRange(
                    Point.Zero,
                    from,
                    to
                );
                for (int i = 0; i < areas.Length; i++)
                {
                    ...
                    Drawing.Rectangle r = new Drawing.Rectangle(
                        this.transform.TransformDirect(box.BottomLeft),
                        this.transform.TransformDirect(box.TopRight)
                    );
            ...
            if (port is Graphics && this.edited && this.textNavigator.Context.CursorTo != -1)
            {
                Graphics graphics = port as Graphics;
                Point c1,
                    c2;
        ...
        protected TextBox(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
            ...
            Drawing.TextStyle.Tab[] tabs = (Drawing.TextStyle.Tab[])
                info.GetValue("TabArray", typeof(Drawing.TextStyle.Tab[]));
