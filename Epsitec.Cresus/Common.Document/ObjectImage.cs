...
namespace Epsitec.Common.Document
{
	...
	{
		public ObjectImage(Document document, AbstractObject model) : base(document, model)
		{
		...
		{
			PropertyImage image = this.PropertyImage;
			if ( image.Filename == "" )
			{
			...
			else
			{
				if ( image.Filename != this.filename )
				{
					...
					try
					{
						this.imageOriginal = Bitmap.FromFile(this.filename);
					}
					catch
					{
						this.imageOriginal = null;
		...
		public override void DrawGeometry(Graphics graphics, DrawingContext drawingContext)
		{
			...
			Image image = drawingContext.IsDimmed ? this.imageDimmed : this.imageOriginal;
			if ( image == null )
			{
				path = this.PathBuildOutline();
				graphics.Rasterizer.AddOutline(path, 1.0/drawingContext.ScaleX);
			...
			else
			{
				...
				if ( width > 0 && height > 0 )
				{
					PropertyImage property = this.PropertyImage;
		...
		protected ObjectImage(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		#endregion
