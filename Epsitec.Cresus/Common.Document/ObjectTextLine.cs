...
namespace Epsitec.Common.Document
{
	...
	{
		public ObjectTextLine(Document document, AbstractObject model) : base(document, model)
		{
		...
		protected override bool ExistingProperty(PropertyType type)
		{
			...
			if ( type == PropertyType.TextLine )  return true;
		...
		{
			return new ObjectTextLine(document, model);
		}
		...
		public override bool Detect(Rectangle rect, bool all)
		{
			if ( this.isHide )  return false;
		...
		protected static Rectangle AdvanceBounds(Font font, string text)
		{
			System.Diagnostics.Debug.Assert(text.Length == 1);
			if ( text[0] == TextLayout.CodeEndOfText )
			{
				return new Rectangle(0, font.Descender, 0.000001, font.Ascender-font.Descender);
			}
			else
			{
		...
		{
			PropertyTextLine justif = this.PropertyTextLine;
								   ...
								   out double angle)
		{
			...
			Rectangle gb = ObjectTextLine.AdvanceBounds(font, character);
		...
		protected void BboxTextCurve(ref Rectangle bbox)
		{
		...
		protected ObjectTextLine(SerializationInfo info, StreamingContext context) : base(info, context)
		{
		}
		#endregion
