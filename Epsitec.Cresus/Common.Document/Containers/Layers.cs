    ...
    public class Layers : Abstract
    {
            ...
            : base(document)
        {
            ...
            ToolTip.Default.SetToolTip(
                this.buttonShow,
                Res.Strings.Container.Layers.Button.HelpShow
            );
            ...
            ToolTip.Default.SetToolTip(
                this.buttonDimmed,
                Res.Strings.Container.Layers.Button.HelpDimmed
            );
            ...
            ToolTip.Default.SetToolTip(
                this.buttonHide,
                Res.Strings.Container.Layers.Button.HelpHide
            );
        ...
        protected void TableUpdateRow(int row, Objects.Layer layer, bool select)
        {
            ...
            if (layer.Type == Objects.LayerType.Show)
                state = ActiveState.Yes;
            if (layer.Type == Objects.LayerType.Dimmed)
                state = ActiveState.Maybe;
            if (select)
                state = ActiveState.Yes;
        ...
        private void HandleButtonMagnetLayerClicked(object sender, MessageEventArgs e)
        {
            ...
            DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
            if (sel < 0 || sel >= context.TotalLayers())
                return;
            ...
            using (
                this.document.Modifier.OpletQueueBeginAction(Res.Strings.Action.LayerChangeMagnet)
            )
            {
        ...
        private void HandleCheckActiveStateChanged(object sender)
        {
            ...
            using (
                this.document.Modifier.OpletQueueBeginAction(Res.Strings.Action.LayerChangeShowOne)
            )
            {
                Objects.LayerType type = Objects.LayerType.None;
                if (bt.ActiveState == ActiveState.Yes)
                    type = Objects.LayerType.Show;
                if (bt.ActiveState == ActiveState.No)
                    type = Objects.LayerType.Hide;
                if (bt.ActiveState == ActiveState.Maybe)
                    type = Objects.LayerType.Dimmed;
                layer.Type = type;
        ...
        protected void UpdateExtended()
        {
            ...
            this.extendedButton.GlyphShape = this.isExtended
        ...
        private void HandleRadioPrintChanged(object sender)
        {
            ...
            using (
                this.document.Modifier.OpletQueueBeginAction(Res.Strings.Action.LayerChangePrint)
            )
            {
                DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
                Objects.LayerPrint print = Objects.LayerPrint.None;
                if (sender == this.radioShowPrint)
                    print = Objects.LayerPrint.Show;
                if (sender == this.radioDimmedPrint)
                    print = Objects.LayerPrint.Dimmed;
                if (sender == this.radioHidePrint)
                    print = Objects.LayerPrint.Hide;
                int sel = context.CurrentLayer;
                Objects.Page page = context.RootObject(1) as Objects.Page;
                Objects.Layer layer = page.Objects[sel] as Objects.Layer;
        ...
        private void UpdateRadio()
        {
            ...
            this.radioShowPrint.ActiveState =
                (print == Objects.LayerPrint.Show) ? ActiveState.Yes : ActiveState.No;
            this.radioDimmedPrint.ActiveState =
                (print == Objects.LayerPrint.Dimmed) ? ActiveState.Yes : ActiveState.No;
            this.radioHidePrint.ActiveState =
                (print == Objects.LayerPrint.Hide) ? ActiveState.Yes : ActiveState.No;
        }
        ...
        private void HandleButtonClicked(object sender, MessageEventArgs e)
        {
            ...
            using (
                this.document.Modifier.OpletQueueBeginAction(Res.Strings.Action.LayerChangeShowAll)
            )
            {
                DrawingContext context = this.document.Modifier.ActiveViewer.DrawingContext;
                Objects.LayerType type = Objects.LayerType.None;
                if (sender == this.buttonShow)
                    type = Objects.LayerType.Show;
                if (sender == this.buttonDimmed)
                    type = Objects.LayerType.Dimmed;
                if (sender == this.buttonHide)
                    type = Objects.LayerType.Hide;
                Objects.Page page = context.RootObject(1) as Objects.Page;
