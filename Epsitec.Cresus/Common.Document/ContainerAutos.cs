...
{
	/// <summary>
	/// La classe ContainerAutos contient tous les panneaux des styles.
	/// </summary>
	[SuppressBundleSupport]
	public class ContainerAutos : AbstractContainer
	{
		public ContainerAutos(Document document) : base(document)
		{
		...
		public override void Hilite(AbstractObject hiliteObject)
		{
			...
			for ( int i=0 ; i<total ; i++ )
			{
				...
				{
					property = this.document.PropertiesAuto[i] as AbstractProperty;
				}
				else
				{
					int ii = i-this.document.PropertiesAuto.Count;
					property = this.document.PropertiesSel[ii] as AbstractProperty;
				}

				bool hilite = ContainerProperties.IsObjectUseByProperty(property, hiliteObject);
		...
		protected void UpdateTable()
		{
			...
			for ( int i=0 ; i<rows ; i++ )
			{
				...
				else
				{
					...
					property = this.document.PropertiesSel[j] as AbstractProperty;
					select = true;
				}
				...
				if ( select )
				{
					st.Text = string.Format("<b>{0}</b>", AbstractProperty.Text(property.Type));
				}
				else
				{
		...
		private void HandleTableFlyOverChanged(object sender)
		{
			...
			if ( rank != -1 )
			{
				...
				{
					property = this.document.PropertiesAuto[rank] as AbstractProperty;
				}
				else
				{
					rank -= this.document.PropertiesAuto.Count;
					property = this.document.PropertiesSel[rank] as AbstractProperty;
				}
			}
