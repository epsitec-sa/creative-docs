...
namespace Epsitec.Common.Document.TextPanels
{
    ...
    {
        public ParagraphLayout(Document document)
            : base(document)
        {
            ...
            this.buttonAlignLeft = this.CreateIconButton(
                Misc.Icon("JustifHLeft"),
                Res.Strings.Action.Text.Paragraph.AlignLeft,
                new MessageEventHandler(this.HandleJustifClicked)
            );
            this.buttonAlignCenter = this.CreateIconButton(
                Misc.Icon("JustifHCenter"),
                Res.Strings.Action.Text.Paragraph.AlignCenter,
                new MessageEventHandler(this.HandleJustifClicked)
            );
            this.buttonAlignRight = this.CreateIconButton(
                Misc.Icon("JustifHRight"),
                Res.Strings.Action.Text.Paragraph.AlignRight,
                new MessageEventHandler(this.HandleJustifClicked)
            );
            this.buttonAlignJustif = this.CreateIconButton(
                Misc.Icon("JustifHJustif"),
                Res.Strings.Action.Text.Paragraph.AlignJustif,
                new MessageEventHandler(this.HandleJustifClicked)
            );

            this.buttonHyphen = this.CreateIconButton(
                Misc.Icon("TextHyphen"),
                Res.Strings.Action.Text.Paragraph.Hyphen,
                new MessageEventHandler(this.HandleHyphenClicked)
            );

            this.fieldLeftMarginFirst = this.CreateTextFieldLabel(
                Res.Strings.Action.Text.Ruler.HandleLeftFirst,
                Res.Strings.TextPanel.ParagraphLayout.Short.LeftFirst,
                Res.Strings.TextPanel.ParagraphLayout.Long.LeftFirst,
                0.0,
                100.0,
                1.0,
                false,
                new EventHandler(this.HandleMarginChanged)
            );
            this.fieldLeftMarginBody = this.CreateTextFieldLabel(
                Res.Strings.Action.Text.Ruler.HandleLeftBody,
                Res.Strings.TextPanel.ParagraphLayout.Short.LeftBody,
                Res.Strings.TextPanel.ParagraphLayout.Long.LeftBody,
                0.0,
                100.0,
                1.0,
                false,
            ...
            this.fieldRightMargin = this.CreateTextFieldLabel(
                Res.Strings.Action.Text.Ruler.HandleRight,
                Res.Strings.TextPanel.ParagraphLayout.Short.Right,
                Res.Strings.TextPanel.ParagraphLayout.Long.Right,
                0.0,
                100.0,
                1.0,
                false,
            ...
            this.document.ParagraphWrapper.Active.Changed += new EventHandler(
                this.HandleWrapperChanged
            );
            this.document.ParagraphWrapper.Defined.Changed += new EventHandler(
                this.HandleWrapperChanged
            );
        ...
        protected override void UpdateAfterChanging()
        {
            ...
            Common.Text.Wrappers.JustificationMode justif = this.document
                .ParagraphWrapper
                .Defined
                .JustificationMode;
#if false
            ...
            this.buttonAlignLeft.ActiveState =
                (justif == Common.Text.Wrappers.JustificationMode.AlignLeft)
                    ? ActiveState.Yes
                    : ActiveState.No;
            this.buttonAlignCenter.ActiveState =
                (justif == Common.Text.Wrappers.JustificationMode.Center)
                    ? ActiveState.Yes
                    : ActiveState.No;
            this.buttonAlignRight.ActiveState =
                (justif == Common.Text.Wrappers.JustificationMode.AlignRight)
                    ? ActiveState.Yes
                    : ActiveState.No;
            this.buttonAlignJustif.ActiveState =
                (justif == Common.Text.Wrappers.JustificationMode.JustifyAlignLeft)
                    ? ActiveState.Yes
                    : ActiveState.No;
        ...
        private void HandleJustifClicked(object sender, MessageEventArgs e)
        {
            if (this.ignoreChanged)
                return;
            if (!this.document.ParagraphWrapper.IsAttached)
                return;
            ...
            if (button == null)
                return;
            button.ActiveState =
                (button.ActiveState == ActiveState.Yes) ? ActiveState.No : ActiveState.Yes;

            if (this.buttonAlignLeft != button)
                this.buttonAlignLeft.ActiveState = ActiveState.No;
            if (this.buttonAlignCenter != button)
                this.buttonAlignCenter.ActiveState = ActiveState.No;
            if (this.buttonAlignRight != button)
                this.buttonAlignRight.ActiveState = ActiveState.No;
            if (this.buttonAlignJustif != button)
                this.buttonAlignJustif.ActiveState = ActiveState.No;

            Common.Text.Wrappers.JustificationMode justif = Common
                .Text
                .Wrappers
                .JustificationMode
                .Unknown;

            if (this.buttonAlignLeft.ActiveState == ActiveState.Yes)
                justif = Common.Text.Wrappers.JustificationMode.AlignLeft;
            if (this.buttonAlignCenter.ActiveState == ActiveState.Yes)
                justif = Common.Text.Wrappers.JustificationMode.Center;
            if (this.buttonAlignRight.ActiveState == ActiveState.Yes)
                justif = Common.Text.Wrappers.JustificationMode.AlignRight;
            if (this.buttonAlignJustif.ActiveState == ActiveState.Yes)
                justif = Common.Text.Wrappers.JustificationMode.JustifyAlignLeft;
        ...
        private void HandleHyphenClicked(object sender, MessageEventArgs e)
        {
            ...
            this.buttonHyphen.ActiveState =
                (this.buttonHyphen.ActiveState == ActiveState.Yes)
                    ? ActiveState.No
                    : ActiveState.Yes;
