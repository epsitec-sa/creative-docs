...
namespace Epsitec.Common.Dialogs
{
    ...
    public abstract class AbstractOkCancelDialog : AbstractMessageDialog
    {
        ...
        protected override Window CreateWindow()
        {
            ...
            button1.SetManualBounds(
                new Drawing.Rectangle(
                    dialogWindow.ClientSize.Width - 2 * 75 - 2 * 8,
                    16,
                    75,
                    button1.PreferredHeight
                )
            );
            button1.FormattedText = this.okText.IsNullOrEmpty()
                ? Widgets.Res.Strings.Dialog.Button.OK
                : this.okText;
            ...
            if (this.hideCancel == false)
            {
                ...
                button2.SetManualBounds(
                    new Drawing.Rectangle(
                        dialogWindow.ClientSize.Width - 1 * 75 - 1 * 8,
                        16,
                        75,
                        button2.PreferredHeight
                    )
                );
                button2.FormattedText = this.cancelText.IsNullOrEmpty()
                    ? Widgets.Res.Strings.Dialog.Button.Cancel
                    : this.cancelText;
                button2.Name = "Cancel";
        ...
        protected void CommandValidateDialog()
        {
            ...
            {
                this.DispatchWindow.QueueCommand(
                    this.DialogWindow,
                    string.Format(this.commandTemplate, this.CommandArgs)
                );
            }
