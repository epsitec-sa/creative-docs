...
namespace Epsitec.Common.Text
{
    internal class TextBreakEngine : Drawing.ITextBreakEngine, ILanguageRecognizer
    {
        ...
        void Epsitec.Common.Drawing.ITextBreakEngine.SetText(
            string text,
            Epsitec.Common.Drawing.TextBreakMode mode
        )
        {
            this.SetText(text, mode);
        }

        void Epsitec.Common.Drawing.ITextBreakEngine.SetRuns(
            ICollection<Epsitec.Common.Drawing.TextBreakRun> runs
        )
        {
        ...
        bool Epsitec.Common.Drawing.ITextBreakEngine.GetNextBreak(
            double maxWidth,
            out string text,
        ...
        private void AnalyzeTextBreaks()
        {
            ...
            if (
                ((this.mode & Drawing.TextBreakMode.SingleLine) != 0)
            ...
            )
            {
                ...
                {
                    if (
                        (this.breaks[i] == Unicode.BreakInfo.Yes)
        ...
        private string FindNextBreak(ref double width, out int nChars)
        {
            ...
            for (; ; )
            {
                ...
                if (
                    (breakMode != Unicode.BreakInfo.No)
                    && (breakMode != Unicode.BreakInfo.NoAlpha)
                    && (font != null)
                )
                {
                    if (
                        (breakMode == Unicode.BreakInfo.HyphenateGoodChoice)
                ...
                try_fit:
                if ((breakOffset == 0) && (offset >= 0) && (width < (advance + charWidth)))
                {
                    ...
                    if (
                        ((this.mode & Drawing.TextBreakMode.Ellipsis) != 0)
                        && (font != null)
                        && (addEllipsis == false)
                    )
                    {
                        ...
                        if (
                            (font.OpenTypeFont.EllipsisWidth > 0)
                    ...
                    {
                        if (
                            (font.OpenTypeFont.PeriodWidth > 0)
                    ...
                    if (((this.mode & Drawing.TextBreakMode.Split) != 0) || (addEllipsis))
                    {
        ...
        bool ILanguageRecognizer.GetLanguage(
            ulong[] text,
            int offset,
            out double hyphenation,
            out string locale
        )
        {
            int runIndex = 0;

            while ((runIndex < this.runs.Count) && (this.runs[runIndex].Length <= offset))
            {
