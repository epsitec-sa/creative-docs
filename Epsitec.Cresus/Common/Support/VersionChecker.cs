...
namespace Epsitec.Common.Support
{
    ...
    public class VersionChecker
    {
        ...
        public static VersionChecker CheckUpdate(
        ...
        private VersionChecker()
        {
            ...
            System.Net.NetworkInformation.NetworkChange.NetworkAddressChanged +=
                this.HandleNetworkAddressChanged;
        }
        ...
        private VersionChecker(string version)
            : this()
        {
            VersionChecker.SplitVersionString(
                version,
                out this.currentMajor,
                out this.currentMinor,
                out this.currentRevision,
                out this.currentBuild
            );
        }
            ...
            get
            {
                return this.IsCheckSuccessful
                    && (
                        ...
                        || (
                            (this.currentMajor == this.foundMajor)
                            && (this.currentMinor < this.foundMinor)
                        )
                        || (
                            (this.currentMajor == this.foundMajor)
                            && (this.currentMinor == this.foundMinor)
                            && (this.currentRevision < this.foundRevision)
                        )
                        || (
                            (this.currentMajor == this.foundMajor)
                            && (this.currentMinor == this.foundMinor)
                            && (this.currentRevision == this.foundRevision)
                            && (this.currentBuild < this.foundBuild)
                        )
                    );
            }
        }
        ...
        {
            get { return this.foundVersion; }
        }

        public string NewerVersionUrl
        {
            get { return this.foundUrl; }
        }

        public static VersionChecker Default
        {
            get { return VersionChecker.defaultChecker; }
        }
        ...
        private void StartCheck(string url)
        {
            ...
            System.Threading.WaitCallback callback = delegate
            {
                try
                {
