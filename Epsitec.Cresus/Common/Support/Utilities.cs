...
namespace Epsitec.Common.Support
{
    ...
    public static class Utilities
    {
            ...
            if (value.IndexOf(startQuote) == 0)
            {
                if ((value.Length > 1) && (value.LastIndexOf(endQuote) == value.Length - 1))
                {
                    ...
                    for (int i = 1; i < value.Length - 1; i++)
                    {
                        ...
                        if ((c == startQuote) && (startQuote == endQuote))
                        {
                            i++;
                            c = value[i];
                            if ((i == value.Length - 1) || (c != startQuote))
                            {
                                throw new System.Exception(
                                    string.Format("Quotes mismatch in {0}", value)
                                );
                            }
                        }
                        buffer.Append(c);
        ...
        public static IEnumerable<string> StringToTokens(
            string value,
            char sep,
        ...
        public static IEnumerable<string> StringToTokens(string value, char sep, string trimchars)
        {
            ...
            System.Func<string> getToken = delegate
            {
        ...
        public static bool CheckForDuplicates(
            System.Collections.ICollection data,
                ...
                else if (c == '<')
                {
                    ...
                    if (pos < offset)
                    {
                        throw new System.FormatException(
                            string.Format("Invalid XML cannot be converted to text.")
                        );
                    }
        ...
        public static char ParseCharOrXmlEntity(string text, ref int offset)
        {
            if (text[offset] == '&')
            {
                ...
                {
                    throw new System.FormatException(
                        string.Format("Bad entity syntax in '{0}'.", text.Substring(offset))
                    );
                }
                ...
                switch (entity)
                {
                    ...
                    default:
                        ...
                        {
                            throw new System.FormatException(
                                string.Format("Unrecognized entity '{0}'.", entity)
                            );
                        }
                        break;
                }
                                ...
                                if ((depth == 0) && (c == sep))
                                {
                                    ...
                                    if (argCount >= max - 1)
                                    {
                                        throw new System.ArgumentException(
                                            string.Format("Text too complex to split: {0}.", text)
                                        );
                                    }
                                }
                                break;
        ...
        public static string[] Split(string text, string separator)
        {
            ...
            {
                throw new System.ArgumentException(
                    string.Format("Empty separator for split: {0}.", text)
                );
            }
            ...
            for (int i = 0; i < argCount; i++)
            {
                args[i] = text.Substring(
                    sepPos[i] + separator.Length,
                    sepPos[i + 1] - sepPos[i] - separator.Length
                );
            }
