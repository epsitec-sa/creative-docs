...
namespace Epsitec.Common.Types.Converters
{
    ...
    public static class TextConverter
    {
        ...
        {
            return TextConverter.ConvertToSimpleText(
                text,
                imageReplacement == 0 ? "" : imageReplacement.ToString()
            );
        }
            ...
            for (int offset = pos; offset < text.Length; )
            {
                ...
                if (c == '<')
                {
                    ...
                    if (length > 0)
                    {
                        ...
                        if (tag.StartsWith("<tab/>") || tag.StartsWith("<list"))
                        {
                            buffer.Append("\t");
                        }
            ...
            if (text[offset] == '&')
            {
                ...
                if (length < 3)
                {
                    throw new System.FormatException(
                        string.Format("Invalid entity found (too short).")
                    );
                }
                ...
                switch (entity)
                {
                    ...
                    default:
                        ...
                        {
                            throw new System.FormatException(
                                string.Format("Invalid entity {0} found.", entity)
                            );
                        }
                        break;
                }
        ...
        private static class Tables
        {
            ...
            public static char[] StripAccentsToLower = new char[
                TextConverter.ConversionCharacterCount
            ];
            public static char[] StripAccentsToUpper = new char[
                TextConverter.ConversionCharacterCount
            ];
