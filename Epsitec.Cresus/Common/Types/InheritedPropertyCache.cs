    ...
    public struct InheritedPropertyCache : IInheritedPropertyCache
    {
        ...
        public void ClearAllValues(DependencyObject node)
        {
                ...
                for (int i = 0; i < InheritedPropertyCache.MaskBits; i++)
                {
                    ...
                    if ((mask & this.cachedValueFlags) != 0)
                    {
                        DependencyProperty property =
                            DependencyProperty.GetInheritedPropertyFromCacheMask(mask);
            ...
            if (this.more != null)
            {
                ...
                if ((this.more.ContainsChanges() == false) && (this.more.ContainsData() == false))
                {
                ...
                if (mask == 0)
                {
                    if (this.more != null)
                    {
                        if (node.ContainsLocalValue(property) == false)
                        {
                            ...
                            if (
                                (this.more.ContainsChanges() == false)
        ...
        public void InheritValuesFromParent(DependencyObject node, DependencyObject parent)
        {
            ...
            else
            {
                this.SetValues(
                    node,
                    InheritedPropertyCache.OnlyUndefinedEntries(
                        node,
                        parent.InheritedPropertyCache.GetValues(parent)
                    )
                );
            }
        }

        private static IEnumerable<PropertyValuePair> OnlyUndefinedEntries(
            DependencyObject node,
            IEnumerable<PropertyValuePair> entries
        )
        {
        ...
        public void SetValues(DependencyObject node, IEnumerable<PropertyValuePair> propertyValues)
        {
            ...
            foreach (PropertyValuePair propertyValue in propertyValues)
            {
                ...
                else
                {
                    ...
                    {
                        bits =
                            ((bool)propertyValue.Property.DefaultMetadata.DefaultValue)
                                ? mask
                                : (byte)0;
                        bits ^= this.defaultValues;
                        bits &= mask;
                    ...
                    if (((mask & this.cachedValueFlags) == 0) || (bits != 0))
                    {
        ...
        private static void ClearChildrenProperties(
            DependencyObject node,
        ...
        private static void SetChildrenProperties(
            DependencyObject node,
            List<PropertyValuePair> properties
        )
        {
            ...
            if (DependencyObjectTree.GetHasChildren(node))
            {
                ...
                foreach (DependencyObject child in children)
                {
                    child.InheritedPropertyCache.SetValues(
                        child,
                        InheritedPropertyCache.OnlyUndefinedEntries(child, properties)
                    );
                }
            }
        }
        ...
        private abstract class More
        {
            public abstract void ClearAllValues(
                DependencyObject node,
                ref ListWrapper<DependencyProperty> list
            );
            ...
            public abstract IEnumerable<Epsitec.Common.Types.PropertyValuePair> GetValues(
                DependencyObject node
            );
        }
        ...
        {
            public override void ClearAllValues(
                DependencyObject node,
                ref ListWrapper<DependencyProperty> list
            )
            {
            ...
            public override IEnumerable<Epsitec.Common.Types.PropertyValuePair> GetValues(
                DependencyObject node
            )
            {
                if (this.isValueDefined)
