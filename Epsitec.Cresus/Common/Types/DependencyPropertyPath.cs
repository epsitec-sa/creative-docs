...
namespace Epsitec.Common.Types
{
    public class DependencyPropertyPath : System.IEquatable<DependencyPropertyPath>
    {
        ...
        {
            get { return new Collections.ReadOnlyList<DependencyProperty>(this.elements); }
            set { this.elements = value.ToArray(); }
        }
        public bool IsEmpty
        {
            get
        ...
        public static DependencyPropertyPath Combine(
            DependencyPropertyPath p1,
            DependencyPropertyPath p2
        )
        {
            if (p1 == null)
            {
            ...
            if ((p1.path == null) || (p1.path.Length == 0))
            {
                path = p2.GetFullPath();
            }
            else if ((p2.path == null) || (p2.path.Length == 0))
            {
                path = p1.GetFullPath();
            }
            else
