...
namespace Epsitec.Common.IO
{
    ...
    public static class Tools
    {
        ...
        /// </returns>
        public static bool WaitForFileReadable(
            string path,
            int timeout,
            ShouldInterruptCallback shouldInterruptCallback
        )
        {
            bool ok = false;
            int wait = 0;
            ...
            for (
                int i = increment;
                wait < timeout;
                i = System.Math.Min(i + increment, 100 * increment)
            )
            {
            ...
            if (wait > 0)
            {
                ...
                else
                {
                    System.Diagnostics.Debug.WriteLine(
                        string.Format("Waited for file for {0} ms.", wait)
                    );
                }
            }
