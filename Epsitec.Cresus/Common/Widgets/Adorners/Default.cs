...
namespace Epsitec.Common.Widgets.Adorners
{
    ...
    public class Default : AbstractAdorner
    {
        ...
        public static Path GetGlyphPath(
            Rectangle rect,
            WidgetPaintState state,
        ...
        )
        {
            ...
            else if (style == ButtonStyle.ToolItem || style == ButtonStyle.ComboItem)
            {
                graphics.AddFilledRectangle(rect);
        ...
        )
        {
            ...
            graphics.AddFilledRectangle(rect);
            graphics.RenderSolid(
                Drawing.Color.FromBrightness(this.colorControl.GetBrightness() - 0.1)
            );
        }
        ...
        )
        {
            ...
            else
            {
                ...
                text.Paint(
                    ...
                    clipRect,
                    this.colorControlLightLight,
                    Drawing.GlyphPaintStyle.Shadow
                );
        ...
        protected void PaintHalfCircle(
            Drawing.Graphics graphics,
            Drawing.Rectangle rect,
            Drawing.Color color,
            Widgets.Direction dir
        )
        {
            ...
            switch (dir)
            {
                case Direction.Up:
                    angle = 180 + 45;
                    break; // en bas à droite
                case Direction.Down:
                    angle = 45;
                    break; // en haut à gauche
                case Direction.Left:
                    angle = 90 + 45;
                    break; // en bas à gauche
                case Direction.Right:
                    angle = 270 + 45;
                    break; // en haut à droite
            }
        ...
        )
        {
            ...
            using (Drawing.Path path = new Drawing.Path())
            {
                ...
                px = -rx;
                py = 0;
                this.RotatePoint(angle, ref px, ref py);
                path.MoveTo(c.X + px, c.Y + py);

                c1x = -rx;
                c1y = ry * Drawing.Path.Kappa;
                this.RotatePoint(angle, ref c1x, ref c1y);
                c2x = -rx * Drawing.Path.Kappa;
                c2y = ry;
                this.RotatePoint(angle, ref c2x, ref c2y);
                px = 0;
                py = ry;
                this.RotatePoint(angle, ref px, ref py);
                path.CurveTo(c.X + c1x, c.Y + c1y, c.X + c2x, c.Y + c2y, c.X + px, c.Y + py);

                c1x = rx * Drawing.Path.Kappa;
                c1y = ry;
                this.RotatePoint(angle, ref c1x, ref c1y);
                c2x = rx;
                c2y = ry * Drawing.Path.Kappa;
                this.RotatePoint(angle, ref c2x, ref c2y);
                px = rx;
                py = 0;
                this.RotatePoint(angle, ref px, ref py);
        ...
        {
            if (
                paintStyle == Drawing.GlyphPaintStyle.Disabled
