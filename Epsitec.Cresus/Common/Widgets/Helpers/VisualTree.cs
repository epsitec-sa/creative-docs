...
namespace Epsitec.Common.Widgets.Helpers
{
    ...
    public sealed class VisualTree
    {
        ...
        public static Drawing.Point MapVisualToAncestor(
            Visual visual,
            Visual ancestor,
            Drawing.Point value
        )
        ...
        public static Drawing.Rectangle MapVisualToAncestor(
            Visual visual,
            Visual ancestor,
            Drawing.Rectangle value
        )
        {
            if (ancestor == null)
            {
        ...
        public static Support.ICaptionResolver FindCaptionResolver(Visual visual)
        {
            ...
            {
                resolver =
                    visual.CaptionResolver ?? Support.ResourceManager.GetResourceManager(visual);
        ...
        public static Support.ResourceManager FindResourceManager(Visual visual)
        {
            while (visual != null)
            {
                Support.ResourceManager manager = Support.ResourceManager.GetResourceManager(
                    visual
                );
        ...
        public static void DebugDump(Visual root, int depth)
        {
            ...
            if (root != null)
            {
                ...
                System.Diagnostics.Debug.WriteLine(
                    string.Format("{0}{1}", indent, root.ToString())
                );
                System.Diagnostics.Debug.WriteLine(
                    string.Format(
                        "  {0}Dock={1}, Anchor={2}, Visibility={3}, IsVisible={4}",
                        indent,
                        root.Dock,
                        root.Anchor,
                        root.Visibility,
                ...
                System.Diagnostics.Debug.WriteLine(
                    string.Format(
                        ...
                        indent,
                        root.ActualBounds,
                        VisualTree.MapVisualToRoot(root, root.Client.Bounds)
                    )
                );
                System.Diagnostics.Debug.WriteLine(
                    string.Format(
                        "  {0}Measure DX={1}",
                        indent,
                        measureDx == null ? "none" : measureDx.ToString()
                    )
                );
                System.Diagnostics.Debug.WriteLine(
                    string.Format(
                        "  {0}Measure DY={1}",
                        indent,
                        measureDy == null ? "none" : measureDy.ToString()
                    )
                );
        ...
        private static void AddToDispatcherList(
            List<CommandDispatcher> list,
            IEnumerable<CommandDispatcher> dispatchers
        )
        {
        ...
        )
        {
            if (dispatcher != null)
            {
                ...
                {
                    list.Add(dispatcher);
                    VisualTree.AddToDispatcherList(
                        list,
                        CommandDispatcher.GetDispatcher(dispatcher)
                    );
                }
            }
        }
