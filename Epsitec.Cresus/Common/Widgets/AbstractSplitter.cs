...
namespace Epsitec.Common.Widgets
{
    public abstract class AbstractSplitter : Widget, Behaviors.IDragBehaviorHost
    {
        ...
        {
            get { return this.IsVertical == false; }
        }
            ...
            if (parent != null)
            {
                ...
                for (int i = 0; i < siblings.Length; i++)
                {
                    if (siblings[i] == widget)
                    {
                        ...
                        if (
                            ((left == null) || (left.Dock != DockStyle.Fill))
            ...
            if (parent != null)
            {
                ...
                for (int i = 0; i < siblings.Length; i++)
                {
                    if (siblings[i] == widget)
                    {
                        ...
                        if (
                            ((bottom == null) || (bottom.Dock != DockStyle.Fill))
        ...
        {
            get { return Drawing.Point.Zero; }
        }

        bool Epsitec.Common.Widgets.Behaviors.IDragBehaviorHost.OnDragBegin(
            Epsitec.Common.Drawing.Point cursor
        )
        {
        ...
        private void ProcessDragging(Drawing.Point delta)
        {
            ...
            switch (this.Dock)
            {
                ...
                case DockStyle.Right:
                    ...
                    else
                    {
                        throw new System.InvalidOperationException(
                            string.Format("Invalid splitter with DockStyle.{0}", this.Dock)
                        );
                    }
                    break;
                ...
                default:
                    throw new System.InvalidOperationException(
                        string.Format("Cannot drag splitter; DockStyle.{0}", this.Dock)
                    );
            }
        ...
        {
            if ((widget.IsVisible) && (widget.IsActualGeometryValid))
            {
        ...
        private static double GetUpdatedWidth(Widget widget, double dx)
        {
            if ((widget.IsVisible) && (widget.IsActualGeometryValid))
            {
                ...
                if ((w < 5) && (AbstractSplitter.GetAutoCollapseEnable(widget)))
                {
                    w = 0;
                }
                else
        ...
        private static double GetUpdatedHeight(Widget widget, double dy)
        {
            if ((widget.IsVisible) && (widget.IsActualGeometryValid))
            {
                ...
                if ((h < 5) && (AbstractSplitter.GetAutoCollapseEnable(widget)))
                {
                    h = 0;
                }
                else
        ...
        {
            add { this.AddUserEventHandler("SplitterDragged", value); }
            remove { this.RemoveUserEventHandler("SplitterDragged", value); }
        }
        #endregion
        ...
        public static readonly DependencyProperty AutoCollapseEnableProperty =
            DependencyProperty.RegisterAttached(
                "AutoCollapseEnable",
                typeof(bool),
                typeof(AbstractSplitter),
