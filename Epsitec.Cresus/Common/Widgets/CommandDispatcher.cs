...
namespace Epsitec.Common.Widgets
{
    ...
    public class CommandDispatcher : DependencyObject
    {
            ...
            switch (this.level)
            {
                case CommandDispatcherLevel.Root:
                    if (CommandDispatcher.defaultDispatcher != null)
                    {
                        throw new System.InvalidOperationException(
                            "Root command dispatcher already defined"
                        );
                    }
                    break;
                ...
                default:
                    throw new System.ArgumentException(
                        string.Format(
                            "CommandDispatcherLevel {0} not valid for dispatcher {1}",
                            level,
                            name
                        ),
                        "level"
        ...
        {
            get { return this.options.HasFlag(CommandDispatcherOptions.AutoForwardCommands); }
        }
        ...
        {
            get { return this.options.HasFlag(CommandDispatcherOptions.ActivateWithoutFocus); }
        }
        ...
        {
            if ((dispatcherChain != null) && (commandObject != null))
            {
        ...
        public void RegisterController(object target)
        {
            ...
            if (controller != null)
            {
                ...
                {
                    foreach (
                        System.Reflection.MemberInfo member in type.GetMembers(
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic
                        )
                    )
                    {
                        if (
                            (member.IsDefined(CommandDispatcher.commandAttributeType, true))
        ...
        public void Register(Command command, System.Action handler)
        {
            this.Register(
                command,
                delegate(CommandDispatcher d, CommandEventArgs e)
                {
                    handler();
                }
            );
        ...
        static CommandDispatcher()
        {
            ...
            CommandDispatcher.defaultDispatcher = new CommandDispatcher(
                "default",
        ...
        private void RegisterMethod(object target, System.Reflection.MethodInfo info)
        {
            object[] attributes = info.GetCustomAttributes(
                CommandDispatcher.commandAttributeType,
                true
            );
        ...
        private void RegisterMethod(
            ...
            Support.CommandAttribute attribute
        )
        {
            if (string.IsNullOrEmpty(attribute.CommandName))
            {
                this.Register(
                    Command.Get(attribute.CommandId),
                    new MethodHandler(target, methodInfo)
                );
            }
            else
            {
                this.Register(
                    Command.Get(attribute.CommandName),
                    new MethodHandler(target, methodInfo)
                );
            }
        }
        ...
        private sealed class EventSlot : ICommandDispatcher
        {
            ...
            public bool IsEmpty
            {
                get { return this.handlers.Count == 0; }
            }
        ...
        private class MethodHandler : AbstractHandler
        {
            public MethodHandler(object controller, System.Reflection.MethodInfo methodInfo)
            {
                ...
                switch (paramInfo.Length)
                {
                    ...
                    case 2: //	void Method(CommandDispatcher, CommandEventArgs)
                        if (
                            (paramInfo[0].ParameterType == typeof(CommandDispatcher))
            ...
            {
                get { return CommandDispatcher.ResolveWeakController(this.controller); }
            }
            ...
            public override bool Equals(AbstractHandler obj)
            {
                ...
                {
                    return this.Target == other.Target && this.methodInfo == other.methodInfo;
                }
            }
