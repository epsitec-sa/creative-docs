...
namespace Epsitec.Common.Drawing
{
    ...
    public struct Point
    {
        ...
        public PointDistance GetNearest<T>(
            IEnumerable<T> collection,
            System.Func<T, Point> selector
        )
        {
        ...
        {
            return new Point(
                Point.GridAlign(point.X, offset, step),
                Point.GridAlign(point.Y, offset, step)
            );
        }
        ...
        {
            return new Point(
                Point.GridAlign(point.X, offset.X, step.X),
                Point.GridAlign(point.Y, offset.Y, step.Y)
            );
        }
        ...
        {
            return Math.Equal(a.X, b.X, δ) && Math.Equal(a.Y, b.Y, δ);
        }
        ...
        public static Point Parse(string value)
        {
            ...
            if (args.Length != 2)
            {
                throw new System.ArgumentException(
                    string.Format("Invalid point specification ({0}).", value)
                );
            }
        ...
        public static bool DetectBezier(
            Point p1,
            Point s1,
            Point s2,
            Point p2,
        ...
        public static Point FromBezier(Point p1, Point s1, Point s2, Point p2, double t)
        {
            ...
            return new Point(
                p1.X * t1 + s1.X * t2 + s2.X * t3 + p2.X * t4,
        ...
        public static bool IntersectsWithHorizontal(Point a, Point b, double y, out Point i)
        {
            ...
            if ((y < System.Math.Min(a.Y, b.Y)) || (y >= System.Math.Max(a.Y, b.Y)))
            {
                return false;
            }
        ...
        public static bool IntersectsWithVertical(Point a, Point b, double x, out Point i)
        {
            ...
            if ((x < System.Math.Min(a.X, b.X)) || (x >= System.Math.Max(a.X, b.X)))
            {
                return false;
            }
