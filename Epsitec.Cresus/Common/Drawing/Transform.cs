...
namespace Epsitec.Common.Drawing
{
    ...
    public struct Transform : System.IComparable
    {
        ...
        public bool OnlyScaleOrTranslate
        {
            get
            {
                if ((Transform.IsZero(this.xy)) && (Transform.IsZero(this.yx)))
                {
                    return true;
                }
                else
        ...
        public Transform RotateDeg(double angle, Point center)
        {
            if (angle != 0)
            {
                return this.MultiplyBy(
                    Transform.CreateRotationDegTransform(angle, center.X, center.Y)
                );
            }
            else
            {
        ...
        {
            return new Point(a.xx * b.X + a.xy * b.Y + a.tx, a.yx * b.X + a.yy * b.Y + a.ty);
        }
