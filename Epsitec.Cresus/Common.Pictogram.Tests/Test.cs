...
namespace Epsitec.Common.Pictogram
{
	...
	public class PictogramTest
	{
		[Test] public void CheckApplication()
		{
			...
			window.Root.LayoutChanged += new EventHandler(this.HandleRootLayoutChanged);
			...
			for ( int i=0 ; i<texts.Length ; i++ )
			{
				...
				{
					string name = @"file:images/" + texts[i] + @".icon";
					IconButton button = new IconButton(name);
			...
			check.Width = 100;
			check.Location = new Drawing.Point(100, window.ClientSize.Height-115);
			...
			Drawing.Point pos = new Drawing.Point(100, window.ClientSize.Height-140);
		...
		private void HandleRootLayoutChanged(object sender)
		{
			...
			double zoom = System.Math.Max(root.Height/400, 1);
		...
		[Test] public void CheckIconEditor()
		{
			...
			window.ClientSize = new Drawing.Size(800, 580);
