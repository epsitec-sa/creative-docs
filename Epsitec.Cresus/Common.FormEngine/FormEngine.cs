...
namespace Epsitec.Common.FormEngine
{
	public class FormEngine
	{
		...
		public Widget CreateForm(Druid entityId)
		{
			...
			this.CreateFormRows (entityType, root, grid, "Data.", 0, entitiesBlackList, ref row);

			return root;
		}

		private void CreateFormRows(StructuredType entityType, UI.Panel root, Widgets.Layouts.GridLayoutEngine grid, string prefix, int depth, List<Druid> entitiesBlackList, ref int row)
		{
			if (depth > 3)
			{
				return;
			...
			foreach (string fieldId in entityType.GetFieldIds ())
			{
				...
				if (field.Relation == FieldRelation.None)
				{
					...
					placeholder.SetBinding (UI.Placeholder.ValueProperty, new Binding (BindingMode.TwoWay, prefix + field.Id));
				...
				{
					this.CreateFormRows (field.Type as StructuredType, root, grid, prefix + field.Id + ".", depth+1, entitiesBlackList, ref row);
				}
			}
		}
