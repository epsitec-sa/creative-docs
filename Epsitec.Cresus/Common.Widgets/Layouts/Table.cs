...
namespace Epsitec.Common.Widgets.Layouts
{
	...
	/// <summary>
	/// La classe Table implémente un Layout Manager basé sur une table.
	/// </summary>
	public class Table : Abstract
	{
		public Table()
		{
		...
		public Table.ColumnCollection	Columns
		{
		...
		protected override void Rebuild()
		{
			...
			this.AnalyseVerticals (this.panel, 0, ref max_index, out num_lines);
			...
			this.FillListVerticals (this.Panel, 0);
		...
		protected override void UpdateGeometry()
		{
			...
			this.LayoutWidgets (this.panel, 0, 0, 0);
			...
			this.panel.Size = new Drawing.Size (this.current_width, this.current_height);
			this.panel.Invalidate ();
		}
		...
		{
			double dy = this.panel.Client.Height;
			...
			for (int i = 1; i < this.verticals.Length; i++)
			{
				...
				{
					graphics.AddFilledRectangle (x-Table.GripperRadius,  0-Table.GripperRadius, Table.GripperRadius*2, Table.GripperRadius*2);
					graphics.AddFilledRectangle (x-Table.GripperRadius, dy-Table.GripperRadius, Table.GripperRadius*2, Table.GripperRadius*2);
				}
			}
		...
		internal bool ProcessMessage(Message message)
		{
			...
			if (message.IsMouseType)
			{
				...
				Drawing.Point rel_mouse = this.panel.MapRootToClient (abs_mouse);
		...
		{
			Widget widget = this.panel.FindChild (mouse);
			
			if (widget != null)
			{
				while (widget != this.panel)
				{
					if (widget.Dock == DockStyle.Layout)
					{
		...
		protected void WidgetDraggingEnd(Drawing.Point mouse)
		{
			...
			if (this.designer.IsDropTargetValid)
			{
				...
				int pos_old = this.panel.Children.IndexOf (this.designer.Widget);
				...
				if (pos_old != pos_new)
				{
					...
					this.panel.Children.InsertAt (pos_new, this.designer.Widget);
				}
		...
		protected int  FindWidgetIndexBeforeHotDrop()
		{
			...
			{
				Table.Horizontal horizontal = this.horizontals[i];
			...
			int pos_new = (find == null) ? 0 : (this.panel.Children.IndexOf (find) + 1);
			int pos_old = this.panel.Children.IndexOf (this.designer.Widget);
		...
		{
			Table.Horizontal horizontal = this.horizontals[index];
		...
		{
			Table.Horizontal[] copy = new Table.Horizontal[this.horizontals.Length+1];
			...
			copy[index] = new Table.Horizontal (0);
		...
		{
			public ColumnCollection(Table host)
			{
				this.host = host;
				this.list = new ArrayList ();
			}
			...
			public Table.Column			this[int index]
			{
				get
				{
					...
					return this.list[index] as Table.Column;
				}
			}
		...
		protected Table.Vertical[]		verticals;
		protected Table.Horizontal[]	horizontals;
