...
[assembly: Epsitec.Common.Types.DependencyClass(
    typeof(Epsitec.Common.Tests.Types.CaptionTest.Stuff)
)]
...
namespace Epsitec.Common.Tests.Types
{
    ...
    public class CaptionTest
    {
        ...
        public void CheckBindingCaptionExtraction()
        {
            ...
            Assert.AreEqual(
                "Name",
                target.GetBindingExpression(Stuff.NameProperty).GetSourceName()
            );
            Assert.AreEqual(
                Druid.Empty,
                target.GetBindingExpression(Stuff.NameProperty).GetSourceCaptionId()
            );
            Assert.AreEqual(
                "Default.String",
                target.GetBindingExpression(Stuff.NameProperty).GetSourceNamedType().Name
            );
            Assert.AreEqual(
                "StringType",
                target.GetBindingExpression(Stuff.NameProperty).GetSourceNamedType().GetType().Name
            );

            Assert.AreEqual(
                "Text",
                target.GetBindingExpression(Stuff.TextProperty).GetSourceName()
            );
            Assert.AreEqual(
                Druid.FromLong(0x0000400000000004L),
                target.GetBindingExpression(Stuff.TextProperty).GetSourceCaptionId()
            );
            Assert.AreEqual(
                "Default.String",
                target.GetBindingExpression(Stuff.TextProperty).GetSourceNamedType().Name
            );

            Assert.AreEqual(
                "Enum",
                target.GetBindingExpression(Stuff.EnumProperty).GetSourceName()
            );
            Assert.AreEqual(
                Druid.Empty,
                target.GetBindingExpression(Stuff.EnumProperty).GetSourceCaptionId()
            );
            Assert.IsNotNull(target.GetBindingExpression(Stuff.EnumProperty).GetSourceNamedType());
            Assert.AreEqual(
                "EnumType",
                target.GetBindingExpression(Stuff.EnumProperty).GetSourceNamedType().GetType().Name
            );
            Assert.AreEqual(
                "OneOfThree",
                target.GetBindingExpression(Stuff.EnumProperty).GetSourceNamedType().Name
            );
            Assert.AreEqual(
                Druid.FromLong(0x0000400000000005L),
                target.GetBindingExpression(Stuff.EnumProperty).GetSourceNamedType().CaptionId
            );
        }
        ...
        public void CheckChanged()
        {
            ...
            caption2.Changed += delegate(object sender)
            {
                count++;
            };
        ...
        internal class Stuff : DependencyObject
        {
            static Stuff()
            {
                DependencyPropertyMetadata metadataText = Stuff.TextProperty.GetMetadata(
                    typeof(Stuff)
                );
                DependencyPropertyMetadata metadataEnum = Stuff.EnumProperty.GetMetadata(
                    typeof(Stuff)
                );
            ...
            public static readonly DependencyProperty NameProperty =
                DependencyProperty.RegisterAttached("Name", typeof(string), typeof(Stuff));
        ...
        private static EnumType GetMyEnumEnumType()
        {
            ...
            {
                enumType[MyEnum.None]
                    .DefineCaption(
                        Epsitec.Common.Support.Resources.DefaultManager.GetCaption(
                            Druid.FromLong(0x0000400000000006L)
                        )
                    );
                enumType[MyEnum.First]
                    .DefineCaption(
                        Epsitec.Common.Support.Resources.DefaultManager.GetCaption(
                            Druid.FromLong(0x0000400000000007L)
                        )
                    );
                enumType[MyEnum.Second]
                    .DefineCaption(
                        Epsitec.Common.Support.Resources.DefaultManager.GetCaption(
                            Druid.FromLong(0x0000400000000008L)
                        )
                    );
                enumType[MyEnum.Third]
                    .DefineCaption(
                        Epsitec.Common.Support.Resources.DefaultManager.GetCaption(
                            Druid.FromLong(0x0000400000000009L)
                        )
                    );
            }
