	...
	public sealed class UnitTestEntityKeyProxy
	{
		...
		public void DiscardWriteEntityValueTest()
		{
			using (DataContext dataContext = new DataContext (DatabaseHelper.DbInfrastructure))
			{
				...
				Assert.IsFalse (proxy.DiscardWriteEntityValue (new TestStore (), "", ref obj));
			}
		}
		...
		public void GetReadEntityValueTest()
		{
			using (DataContext dataContext = new DataContext (DatabaseHelper.DbInfrastructure))
			{
				...
				object person2 = proxy.GetReadEntityValue (testStore, "myId");
				object person3 = testStore.GetValue ("myId");
