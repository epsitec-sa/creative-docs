...
namespace Epsitec.Common.Tests.UI
{
    ...
    public class PanelTest
    {
        ...
        public void CheckInteractive()
        {
            ...
            field1.Anchor =
                Epsitec.Common.Widgets.AnchorStyles.TopLeft
                | Epsitec.Common.Widgets.AnchorStyles.Right;
        ...
        public void CheckSampleDataSourceWithBinding()
        {
            ...
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Record.A")
            );
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Record.B")
            );
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Record.R")
            );

            placeholder1.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.OneWay, "Record.A")
            );
            placeholder2.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.OneWay, "Record.B")
            );
            placeholder3.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.OneWay, "Record.R.R.A")
            );
        ...
        public void CheckSampleDataSourceWithBindingRealTypes()
        {
            ...
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Person.FirstName")
            );
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Person.LastName")
            );
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Person.Zip")
            );
            Assert.AreEqual(
                UnknownValue.Value,
                StructuredTree.GetValue(panel.DataSource, "Person.City")
            );

            placeholder1.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.TwoWay, "Person.FirstName")
            );
            placeholder2.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.TwoWay, "Person.LastName")
            );
            placeholder3.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.TwoWay, "Person.Zip")
            );
            placeholder4.SetBinding(
                Placeholder.ValueProperty,
                new Binding(BindingMode.TwoWay, "Person.City")
            );
