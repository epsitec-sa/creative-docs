    ...
    public class ScreenInfoTest
    {
        ...
        public void CheckAllScreens()
        {
            ...
            System.Console.Out.WriteLine(
                "Found {0} screens. GlobalArea={1}",
                screens.Length,
                ScreenInfo.GlobalArea
            );
            System.Console.Out.WriteLine();
        ...
        [Test]
        public void CheckFindPoint()
        {
            ScreenInfo[] screens = ScreenInfo.AllScreens;

            foreach (ScreenInfo info in screens)
            {
                ScreenInfo found = ScreenInfo.Find(
                    new Point(
                        info.Bounds.Left + info.Bounds.Width / 2,
                        info.Bounds.Bottom + info.Bounds.Height / 2
                    )
                );
        ...
        [Test]
        public void CheckFindRectangle()
        {
            ScreenInfo[] screens = ScreenInfo.AllScreens;
            ...
            foreach (ScreenInfo info in screens)
            {
                Rectangle rect1 = new Rectangle(
                    info.Bounds.X - 100,
                    info.Bounds.Bottom + info.Bounds.Height / 2 - 100,
                    120,
                    200
                );
                ...
                System.Console.Out.WriteLine(
                    "First rect found on {0}, Bounds={1}",
                    found1.DeviceName,
                    found1.Bounds.ToString()
                );
                System.Console.Out.WriteLine(
                    "Second rect found on {0}, Bounds={1}",
                    found2.DeviceName,
                    found2.Bounds.ToString()
                );
                System.Console.Out.WriteLine();
            }
        }
        ...
        {
            Epsitec.Common.Widgets.Tools.Magnifier magnifier =
                new Epsitec.Common.Widgets.Tools.Magnifier();
        ...
        private class Magnifier : Widget
        {
            ...
            )
            {
                ...
                if ((this.mask == null) || (this.mask_dx != dx) || (this.mask_dy != dy))
                {
                    if (this.mask != null)
                    {
                        this.mask.Dispose();
