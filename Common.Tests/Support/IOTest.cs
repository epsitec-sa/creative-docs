...
namespace Epsitec.Common.Tests.Support
{
    ...
    public class IOTest
    {
        ...
        public void CheckBZip2Stream()
        {
            ...
            for (int i = 0; i < buffer.Length; i++)
            {
                Assert.AreEqual(
                    buffer[i],
                    read[i],
                    string.Format("offset {0}: {1} != {2}", i, (char)buffer[i], (char)read[i])
                );
            }
        }
        ...
        public void CheckDeflateCompressor()
        {
            ...
            System.Console.Out.WriteLine(
                "Raw data length: {0}, compressed: {1}, decompressed: {2}",
                data.Length,
                compressed.Length,
                decompressed.Length
            );
        ...
        public void CheckZipFile()
        {
            ...
            file.AddEntry(
                "images/i1.bin",
                new byte[] { 0x1, 0x2, 0x3, 0x4, 0x5, 0x1, 0x2, 0x3, 0x4, 0x5 }
            );
            file.AddEntry(
                "images/i2.bin",
                new byte[] { 0x1, 0x2, 0x3, 0x4, 0x5, 0x1, 0x2, 0x3, 0x4, 0x5 }
            ...
            file.AddEntry(
                "text/more.txt",
                System.Text.Encoding.UTF8.GetBytes(
                    "More simple text in a subfolder in the ZIP archive.\r\n"
                )
            );
            ...
            Assert.IsTrue(
                new System.TimeSpan(
                    System.Math.Abs(
                        file["file 2.txt"].DateTime.Ticks - read["file 2.txt"].DateTime.Ticks
                    )
                ).Seconds <= 1
            );
