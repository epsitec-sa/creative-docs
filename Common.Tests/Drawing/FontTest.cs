    ...
    public class FontTest
    {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (font.GetGlyphIndex('e') == 0)
                {
                    System.Console.Out.WriteLine(
                        "{0} ({1}/{2}) is not a Latin font ({3})",
                        font.FullName,
                        font.FaceName,
                        font.StyleName,
                        font.OpenTypeFont.FontIdentity.IsSymbolFont ? "symbol" : "-"
                    );
                }
                else
                {
        ...
        public void CheckFaces()
        {
            ...
            for (int i = 0; i < faces.Length; i++)
            {
                ...
                if (font_regular != null && font_regular.IsSynthetic)
                    buffer.Append("* regular ");
                if (font_italic != null && font_italic.IsSynthetic)
                    buffer.Append("* italic ");
                if (font_bold != null && font_bold.IsSynthetic)
                    buffer.Append("* bold ");
                if (font_bold_italic != null && font_bold_italic.IsSynthetic)
        ...
        public void CheckGlyphPaint()
        {
            ...
            System.Console.Out.WriteLine(
                "Times New Roman caret slope set to {0}.",
                font.CaretSlope
            );
        ...
        public void CheckFontPaintingByWidgets()
        {
            ...
            multi.Text =
                "Simple text<br/>Other text<br/>More <i>italic</i> text.<br/>And some <b>bold</b> text.";
        ...
        public void CheckSyntheticGlyphPaint()
        {
            ...
            System.Console.Out.WriteLine(
                "Tahoma Regular caret slope set to {0}.",
                font2.CaretSlope
            );
        ...
        public void CheckGlyphDynamicBold()
        {
            ...
            for (int bold_width = 0; bold_width < 40; bold_width += 8)
            {
                ...
                double y =
                    80 + bold_width / 2 * (font.Ascender) / font.LineHeight + size * bold_width / 8;
        ...
        public void CheckRenderingSpeedGDIPlus()
        {
            ...
            System.Console.Out.WriteLine(
                "Mean Rendering : "
                    + (tot / 100).ToString()
                    + " -> "
                    + (tot * 1000 / 100 / cpu_speed / text.Length)
                    + "ns / char in GDI+"
            );
        }

        [System.Runtime.InteropServices.DllImport("GDI32.dll")]
        static extern void TextOut(System.IntPtr hdc, int x, int y, string text, int len);

        [System.Runtime.InteropServices.DllImport("GDI32.dll")]
        static extern void SelectObject(System.IntPtr hdc, System.IntPtr hfont);
            ...
            foreach (FontFaceInfo face in Font.Faces)
            {
                if (face.Name == "Arial")
                {
                    foreach (Font font in face.GetFonts())
                    {
                        ...
                        System.Console.WriteLine(
                            "'A' bounds: {0}, advance: {1}",
                            font.GetCharBounds('A'),
                            font.GetCharAdvance('A')
                        );
                        System.Console.WriteLine(
                            "'B' bounds: {0}, advance: {1}",
                            font.GetCharBounds('B'),
                            font.GetCharAdvance('B')
                        );
                        System.Console.WriteLine(
                            "'I' bounds: {0}, advance: {1}",
                            font.GetCharBounds('I'),
                            font.GetCharAdvance('I')
                        );
                        System.Console.WriteLine(
                            "'i' bounds: {0}, advance: {1}",
                            font.GetCharBounds('i'),
                        ...
                        System.Console.WriteLine(
                            "'O' bounds: {0}, advance: {1}",
                            font.GetCharBounds('O'),
                            font.GetCharAdvance('O')
                        );
                        System.Console.WriteLine(
                            "'o' bounds: {0}, advance: {1}",
                            font.GetCharBounds('o'),
                        ...
                        System.Console.WriteLine(
                            "'p' bounds: {0}, advance: {1}",
                            font.GetCharBounds('p'),
                            font.GetCharAdvance('p')
                        );
                        System.Console.WriteLine(
                            "'q' bounds: {0}, advance: {1}",
                            font.GetCharBounds('q'),
                            font.GetCharAdvance('q')
                        );
                        System.Console.WriteLine(
                            "' ' bounds: {0}, advance: {1}",
                            font.GetCharBounds(' '),
        ...
        [Test]
        public void CheckFillPixelCache()
        {
            Graphics gra = new Epsitec.Common.Drawing.Graphics();
            int loops = 1;
            double size = 10.6;
            ...
            System.Console.Out.WriteLine(
                "Timer overhead: " + (c0) + " cycles -> " + (c0 / cpu_speed) + "us"
            );
            ...
            System.Console.Out.WriteLine(
                "Calling FillPixelCache, no work done: "
                    + (c2)
                    + " cycles -> "
                    + (c2 * 1000 / cpu_speed)
                    + "ns"
            ...
            System.Console.Out.WriteLine(
                "Mean Rendering : "
                    + (tot / loops).ToString()
                    + " cycles -> "
                    + (tot * 10 / cpu_speed / text.Length)
                    + "ns / char in Cached AGG"
            );
...
#endif
        [Test]
        public void CheckFontFaceBalloon()
        {
            int n = Font.Count;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    System.Console.Out.WriteLine(
                        font.FaceName + " / " + font.StyleName + " / " + font.OpticalName
                    );
                    System.Console.Out.WriteLine(
                        "  "
                            + font.FullName
        ...
        public void CheckXHeightAdjustment()
        {
            ...
            widget.PaintBackground += delegate(
                object sender,
                Epsitec.Common.Widgets.PaintEventArgs e
            )
            {
                ...
                g.AddText(
                    10,
                    190,
                    "1. Hello, world. Candara non-adjusted FONT attempt.",
                    Font.GetFont("Candara", "Regular"),
                    size
                );
                g.AddText(
                    10,
                    170,
                    "2. Hello, world. Candara x-adjusted FONT attempt.",
                    Font.GetFont("Candara", "Regular"),
                    scale2x
                );
                g.AddText(
                    10,
                    150,
                    "3. Hello, world. Candara cap-adjusted FONT attempt.",
                    Font.GetFont("Candara", "Regular"),
                    scale2c
                );
                ...
                g.AddText(
                    10,
                    120,
                    "4. Hello, world. Tahoma non-adjusted FONT attempt.",
                    Font.GetFont("Tahoma", "Regular"),
                    size
                );
                g.AddText(
                    10,
                    100,
                    "5. Hello, world. Tahoma x-adjusted FONT attempt.",
                    Font.GetFont("Tahoma", "Regular"),
                    scale1x
                );
                g.AddText(
                    10,
                    80,
                    "6. Hello, world. Tahoma cap-adjusted FONT attempt.",
                    Font.GetFont("Tahoma", "Regular"),
                    scale1c
                );
                ...
                g.AddText(
                    10,
                    50,
                    "7. Hello, world. Verdana non-adjusted FONT attempt.",
                    Font.GetFont("Verdana", "Regular"),
                    size
                );
                g.AddText(
                    10,
                    30,
                    "8. Hello, world. Verdana x-adjusted FONT attempt.",
                    Font.GetFont("Verdana", "Regular"),
                    scale3x
                );
                g.AddText(
                    10,
                    10,
                    "9. Hello, world. Verdana cap-adjusted FONT attempt.",
                    Font.GetFont("Verdana", "Regular"),
                    scale3c
                );
