    ...
    private void CreateBricksParishOfGermanLanguage(BrickWall<AiderGroupEntity> wall)
    {
      ...
      if (isPla)
      {
        wall.AddBrick(x => x.PlaParishGroups)
          .EnableActionMenu<ActionAiderGroupViewController11AddPlaGroup>().IfTrue(this.HasUserPowerLevel(UserPowerLevel.Administrator))
          .EnableActionMenu<ActionAiderGroupViewController12RemovePlaGroup>().IfTrue(this.HasUserPowerLevel(UserPowerLevel.Administrator))
          .Attribute(BrickMode.DefaultToSummarySubView)
          .Attribute(BrickMode.AutoGroup)
          .Attribute(BrickMode.HideAddButton)
          .Attribute(BrickMode.HideRemoveButton)
